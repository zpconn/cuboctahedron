...
from exact_profile import (
    ...
    sym_word,
    sym_vec,
    canonical_translation_combined,
...
DEFAULT_COMBINED_RESIDUAL_PORTFOLIO_OUTPUT = (
    REPO_ROOT
    / "scripts"
    / "generated"
    / "combined_residual_portfolio_profile_000000000_000100000.json"
)


def default_translation_farkas_shape_map_output(limit: int | None) -> Path:
    rank_end = EXPECTED_PAIR_WORDS if limit is None else min(EXPECTED_PAIR_WORDS, limit)
    return (
        REPO_ROOT
        / "scripts"
        / "generated"
        / f"translation_farkas_shape_map_profile_000000000_{rank_end:09d}.json"
    )
...
def default_translation_farkas_phase6w_output(limit: int | None) -> Path:
    rank_end = EXPECTED_PAIR_WORDS if limit is None else min(EXPECTED_PAIR_WORDS, limit)
    return (
        REPO_ROOT
        / "scripts"
        / "generated"
        / f"translation_farkas_phase6w_000000000_{rank_end:09d}.json"
    )
...
class CombinedResidualPortfolioProfiler:
    """Phase 6N profiler for ordered combinations of existing filters.

    This mode is deliberately untrusted and emits no Lean evidence.  It asks
    whether the current collection of semantic filters is already small enough
    when treated as an ordered portfolio instead of as independent standalone
    backends.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.nonidentity_words = 0
        self.translation_sign_assignments = 0
        self.translation_good_direction_masks = 0
        self.translation_denominator_nonpositive_masks = 0
        self.translation_zero_vector_masks = 0
        self.nonidentity_forced_balance_survivors = 0

        self.first_filter_case_counts: Counter[str] = Counter()
        self.primary_shapes = DistinctTracker(max_distinct_tracked, sample_limit)
        self.primary_shapes_by_filter: dict[str, DistinctTracker] = {}
        self.primary_shape_reuse: Counter[str] = Counter()
        self.overlap_shapes_by_filter: dict[str, DistinctTracker] = {}
        self.overlap_case_counts: Counter[str] = Counter()
        self.samples: dict[str, list[dict[str, Any]]] = {}

    def tracker_for(
        self,
        table: dict[str, DistinctTracker],
        name: str,
    ) -> DistinctTracker:
        if name not in table:
            table[name] = DistinctTracker(self.max_distinct_tracked, self.sample_limit)
        return table[name]

    def add_sample(self, filter_name: str, sample: dict[str, Any]) -> None:
        bucket = self.samples.setdefault(filter_name, [])
        if len(bucket) < self.sample_limit:
            bucket.append(sample)
    ...
    def record_primary(
        ...
        *,
        ...
        self.first_filter_case_counts[filter_name] += case_count
        if shape_key is not None:
            full_key = f"{filter_name}|{shape_key}"
            self.primary_shapes.add(full_key)
            self.tracker_for(self.primary_shapes_by_filter, filter_name).add(shape_key)
            self.primary_shape_reuse[full_key] += case_count
        if sample is not None:
            self.add_sample(filter_name, sample)
    ...
    def record_overlap(
        ...
        *,
        ...
        self.overlap_case_counts[filter_name] += case_count
        self.tracker_for(self.overlap_shapes_by_filter, filter_name).add(shape_key)
    ...
    def survivor_shape_digest(self, word: tuple[str, ...], mask: int) -> str:
        denoms, b, seq = self.denominator_values_for_choice(word, mask)
        if not all(denom > 0 for denom in denoms):
            raise ValueError("survivor_shape_digest called for non-survivor mask")
        return stable_digest(normalized_constraints_key(translation_constraints(seq, b)))

    def analyze_translation_word(
        self,
        word: tuple[str, ...],
        pref: list,
    ) -> tuple[tuple[QuadraticRow, ...], int, dict[int, str], int]:
        impact_values: list[list[Fraction]] = [[] for _ in range(13)]
        survivor_bits = 0
        survivor_shape_by_mask: dict[int, str] = {}
        zero_vector_count = 0
        ...
        for mask in range(64):
            ...
            if all(denom > 0 for denom in denoms):
                survivor_bits |= 1 << mask
                self.translation_good_direction_masks += 1
                canonical_word, canonical_mask, _sym_id = self.canonical_translation_choice(
                    word,
                    mask,
                )
                survivor_shape_by_mask[mask] = self.survivor_shape_digest(
                    canonical_word,
                    canonical_mask,
                )
            else:
                self.translation_denominator_nonpositive_masks += 1
                if b == ZERO_VEC:
                    zero_vector_count += 1
                    self.translation_zero_vector_masks += 1
        ...
        return tuple(rows), survivor_bits, survivor_shape_by_mask, zero_vector_count
    ...
    def classify_translation_leaf(
        ...
        rows, survivor_bits, survivor_shape_by_mask, _zero_count = (
            self.analyze_translation_word(word, pref)
        )
        bad_mask_count = 64 - survivor_bits.bit_count()
        if bad_mask_count:
            self.record_primary(
                "translation.generic_good_direction",
                None,
                case_count=bad_mask_count,
                sample={
                    "rank": rank,
                    "word": word_key(word),
                    "case_count": bad_mask_count,
        ...
        for mask, shape_digest in sorted(survivor_shape_by_mask.items()):
            self.record_primary(
        ...
        if bad_mask_count:
            canonical_rows, canonical_problem_key, canonical_bits, _transform, _tie = (
                canonical_lifted_problem(rows, survivor_bits)
            )
            canonical_problem_digest = stable_digest(canonical_problem_key)
            bad_cubes, _point_fallbacks, _max_cube_size = greedy_bad_cube_cover(
            ...
            for bad_cube in bad_cubes:
                ...
                self.record_overlap(
                    "translation.lifted_pb_bad_cube_diagnostic",
                    shape_key,
                    case_count=bad_cube["size"],
                )
    ...
    def classify_nonidentity_leaf(
        ...
        try:
            ...
            self.record_primary(
                "nonidentity.no_fixed_axis",
                f"linear={mat_key(matrix)}",
                sample={"rank": rank, "word": word_key(word), "linear": mat_key(matrix)},
            )
            return
        ...
        if final_dot == 0:
            ...
            self.record_primary(
                ...
                sample={"rank": rank, "word": word_key(word), "axis": vec_key(axis)},
            )
            return
        ...
        if len(set(forced_seq)) != 14:
            self.record_primary(
                ...
                NonIdentityTerminalAlgebraProfiler.bad_pair_balance_key(forced_seq),
                sample={
                    "rank": rank,
                    "word": word_key(word),
                    "axis": vec_key(axis),
        ...
        self.nonidentity_forced_balance_survivors += 1
        failure, details, key = terminal_axis_candidate_algebra_failure(forced_seq, axis)
        filter_name = f"nonidentity.terminal_algebra.{failure}"
        if failure == "simulates_valid_orbit":
            filter_name = "nonidentity.residual_singleton_fallback"
            key = f"rank={rank}"
        sample = {
            "rank": rank,
            "word": word_key(word),
        ...
        sample.update(details)
        self.record_primary(filter_name, key, sample=sample)

    def classify_leaf(self, rank: int, word: tuple[str, ...], pref: list) -> None:
        self.pair_words_scanned += 1
        if self.progress_interval and self.pair_words_scanned % self.progress_interval == 0:
            print(
                f"combined portfolio scanned {self.pair_words_scanned:,} pair words",
                file=sys.stderr,
            )
        matrix = mat_mul(pref[-1], RPAIR["x"])
        if matrix == IDENTITY:
    ...
    def decision(self) -> dict[str, Any]:
        reasons: list[str] = []
        if self.primary_shapes.exact_count is None:
            reasons.append("primary shape tracker overflowed")
        if self.primary_shapes.lower_bound > self.max_lean_leaves:
            reasons.append(
                "projected primary Lean-heavy leaves "
                f"{self.primary_shapes.lower_bound} exceed max {self.max_lean_leaves}"
            )
        elif self.primary_shapes.lower_bound > self.warn_lean_leaves:
            reasons.append(
                "warning: projected primary Lean-heavy leaves "
                f"{self.primary_shapes.lower_bound} exceed warning threshold "
                f"{self.warn_lean_leaves}"
            )
        largest_filter = self.largest_primary_filter()
        if largest_filter and largest_filter["shape_count"] > self.max_lean_leaves:
            reasons.append(
                f"largest residual family {largest_filter['filter']} has "
                f"{largest_filter['shape_count']} unique shapes"
            )
        rejected = any(not reason.startswith("warning:") for reason in reasons)
        return {
            "status": "rejected" if rejected else "accepted",
            "accepted_for_phase_6n": not rejected,
            "planned_lean_heavy_leaves_exact": self.primary_shapes.exact_count,
            "planned_lean_heavy_leaves_lower_bound": self.primary_shapes.lower_bound,
            "max_lean_leaves": self.max_lean_leaves,
            "warn_lean_leaves": self.warn_lean_leaves,
            "largest_residual_family": largest_filter,
            "notes": reasons or [
                "combined portfolio is below the bounded leaf gate",
            ],
        }

    def largest_primary_filter(self) -> dict[str, Any] | None:
        if not self.primary_shapes_by_filter:
            return None
        filter_name, tracker = max(
            self.primary_shapes_by_filter.items(),
            key=lambda item: (item[1].lower_bound, item[0]),
        )
        return {
            "filter": filter_name,
            "shape_count": tracker.lower_bound,
            "exact": tracker.exact_count is not None,
        }

    def payload(self, *, elapsed_seconds: float) -> dict[str, Any]:
        primary_by_filter = {
            name: tracker.payload()
            for name, tracker in sorted(self.primary_shapes_by_filter.items())
        }
        overlap_by_filter = {
            name: tracker.payload()
            for name, tracker in sorted(self.overlap_shapes_by_filter.items())
        }
        return {
            "schema_version": 1,
            "mode": "combined-residual-portfolio-profile",
            "trusted_as_proof": False,
            ...
            "options": {
                ...
                "translation_bad_direction": "generic GoodDirection theorem",
                "translation_survivors": "normalized Farkas shape digests",
                "nonidentity": "ordered coarse terminal algebra filters",
                "emits_lean_evidence": False,
            },
            "actual_counts": {
                "pair_words_scanned": self.pair_words_scanned,
                ...
                "nonidentity_words": self.nonidentity_words,
                "translation_sign_assignments": self.translation_sign_assignments,
                "translation_good_direction_masks":
                    self.translation_good_direction_masks,
                "translation_denominator_nonpositive_masks":
                    self.translation_denominator_nonpositive_masks,
                "translation_zero_vector_masks": self.translation_zero_vector_masks,
                "nonidentity_forced_balance_survivors":
                    self.nonidentity_forced_balance_survivors,
            },
            "primary_portfolio": {
                "first_filter_case_counts": dict(
                    sorted(self.first_filter_case_counts.items())
                ),
                "unique_primary_shapes": self.primary_shapes.payload(),
                "unique_primary_shapes_by_filter": primary_by_filter,
                "shape_reuse": {
                    "shape_count": len(self.primary_shape_reuse),
                    "shared_shape_count": sum(
                        1 for count in self.primary_shape_reuse.values()
                        if count > 1
                    ),
                    "max_reuse": max(self.primary_shape_reuse.values(), default=0),
                },
                "samples": self.samples,
            },
            "overlap_diagnostics": {
                "case_counts": dict(sorted(self.overlap_case_counts.items())),
                "unique_shapes_by_filter": overlap_by_filter,
                "notes": [
                    "Overlap diagnostics are not counted as primary generated leaves.",
                    "They measure alternative killability by previously rejected filters.",
                ],
            },
            "decision": self.decision(),
        }
    ...
    def __init__(
        ...
        *,
        ...
        self.rank_end = (
            EXPECTED_PAIR_WORDS
            if limit is None
            else min(EXPECTED_PAIR_WORDS, rank_start + limit)
        )
        self.target_hi = self.rank_end
        self.limit = None if limit is None else self.rank_end - rank_start
        self.max_lean_leaves = max_lean_leaves
...
def strict_integer_row_key(row: tuple[Fraction, ...]) -> str:
    """Primitive integer row key preserving inequality orientation."""
    ...
    for value in row:
        lcm = lcm * value.denominator // math.gcd(lcm, value.denominator)
    ints = [value.numerator * (lcm // value.denominator) for value in row]
    content = 0
    for value in ints:
        content = math.gcd(content, abs(value))
    if content == 0:
...
def row_support_key(row: tuple[Fraction, ...]) -> str:
    ...
    return "".join(sign(value) for value in row)


def constraint_row_multiset_key(constraints: list[tuple[Fraction, Fraction, Fraction]]) -> str:
    return "|".join(sorted(strict_integer_row_key(row) for row in constraints))


def constraint_row_support_key(constraints: list[tuple[Fraction, Fraction, Fraction]]) -> str:
    return "|".join(sorted(row_support_key(row) for row in constraints))


def canonical_survivor_shape_map_key(
    survivor_bits: int,
    shape_by_mask: dict[int, str],
) -> tuple[str, int, int]:
    """Canonicalize a survivor->shape map under signed variable transforms.

    This is diagnostic only: it measures whether sign-bit renaming/complement
    could share obligations if later backed by a Lean transport theorem.
    """

    if not shape_by_mask:
        return "empty", 0, 0
    canonical_bits, transform_indices = canonical_survivor_bits_and_transforms(
        survivor_bits
    )
    ...
    for index in transform_indices:
        ...
        pieces = sorted(
            f"{transform.new_mask_from_old[mask]}:{digest}"
            for mask, digest in shape_by_mask.items()
        )
        key = f"bits={canonical_bits}|{'|'.join(pieces)}"
        if best_key is None or key < best_key:
            best_key = key
    if best_key is None:
        raise RuntimeError("no signed-variable transforms")
...
def sorted_source_farkas_terms(
    source_terms: list[dict[str, Any]],
) -> list[dict[str, Any]]:
    return sorted(
        source_terms,
        key=lambda term: canonical_json_key(term["source"]),
    )


def source_farkas_multiplier_vector(source_terms: list[dict[str, Any]]) -> tuple[Fraction, ...]:
    return tuple(
        Fraction(term["multiplier"])
        for term in sorted_source_farkas_terms(source_terms)
    )
...
def multiplier_vector_key(vector: tuple[Fraction, ...]) -> str:
...
def scalar_normalized_multiplier_key(source_terms: list[dict[str, Any]]) -> str:
    return multiplier_vector_key(scalar_normalized_multiplier_vector(source_terms))


@dataclass
class SourceSupportClassStats:
    support_digest: str
    source_terms: list[dict[str, Any]]
    cases: int = 0
    exact_multiplier_patterns: Counter[str] = field(default_factory=Counter)
    ...
    survivor_masks: Counter[int] = field(default_factory=Counter)
    ...
    def add_case(
        ...
        *,
        ...
        exact_key = multiplier_vector_key(source_farkas_multiplier_vector(source_terms))
        scalar_key = scalar_normalized_multiplier_key(source_terms)
        self.cases += 1
        self.exact_multiplier_patterns[exact_key] += 1
        self.scalar_multiplier_patterns[scalar_key] += 1
        self.normalized_shapes[raw_shape_digest] += 1
        self.survivor_masks[mask] += 1
        self.words[word_key(word)] += 1
        if len(self.samples) < sample_limit:
            self.samples.append({
                ...
                "word": word_key(word),
                "seq": seq_key(seq),
                "seq_digest": stable_digest(seq_key(seq)),
                "shape_digest": raw_shape_digest,
                "exact_multiplier_key": exact_key,
                "exact_multiplier_digest": stable_digest(exact_key),
                "scalar_multiplier_key": scalar_key,
                "scalar_multiplier_digest": stable_digest(scalar_key),
            })
    ...
    def payload(self) -> dict[str, Any]:
        return {
            ...
            "exact_multiplier_patterns": self.counter_payload(
                self.exact_multiplier_patterns
            ),
            "scalar_normalized_multiplier_patterns": self.counter_payload(
                self.scalar_multiplier_patterns
            ),
            "normalized_farkas_shapes": self.counter_payload(self.normalized_shapes),
            "survivor_masks": self.counter_payload(self.survivor_masks),
            "words": self.counter_payload(self.words, limit=4),
            "samples": self.samples,
        }
...
class TranslationFarkasShapeMapProfiler(TranslationSurvivorProfiler):
    ...
    It measures increasingly semantic keys for the GoodDirection survivor
    obligations that remain after the generic bad-direction theorem.  It emits
    no Lean evidence and intentionally labels unformalized transports as
    diagnostic-only.
    """

    LAYER_METADATA: tuple[dict[str, Any], ...] = (
        {
            "name": "raw_normalized_farkas_shape",
            "proof_ready": True,
            "requirement": "direct exact normalized translation constraint system",
        },
        {
            "name": "d4_normalized_farkas_shape",
            "proof_ready": False,
            "requirement": "requires Lean D4 translation/Farkas transport theorem",
        },
        {
            "name": "reversal_normalized_farkas_shape",
            "proof_ready": False,
            "requirement": "requires Lean reversal transport theorem",
        },
        {
            "name": "d4_reversal_normalized_farkas_shape",
            "proof_ready": False,
            "requirement": "requires Lean D4 plus reversal transport theorem",
        },
        {
            "name": "raw_row_multiset",
            "proof_ready": True,
            "requirement": "row permutation and positive row scaling only",
        },
        {
            "name": "raw_row_support_skeleton",
            "proof_ready": False,
            "requirement": "requires theorem that support skeleton determines reusable Farkas proof",
        },
        {
            "name": "denominator_signature",
            "proof_ready": False,
            "requirement": "requires theorem from denominator signature to survivor Farkas system",
        },
        {
            "name": "survivor_bitset",
            "proof_ready": False,
            "requirement": "diagnostic only; bitset alone does not prove Farkas unsat",
        },
        {
            "name": "raw_survivor_shape_map",
            "proof_ready": True,
            "requirement": "direct exact survivor mask to raw shape map",
        },
        {
            "name": "d4_survivor_shape_map",
            "proof_ready": False,
            "requirement": "requires Lean D4 translation/Farkas transport theorem",
        },
        {
            "name": "signed_variable_survivor_shape_map",
            "proof_ready": False,
            "requirement": "requires sign-bit renaming/complement transport theorem",
        },
    )
    ...
    def __init__(
        ...
        *,
        ...
        self.layers = {
            meta["name"]: DistinctTracker(max_distinct_tracked, sample_limit)
            for meta in self.LAYER_METADATA
        }
        self.layer_reuse = {
            meta["name"]: Counter()
            for meta in self.LAYER_METADATA
        }
        self.signed_variable_tie_histogram: Counter[int] = Counter()
        self.survivor_count_by_identity_word: Counter[int] = Counter()
        self.row_count_histogram: Counter[int] = Counter()

    def record_layer(self, name: str, key: str, *, case_count: int = 1) -> str:
        digest = stable_digest(key)
        self.layers[name].add(digest)
        self.layer_reuse[name][digest] += case_count
        return digest

    def layer_payload(self, name: str, meta: dict[str, Any]) -> dict[str, Any]:
        reuse_values = list(self.layer_reuse[name].values())
        tracker = self.layers[name]
        return {
            "proof_ready": meta["proof_ready"],
            "requirement": meta["requirement"],
            "unique": tracker.payload(),
            "reuse": {
                "shape_count": len(reuse_values),
                "shared_shape_count": sum(1 for count in reuse_values if count > 1),
                "max_reuse": max(reuse_values, default=0),
                "top_reuse": [
                    {"digest": digest, "count": count}
                    for digest, count in self.layer_reuse[name].most_common(8)
                ],
            },
        }

    @lru_cache(maxsize=500_000)
    def shape_digest_for_choice(
        self,
        word: tuple[str, ...],
        mask: int,
    ...
    def canonical_combined_choice(
        ...
        *,
        ...
        cword, cmask = canonical_translation_combined(
            list(word),
            mask,
            with_symmetry=with_symmetry,
            with_reversal=with_reversal,
        )
        return tuple(cword), cmask

    def classify_leaf(self, rank: int, word: tuple[str, ...], pref: list) -> None:
        self.pair_words_scanned += 1
        if self.progress_interval and self.pair_words_scanned % self.progress_interval == 0:
            print(
                f"translation shape-map scanned {self.pair_words_scanned:,} pair words",
                file=sys.stderr,
            )
    ...
    def best_layer(self, *, proof_ready: bool | None) -> dict[str, Any] | None:
        candidates: list[dict[str, Any]] = []
        for meta in self.LAYER_METADATA:
            if proof_ready is not None and meta["proof_ready"] != proof_ready:
                continue
            tracker = self.layers[meta["name"]]
            candidates.append({
                "name": meta["name"],
                "proof_ready": meta["proof_ready"],
                "count": tracker.exact_count,
                "lower_bound": tracker.lower_bound,
                "requirement": meta["requirement"],
            })
        if not candidates:
            return None
        return min(candidates, key=lambda item: (item["lower_bound"], item["name"]))

    def decision(self) -> dict[str, Any]:
        proof_ready_best = self.best_layer(proof_ready=True)
        diagnostic_best = self.best_layer(proof_ready=False)
        all_best = self.best_layer(proof_ready=None)
        proof_ready_count = (
            proof_ready_best["count"] if proof_ready_best is not None else None
        )
        proof_ready_lower = (
            proof_ready_best["lower_bound"] if proof_ready_best is not None else 0
        )
        diagnostic_lower = (
            diagnostic_best["lower_bound"] if diagnostic_best is not None else 0
        )
        notes: list[str] = []
        if proof_ready_best is None:
            status = "rejected"
            notes.append("no proof-ready compression layer was recorded")
        elif proof_ready_count is not None and proof_ready_count <= self.max_lean_leaves:
            status = "accepted"
            notes.append(
                f"best proof-ready layer {proof_ready_best['name']} is within the "
                f"{self.max_lean_leaves} leaf gate"
            )
        elif diagnostic_best is not None and diagnostic_lower <= self.max_lean_leaves:
            status = "blocked_on_transport_theorem"
            notes.append(
                f"best diagnostic layer {diagnostic_best['name']} is within the "
                f"{self.max_lean_leaves} leaf gate but requires: "
                f"{diagnostic_best['requirement']}"
            )
        else:
            status = "rejected"
            notes.append(
                "no measured layer reaches the "
                f"{self.max_lean_leaves} leaf gate"
            )
        if proof_ready_best is not None and proof_ready_best["lower_bound"] > self.warn_lean_leaves:
            notes.append(
                f"best proof-ready layer {proof_ready_best['name']} exceeds warning "
                f"threshold {self.warn_lean_leaves}"
            )
        return {
            "status": status,
            "accepted_for_phase_6o": status == "accepted",
            "best_proof_ready_layer": proof_ready_best,
            "best_diagnostic_layer": diagnostic_best,
            "best_overall_layer": all_best,
            "planned_lean_heavy_leaves_exact": proof_ready_count,
            "planned_lean_heavy_leaves_lower_bound": proof_ready_lower,
            "max_lean_leaves": self.max_lean_leaves,
            "warn_lean_leaves": self.warn_lean_leaves,
            "notes": notes,
        }
    ...
    def payload(self, *, elapsed_seconds: float) -> dict[str, Any]:
        layer_payloads = {
            meta["name"]: self.layer_payload(meta["name"], meta)
            for meta in self.LAYER_METADATA
        }
        return {
            "schema_version": 1,
            "mode": "translation-farkas-shape-map-profile",
            "trusted_as_proof": False,
            "complete": self.limit is None,
            "profile_limit": self.limit,
            "elapsed_seconds": elapsed_seconds,
            "options": {
                ...
                "emits_bad_direction_evidence": False,
                "emits_lean_evidence": False,
                "d4_layers": "diagnostic until Lean transport is wired into this API",
                "reversal_layers": "diagnostic only",
                "signed_variable_layers": "diagnostic only",
            },
            "expected_counts": {
                "pair_words": EXPECTED_PAIR_WORDS,
                "identity_linear_words": EXPECTED_IDENTITY_WORDS,
            ...
            "diagnostics": {
                ...
                "survivor_count_by_identity_word": dict(
                    sorted(self.survivor_count_by_identity_word.items())
                ),
                "denominator_degree_histogram": dict(
                    sorted(self.denominator_degree_histogram.items())
                ),
                "row_count_histogram": dict(sorted(self.row_count_histogram.items())),
                "signed_variable_transform_tie_histogram": dict(
                    sorted(self.signed_variable_tie_histogram.items())
                ),
                "samples": self.samples,
            },
            "decision": self.decision(),
...
class TranslationFarkasPhase6WProfiler(TranslationFarkasPhase6VProfiler):
    """Phase 6W audit for source-support multiplier variation."""
    ...
    def __init__(
        ...
        *,
        ...
        self.source_support_classes: dict[str, SourceSupportClassStats] = {}
    ...
    def record_source_farkas_layers(
        ...
        *,
        ...
        stats.add_case(
            ...
            word=word,
            seq=cert["seq"],
            raw_shape_digest=raw_shape_digest,
            source_terms=source_terms,
    ...
    def source_support_summary(self) -> dict[str, Any]:
        ...
        exact_unique_histogram = Counter(
            len(stats.exact_multiplier_patterns)
            for stats in classes
        )
        shape_unique_histogram = Counter(
            len(stats.normalized_shapes)
            for stats in classes
        )
        return {
            ...
            "exact_multiplier_unique_histogram": dict(
                sorted(exact_unique_histogram.items())
            ),
            "normalized_shape_unique_histogram": dict(
                sorted(shape_unique_histogram.items())
            ),
            "classes_with_single_scalar_pattern": sum(
                1 for stats in classes if len(stats.scalar_multiplier_patterns) == 1
            ),
            "classes_with_single_exact_multiplier_pattern": sum(
                1 for stats in classes if len(stats.exact_multiplier_patterns) == 1
            ),
        }
    ...
    def decision(self) -> dict[str, Any]:
        ...
        top_classes = [
            self.source_support_classes[item["support_digest"]]
            for item in self.top_source_support_payloads()
        ]
        notes: list[str] = []
        if self.source_farkas_failures:
            status = "rejected"
        ...
        elif top_classes and all(
            len(stats.scalar_multiplier_patterns) == 1 for stats in top_classes
        ):
            status = "theorem_candidate"
            notes.append(
                "top source-support classes have a single scalar-normalized "
                "multiplier pattern"
            )
        elif summary["support_class_count"] <= self.max_lean_leaves:
            status = "blocked_parametric"
            notes.append(
                "source-support count is within the leaf gate, but multiplier "
                "variation still needs a parametric theorem"
            )
        else:
            status = "rejected"
            notes.append("source-support sharing does not yield a usable family")
        if summary["classes_with_single_scalar_pattern"] < summary["support_class_count"]:
            notes.append(
                "not all source-support classes have one scalar-normalized "
                "multiplier pattern"
            )
        return {
            "status": status,
            "accepted_for_phase_6w": status == "theorem_candidate",
            "max_lean_leaves": self.max_lean_leaves,
            "warn_lean_leaves": self.warn_lean_leaves,
    ...
    def payload(self, *, elapsed_seconds: float) -> dict[str, Any]:
        ...
        payload["mode"] = "translation-farkas-phase6w-profile"
        payload["options"]["phase"] = "6W"
        payload["options"]["top_source_support_classes"] = self.top_support_classes
        payload["options"]["source_support_sample_cases"] = self.support_sample_cases
        payload["source_support_summary"] = self.source_support_summary()
        payload["top_source_support_classes"] = self.top_source_support_payloads()
        payload["decision"] = self.decision()
        return payload


def phase6w_count_payload(counter: Counter[Any], *, limit: int = 8) -> dict[str, Any]:
    return {
        "unique": len(counter),
        "max_reuse": max(counter.values(), default=0),
        "top": [
...
def phase6w_distinct_payload(
    ...
    *,
    ...
    return {
        "exact": True,
        "count": len(counter),
        "lower_bound": len(counter),
        "cap": None,
        "samples": [
            digest
            for digest, _count in counter.most_common(sample_limit)
        ],
    }


def phase6w_layer_payload_from_counter(
    *,
    meta: dict[str, Any],
    counter: Counter[str],
    sample_limit: int,
) -> dict[str, Any]:
    reuse_values = list(counter.values())
    return {
        "proof_ready": meta["proof_ready"],
        "requirement": meta["requirement"],
        "unique": phase6w_distinct_payload(counter, sample_limit=sample_limit),
        "reuse": {
            "shape_count": len(reuse_values),
            "shared_shape_count": sum(1 for count in reuse_values if count > 1),
            "max_reuse": max(reuse_values, default=0),
            "top_reuse": [
                {"digest": digest, "count": count}
                for digest, count in counter.most_common(8)
            ],
        },
    }


def phase6w_support_class_payload(
    support_digest: str,
    state: dict[str, Any],
) -> dict[str, Any]:
    exact_multiplier_patterns = Counter(state["exact_multiplier_patterns"])
    scalar_multiplier_patterns = Counter(state["scalar_multiplier_patterns"])
    normalized_shapes = Counter(state["normalized_shapes"])
    survivor_masks = Counter({
        int(mask): count
        for mask, count in state["survivor_masks"].items()
    })
    words = Counter(state["words"])
    return {
        "support_digest": support_digest,
        "cases": state["cases"],
        "source_terms": state["source_terms"],
        "source_term_count": len(state["source_terms"]),
        "exact_multiplier_patterns": phase6w_count_payload(
            exact_multiplier_patterns
        ),
        "scalar_normalized_multiplier_patterns": phase6w_count_payload(
            scalar_multiplier_patterns
        ),
        "normalized_farkas_shapes": phase6w_count_payload(normalized_shapes),
        "survivor_masks": phase6w_count_payload(survivor_masks),
        "words": phase6w_count_payload(words, limit=4),
        "samples": state["samples"],
    }


def phase6w_source_support_summary_from_state(
    ...
    exact_unique_histogram = Counter(
        len(state["exact_multiplier_patterns"])
        for state in classes.values()
    )
    shape_unique_histogram = Counter(
        len(state["normalized_shapes"])
        for state in classes.values()
    )
    return {
        "support_class_count": len(classes),
        ...
        "classes_with_single_exact_multiplier_pattern": sum(
            1
            for state in classes.values()
            if len(state["exact_multiplier_patterns"]) == 1
        ),
    }


def phase6w_decision_from_state(
    *,
    source_support_summary: dict[str, Any],
    top_source_support_classes: list[dict[str, Any]],
    ...
    if source_farkas_failure_count:
    ...
    elif top_source_support_classes and all(
        item["scalar_normalized_multiplier_patterns"]["unique"] == 1
        for item in top_source_support_classes
    ):
        status = "theorem_candidate"
        notes.append(
            "top source-support classes have a single scalar-normalized "
        ...
        notes.append("source-support sharing does not yield a usable family")
    if (
        source_support_summary["classes_with_single_scalar_pattern"]
        < source_support_summary["support_class_count"]
    ):
        notes.append(
            "not all source-support classes have one scalar-normalized "
            "multiplier pattern"
...
def phase6w_profiler_state(
    profiler: TranslationFarkasPhase6WProfiler,
    *,
    elapsed_seconds: float,
) -> dict[str, Any]:
    return {
        ...
        "layers": {
            name: dict(counter)
            for name, counter in profiler.layer_reuse.items()
        },
        "diagnostics": {
            "survivor_count_by_identity_word": dict(
                profiler.survivor_count_by_identity_word
            ),
            "denominator_degree_histogram": dict(
                profiler.denominator_degree_histogram
            ),
            "row_count_histogram": dict(profiler.row_count_histogram),
            "signed_variable_transform_tie_histogram": dict(
                profiler.signed_variable_tie_histogram
            ),
            "samples": profiler.samples,
        },
        "source_farkas": {
            "failures": profiler.source_farkas_failures,
            "failure_count": len(profiler.source_farkas_failures),
            "term_count_histogram": dict(profiler.source_term_count_histogram),
            "support_to_exact_term_shapes": dict(
                profiler.source_support_to_term_shapes
            ),
            "samples": profiler.source_samples,
        },
...
def run_phase6w_window(config: dict[str, Any]) -> dict[str, Any]:
    import time
    ...
    profiler.traverse()
    return phase6w_profiler_state(
        profiler,
        elapsed_seconds=time.monotonic() - start,
    )


def merge_phase6w_states(
    ...
    *,
    ...
    if not states:
        raise RuntimeError("no Phase 6W worker states to merge")
    ...
    for state in states:
        ...
        if window["start"] != expected_start:
            raise RuntimeError(
                "Phase 6W shard gap or overlap before "
                f"{window['start']} (expected {expected_start})"
            )
        expected_start = window["end"]
    if expected_start != target_end:
        raise RuntimeError(
            f"Phase 6W shards ended at {expected_start}; expected {target_end}"
        )

    actual_counts = Counter()
    layer_counters = {
        meta["name"]: Counter()
        for meta in TranslationFarkasPhase6WProfiler.LAYER_METADATA
    }
    survivor_count_by_identity_word: Counter[int] = Counter()
    denominator_degree_histogram: Counter[int] = Counter()
    row_count_histogram: Counter[int] = Counter()
    signed_variable_tie_histogram: Counter[int] = Counter()
    diagnostic_samples: list[dict[str, Any]] = []
    source_failures: list[dict[str, Any]] = []
    source_failure_count = 0
    source_term_count_histogram: Counter[int] = Counter()
    source_support_to_term_shapes: Counter[str] = Counter()
    source_samples: list[dict[str, Any]] = []
    support_classes: dict[str, dict[str, Any]] = {}

    for state in states:
        actual_counts.update(state["actual_counts"])
        for name, values in state["layers"].items():
            layer_counters[name].update(values)
        diagnostics = state["diagnostics"]
        survivor_count_by_identity_word.update({
            int(key): value
            for key, value in diagnostics["survivor_count_by_identity_word"].items()
        })
        denominator_degree_histogram.update({
            int(key): value
            for key, value in diagnostics["denominator_degree_histogram"].items()
        })
        row_count_histogram.update({
            int(key): value
            for key, value in diagnostics["row_count_histogram"].items()
        })
        signed_variable_tie_histogram.update({
            int(key): value
            for key, value in diagnostics[
                "signed_variable_transform_tie_histogram"
            ].items()
        })
        ...
        source = state["source_farkas"]
        source_failure_count += source["failure_count"]
        for failure in source["failures"]:
            if len(source_failures) < args.sample_limit:
                source_failures.append(failure)
        source_term_count_histogram.update({
            int(key): value
            for key, value in source["term_count_histogram"].items()
        })
        source_support_to_term_shapes.update(source["support_to_exact_term_shapes"])
        for sample in source["samples"]:
            if len(source_samples) < args.sample_limit:
                source_samples.append(sample)

        for digest, incoming in state["source_support_classes"].items():
            target = support_classes.setdefault(
                digest,
                {
                    "support_digest": digest,
                    "source_terms": incoming["source_terms"],
                    "cases": 0,
                    "exact_multiplier_patterns": Counter(),
            ...
            target["exact_multiplier_patterns"].update(
                incoming["exact_multiplier_patterns"]
            )
            ...
            target["survivor_masks"].update({
                int(mask): count
                for mask, count in incoming["survivor_masks"].items()
            })
            target["words"].update(incoming["words"])
            for sample in incoming["samples"]:
                if len(target["samples"]) < args.source_support_sample_cases:
                    target["samples"].append(sample)

    layers = {
        meta["name"]: phase6w_layer_payload_from_counter(
            meta=meta,
            counter=layer_counters[meta["name"]],
            sample_limit=args.sample_limit,
        )
        for meta in TranslationFarkasPhase6WProfiler.LAYER_METADATA
    }
    support_payloads = [
        phase6w_support_class_payload(digest, state)
        for digest, state in sorted(
            support_classes.items(),
            key=lambda item: (-item[1]["cases"], item[0]),
        )
    ]
    source_support_summary = phase6w_source_support_summary_from_state(
        support_classes
    )
    top_source_support_classes = support_payloads[: args.source_support_top_classes]
    decision = phase6w_decision_from_state(
        source_support_summary=source_support_summary,
        top_source_support_classes=top_source_support_classes,
        source_farkas_failure_count=source_failure_count,
        max_lean_leaves=args.max_lean_leaves,
        warn_lean_leaves=args.warn_lean_leaves,
    )
    return {
        "schema_version": 3,
        "mode": "translation-farkas-phase6w-profile",
        "trusted_as_proof": False,
        ...
        "diagnostics": {
            "survivor_density": (
                actual_counts["good_direction_survivor_masks"]
                / actual_counts["translation_sign_assignments"]
                if actual_counts["translation_sign_assignments"]
                else None
            ),
            "survivor_count_by_identity_word": dict(
                sorted(survivor_count_by_identity_word.items())
            ),
            "denominator_degree_histogram": dict(
                sorted(denominator_degree_histogram.items())
            ),
            "row_count_histogram": dict(sorted(row_count_histogram.items())),
            "signed_variable_transform_tie_histogram": dict(
                sorted(signed_variable_tie_histogram.items())
            ),
        ...
        "source_farkas": {
            ...
            "term_count_histogram": dict(sorted(source_term_count_histogram.items())),
            "support_to_exact_term_shape_count": len(source_support_to_term_shapes),
            "top_support_term_reuse": [
                {"digest": digest, "count": count}
                for digest, count in source_support_to_term_shapes.most_common(8)
            ],
...
def run_phase6w_parallel(
    ...
    *,
    ...
    with ProcessPoolExecutor(max_workers=args.parallel_jobs) as executor:
        futures = {
            executor.submit(run_phase6w_window, config): config
...
def validate_options(args: argparse.Namespace) -> None:
    ...
    if args.source_support_top_classes <= 0:
        raise SystemExit("--source-support-top-classes must be positive")
    if args.source_support_sample_cases < 0:
        raise SystemExit("--source-support-sample-cases must be nonnegative")
...
def write_phase6w_markdown(payload: dict[str, Any], output: Path) -> None:
    counts = payload["actual_counts"]
    decision = payload["decision"]
    ...
    lines = [
        ...
        f"- Max cases per source support: {summary['max_cases_per_support']:,}",
        f"- Singleton source-support classes: {summary['singleton_support_classes']:,}",
        "",
        "## Multiplier Variation",
        "",
        f"- Scalar-normalized multiplier histogram: `{summary['scalar_multiplier_unique_histogram']}`",
        f"- Exact multiplier histogram: `{summary['exact_multiplier_unique_histogram']}`",
        f"- Normalized Farkas-shape histogram: `{summary['normalized_shape_unique_histogram']}`",
        f"- Classes with one scalar pattern: {summary['classes_with_single_scalar_pattern']:,}",
        f"- Classes with one exact multiplier pattern: {summary['classes_with_single_exact_multiplier_pattern']:,}",
        "",
    ...
    for item in payload["top_source_support_classes"]:
        lines.append(
            f"| `{item['support_digest'][:12]}...` | {item['cases']:,} | "
            f"{item['scalar_normalized_multiplier_patterns']['unique']:,} | "
            f"{item['exact_multiplier_patterns']['unique']:,} | "
            f"{item['normalized_farkas_shapes']['unique']:,} |"
        )
    lines.extend([
        "",
        "## Decision",
...
def print_summary(payload: dict[str, Any]) -> None:
    ...
    if payload.get("mode") == "combined-residual-portfolio-profile":
        counts = payload["actual_counts"]
        primary = payload["primary_portfolio"]
        decision = payload["decision"]
        print("combined residual portfolio profile")
        print(f"pair words scanned: {counts['pair_words_scanned']:,}")
        print(f"identity words: {counts['identity_words']:,}")
        print(f"nonidentity words: {counts['nonidentity_words']:,}")
        print(
            "translation GoodDirection masks: "
            f"{counts['translation_good_direction_masks']:,}"
        )
        print(
            "translation non-GoodDirection masks: "
            f"{counts['translation_denominator_nonpositive_masks']:,}"
        )
        print(
            "unique primary shapes: "
            f"{primary['unique_primary_shapes']['lower_bound']}"
        )
        planned_leaves = (
            str(decision["planned_lean_heavy_leaves_exact"])
            if decision["planned_lean_heavy_leaves_exact"] is not None
        ...
        print(f"planned heavy Lean leaves: {planned_leaves}")
        largest = decision["largest_residual_family"]
        if largest is not None:
            print(
                "largest residual family: "
                f"{largest['filter']} ({largest['shape_count']} shapes)"
            )
        print(f"accepted for Phase 6N: {decision['accepted_for_phase_6n']}")
        for note in decision["notes"]:
            print(f"- {note}")
        return
    ...
    if payload.get("mode") in {
        ...
        "translation-farkas-phase6w-profile",
    }:
        counts = payload["actual_counts"]
        layers = payload["layers"]
        decision = payload["decision"]
        if payload.get("mode") == "translation-farkas-phase6w-profile":
            print("translation Farkas Phase 6W profile")
        ...
        for name in sorted(layers):
            layer = layers[name]
            print(
                f"{name}: {layer['unique']['lower_bound']} unique "
                f"(proof_ready={layer['proof_ready']})"
            )
        best_proof = decision.get("best_proof_ready_layer")
        best_diag = decision.get("best_diagnostic_layer")
        if best_proof is not None:
            print(
                "best proof-ready layer: "
                f"{best_proof['name']} ({best_proof['lower_bound']} unique)"
            )
        if best_diag is not None:
            print(
                "best diagnostic layer: "
        ...
        if payload.get("mode") == "translation-farkas-phase6w-profile":
            ...
            print(
                ...
                f"{support['support_class_count']:,}"
            )
            print(
                "scalar-normalized multiplier histogram: "
                f"{support['scalar_multiplier_unique_histogram']}"
            )
        print(f"decision: {decision['status']}")
        for note in decision["notes"]:
            print(f"- {note}")
...
def main() -> None:
    ...
    parser.add_argument(
        "--translation-farkas-phase6w",
        action="store_true",
        help=(
            "Phase 6W audit: inspect multiplier variation inside shared "
            "source-support Farkas classes"
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument("--source-support-top-classes", type=int, default=12)
    parser.add_argument("--source-support-sample-cases", type=int, default=16)
    parser.add_argument(
        "--parallel-jobs",
        type=int,
        default=1,
        help="number of worker processes for shard-safe modes",
    )
    parser.add_argument(
        "--parallel-window-size",
        type=int,
        default=10_000,
        help="rank width per worker shard for Phase 6W parallel profiling",
    )
    parser.add_argument(
        "--no-d4-transport",
        action="store_true",
    ...
    if args.terminal_residual_census:
        ...
        if args.parallel_jobs > 1:
            payload = run_phase6w_parallel(args, elapsed_start=start)
        else:
            phase6w_profiler = TranslationFarkasPhase6WProfiler(
                limit=args.limit,
                rank_start=args.rank_start,
                max_lean_leaves=args.max_lean_leaves,
                warn_lean_leaves=args.warn_lean_leaves,
        ...
        shape_map_profiler.traverse()
        rejected = False
        payload = shape_map_profiler.payload(
            elapsed_seconds=time.monotonic() - start,
        )
    elif args.combined_residual_portfolio:
        portfolio_profiler = CombinedResidualPortfolioProfiler(
            limit=args.limit,
            max_lean_leaves=args.max_lean_leaves,
            warn_lean_leaves=args.warn_lean_leaves,
            max_distinct_tracked=args.max_distinct_tracked,
        ...
        portfolio_profiler.traverse()
        rejected = not portfolio_profiler.decision()["accepted_for_phase_6n"]
