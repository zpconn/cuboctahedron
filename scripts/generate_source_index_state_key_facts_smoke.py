#!/usr/bin/env python3
"""Emit the Phase 6Z.6K.8P descriptor-key facts smoke.

The generated Lean module contains no concrete rank/mask member list. It
proves that explicit source-index/source-check/row facts imply descriptor-key
matches and then route through the accepted GoodDirection classifier surface.
"""
...
DEFAULT_OUT = Path(
    ...
    "SourceIndexStateKeyFactsSmoke.lean"
)
DEFAULT_JSON = Path(
    "scripts/generated/phase6z6k8p_source_index_state_key_facts_smoke.json"
)
DEFAULT_MD = DEFAULT_JSON.with_suffix(".md")
DEFAULT_NAMESPACE = (
    "Cuboctahedron.Generated.Translation.TwoSource.SupportFamilies."
    "SourceIndexStateKeyFactsSmoke"
)
...
def key_lines(index: int, family: Any) -> list[str]:
    ...
    return [
        f"/-- Descriptor-key facts smoke family `{family.key}`.",
...
def classifier_lines(selected: list[Any]) -> list[str]:
    lines = ["inductive FactClassifierApplies : Nat -> SignMask -> Prop"]
    for index, _family in enumerate(selected):
        name = family_name(index)
        ctor = ctor_name(index)
        lines.append(
            f"  | {ctor} {{r : Nat}} {{mask : SignMask}} "
            f"(h : SourceIndexStateKeyFacts {name}_key r mask) : "
    ...
    for index, _family in enumerate(selected):
        ...
        lines.extend([
            f"    | {ctor} h =>",
            f"        exact {name}_key.covered_of_facts h",
        ])
    lines.extend([
        "",
    ...
    for index, _family in enumerate(selected):
        ...
        ctor = ctor_name(index)
        lines.extend([
            f"theorem {name}_factsImplyKey",
            "    {r : Nat} {mask : SignMask}",
            f"    (h : SourceIndexStateKeyFacts {name}_key r mask) :",
            f"    {name}_key.Matches r mask :=",
            f"  {name}_key.matches_of_facts h",
            "",
            f"theorem {name}_factsImplyClassifier",
            "    {r : Nat} {mask : SignMask}",
            f"    (h : SourceIndexStateKeyFacts {name}_key r mask) :",
            "    FactClassifierApplies r mask :=",
            f"  FactClassifierApplies.{ctor} h",
            "",
            f"theorem {name}_factsGoodKilled",
            "    {r : Nat} {hlt : r < numPairWords} {mask : SignMask}",
            f"    (h : SourceIndexStateKeyFacts {name}_key r mask) :",
            "    TranslationGoodCaseKilled (Fin.mk r hlt) mask :=",
            f"  factClassifierKillsOn r hlt mask ({name}_factsImplyClassifier h)",
            "",
        ])
    return lines
...
def markdown(payload: dict[str, Any]) -> str:
    lines = [
        f"# Phase {payload['phase']} Source-Index/State Key-Facts Smoke",
        "",
        "This generated smoke is not global coverage. It packages selected",
        "source-index/state keys and proves that formal source-index/source-check",
        "and row facts imply key matches without concrete rank/mask replay.",
        "",
...
def build_payload(
    *,
    ...
    return {
        ...
        "namespace": namespace,
        "decision": {
            "status": "descriptor-key-facts-smoke-emitted",
            "notes": [
                "no concrete rank/mask examples are emitted",
                "the theorem surface assumes source-index/source-check/row facts",
...
def main() -> None:
    ...
    parser.add_argument("--phase", default="6Z.6K.8P")
