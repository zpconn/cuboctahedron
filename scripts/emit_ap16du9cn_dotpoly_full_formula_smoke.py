#!/usr/bin/env python3
"""Emit a cube00 full dot-polynomial smoke using flat coefficient formulas.

DU.9CM showed that rewriting a single coefficient with
`weightedQuadraticFromDotData_c` is much cheaper than unfolding the nested
weighted sum.  This DU.9CN smoke uses the full flat coefficient record theorem
`weightedQuadraticFromDotData_eq_coeffs` and then proves all 22 fields against
the generated scaled polynomial.
"""
...
BASE_NAME = "WeightedDenomCubeRank6000745TraceCertDotPolyFullFormula"
DEFAULT_REPORT = Path(
...
def main() -> None:
    ...
    payload = {
        "phase": "Phase 6Z.6K.8AP.16DU.9CN",
    ...
    args.report.with_suffix(".md").write_text(
        "\n".join([
            "# Phase 6Z.6K.8AP.16DU.9CN Dot-Polynomial Full Formula Smoke",
            "",
            f"- rank: `{args.rank}`",
    ...
    print(f"wrote {path} with full formula dot-polynomial proof")
