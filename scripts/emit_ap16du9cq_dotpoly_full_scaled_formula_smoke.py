#!/usr/bin/env python3
"""Emit a cube00 full dot-polynomial smoke using scaled coefficient goals.

DU.9CN proved 22 rational field equalities against
`ScaledWalshQuadratic.toQuadratic`, unfolding `coeffRat` in every generated
goal.  This DU.9CQ smoke uses the generic
`ScaledWalshQuadratic.toQuadratic_eq_of_scaled_coeffs` theorem, so generated
goals are integer-scaled coefficient equalities instead.
"""
...
BASE_NAME = "WeightedDenomCubeRank6000745TraceCertDotPolyFullScaledFormula"
DEFAULT_REPORT = Path(
...
def dot_poly_eq_formula(name: str, data_module: str, support: list[int]) -> list[str]:
    defs = [
...
def build_module(*, cube: dict, data_module: str, namespace: str) -> str:
    ...
    lines = [
        ...
        "",
        "/-! Generated DU.9CQ cube00 full scaled formula dot-polynomial smoke module. -/",
        "",
        ...
        *emit_scaled_poly_named(name, cube),
        "",
        *dot_poly_eq_formula(name, data_module, list(cube["support"])),
        "",
        "theorem dotPolyFullScaledFormulaSmoke_builds : True := by",
        "  trivial",
        "",
        f"end {namespace}",
        "",
...
def main() -> None:
    ...
    payload = {
        "phase": "Phase 6Z.6K.8AP.16DU.9CQ",
    ...
    args.report.with_suffix(".md").write_text(
        "\n".join([
            "# Phase 6Z.6K.8AP.16DU.9CQ Dot-Polynomial Full Scaled Formula Smoke",
            "",
            f"- rank: `{args.rank}`",
    ...
    print(f"wrote {path} with full scaled formula dot-polynomial proof")
