...
def emit_module(
    *,
    ...
    lines.extend([
        ...
        f"    {{mask : SignMask}} (hlt : {rank_name} < numPairWords)",
        f"    (hmember : {cube}.Member mask) :",
        f"    ¬ GoodDirectionAtRank (⟨{rank_name}, hlt⟩ : Fin numPairWords) mask := by",
        "  intro hgood",
        "  have hpos :",
        "      0 <",
        "        weightedDenomAtRank",
        f"          (⟨{rank_name}, hlt⟩ : Fin numPairWords) mask {weights_def} :=",
        f"    weightedDenomAtRank_pos_of_goodDirection hlt {weights_def}_nonnegative",
        f"      {weights_def}_positive hgood",
        "  exact (not_lt_of_ge (weightedDenom_nonpos hlt hmember)) hpos",
        "",
        "theorem translationGoodCaseKilled_of_member",
        f"    {{mask : SignMask}} (hlt : {rank_name} < numPairWords)",
        f"    (hmember : {cube}.Member mask) :",
        f"    TranslationGoodCaseKilled (⟨{rank_name}, hlt⟩ : Fin numPairWords) mask := by",
        "  intro hgood",
