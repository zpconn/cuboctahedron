#!/usr/bin/env python3
"""Emit a one-dot coefficient-level smoke module.

DU.9BZ generated the full rank-6000745 coefficient-dot Data/chain modules, but
the full Data target is too broad to try immediately after an OOM warning.  This
DU.9CA emitter isolates exactly one generated normal and one generated
coefficient-level dot proof so the Lean build guard can measure the proof shape
before we import the larger DU.9BZ module.
"""
...
from emit_ap16du9bz_trace_cert_coeff_dot_chain_smoke import (  # noqa: E402
    emit_dot_coeff_eq,
)
...
BASE_NAME = "WeightedDenomCubeRank6000745TraceCertCoeffDotMicro"
DEFAULT_REPORT = Path(
    "scripts/generated/phase6z6k8ap16du9ca_trace_cert_coeff_dot_micro_smoke.json"
)
...
def build_module(*, rank: int, trace_module: str, namespace: str) -> str:
    ...
    lines = [
        ...
        "",
        "/-!",
        "Generated DU.9CA one-dot coefficient-level smoke module.",
        "-/",
        "",
        ...
        *emit_walsh_vec(normal_name, normal_coeffs, visibility=""),
        ...
        "",
        *emit_quadratic_public(f"generatedDot{index:02d}", dot_coeffs),
        "",
        *emit_dot_coeff_eq(index, trace_module),
        "",
        "theorem coeffDotMicroSmoke_builds : True := by",
        "  trivial",
        "",
        f"end {namespace}",
        "",
...
def main() -> None:
    ...
    payload = {
        "phase": "Phase 6Z.6K.8AP.16DU.9CA",
    ...
    args.report.with_suffix(".md").write_text(
        "\n".join([
            "# Phase 6Z.6K.8AP.16DU.9CA Coeff-Dot Micro Smoke",
            "",
            f"- rank: `{args.rank}`",
            "- dot index: `0`",
            f"- module: `{namespace}`",
    ...
    print(f"wrote {path} with one coefficient-dot proof")
