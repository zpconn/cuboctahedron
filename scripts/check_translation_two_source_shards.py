#!/usr/bin/env python3
"""Memory-bounded checker for generated two-source translation shards.

The two-source evidence path intentionally keeps generated pilot ranges out of
the normal package build until their shard behavior is measured.  This helper
checks shard files directly with `lake env lean -j1`, records `/usr/bin/time -v`
peak RSS when requested, and can compile `.olean` outputs into an external cache
directory so generated source cleanup does not wipe the measured artifacts.
"""
...
DEFAULT_COVERAGE_DIR = (
    REPO_ROOT / "Cuboctahedron" / "Generated" / "Translation" / "TwoSource" / "Coverage10k"
)
DEFAULT_CACHE_DIR = REPO_ROOT / "evidence" / ".translation_two_source_olean_cache"
...
def run_task(
    ...
    *,
    ...
    if compile_external:
        ...
        env = os.environ.copy()
        env["LEAN_PATH"] = f"{cache_dir}:{REPO_ROOT}:{lean_path or lake_lean_path()}"
...
def shard_tasks(coverage_dir: Path) -> list[CheckTask]:
    shards = sorted(coverage_dir.glob("Shard*.lean"))
    return [CheckTask(path=path, label=path.stem) for path in shards]
...
def main() -> int:
    ...
    all_tasks = shard_tasks(coverage_dir)
    tasks = selected_tasks(all_tasks, args.select)
    if not tasks:
        raise SystemExit(f"no shard tasks selected from {coverage_dir}")

    jobs = effective_jobs(args)
    lean_path = lake_lean_path() if args.compile_external else None
    print(
        f"checking {len(tasks)} of {len(all_tasks)} shard(s) with jobs={jobs} "
        f"from {coverage_dir.relative_to(REPO_ROOT)}",
        flush=True,
    )
    ...
    payload = {
        "schema_version": 1,
        "coverage_dir": str(coverage_dir.relative_to(REPO_ROOT)),
        "select": args.select,
