...
def case_shape_lines(cc: ClassifiedCase) -> list[str]:
    ...
    if cc.template_id == "eq_eq_pos_var_first":
        ...
        shape_tail = [
            ...
            f"    rw [SupportFamilies.SupportPair.multipliersAt, TwoSourceFarkasSupport.multipliers]",
            f"    change",
            f"      (TwoSourceFarkasSupport.multipliersForLines",
            f"          (FirstLineAt {name}_support {name}_rank.val hlt {name}_mask)",
            f"          (SecondLineAt {name}_support {name}_rank.val hlt {name}_mask)).1 *",
            f"          (FirstLineAt {name}_support {name}_rank.val hlt {name}_mask).c +",
            f"        (TwoSourceFarkasSupport.multipliersForLines",
            f"          (FirstLineAt {name}_support {name}_rank.val hlt {name}_mask)",
            f"          (SecondLineAt {name}_support {name}_rank.val hlt {name}_mask)).2 *",
            f"          (SecondLineAt {name}_support {name}_rank.val hlt {name}_mask).c <= 0",
            f"    rw [FirstLineAt, SecondLineAt, hfirst, hsecond]",
            f"    norm_num [{name}_firstLine, {name}_secondLine, {name}_support,",
            f"      {name}_seq, {name}_b,",
            f"      TwoSourceFarkasSupport.multipliersForLines,",
