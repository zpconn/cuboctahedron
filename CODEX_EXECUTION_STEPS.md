...
## Step 14E.7B6: Kernel-Reducible Packed-Byte Fallback

Goal: replace the generated fallback chunk wrappers that ask Lean to decode
Base64 strings with wrappers that expose predecoded raw packed bytes as Lean
literal lists and then run the compact binary parser/checker on those bytes.

This step exists because the packed blob backend is size-safe, but
`include_str` plus Base64 decoding is too expensive for trusted kernel
reduction: even a one-certificate packed chunk can fail to reduce with
`by decide`. Do not switch to full structured `CompactNonIdResidualCert` or
`CompactTranslationFarkasCert` literals unless the packed-byte-list backend also
fails, because structured literals expand the generated source by many GiB.
...
- Add trusted Boolean byte-check entry points:
  - `checkPackedResidualBytes : List Nat -> Bool`;
  - `checkPackedResidualBytes_sound`;
  - `checkPackedTranslationFarkasBytes : List Nat -> Bool`;
  - `checkPackedTranslationFarkasBytes_sound`.
- Generated chunks must define `chunkBytes : List Nat` using raw byte literals
  decoded externally from the `.b64` staging blob, and must prove
  `checkPacked...Bytes chunkBytes = true` with kernel reduction.
- The `.b64` files may remain as compact audit/staging artifacts, but the Lean
  proof path must not call `include_str`, `decodeBase64`, or
  `checkPacked...Certs` for full fallback chunks.
- Update the exhaustive manifest strategies to make the checked backend
  explicit, for example:
  - `predecoded_packed_nonid_residual_bytes`;
  - `predecoded_packed_translation_farkas_bytes`.
- The independent checker must still decode the `.b64` staging artifacts and
  verify that the generated Lean chunk record matches the same counts,
  ordering, hashes, and no-forbidden-token constraints.
- The smoke guard must select the largest generated chunk wrapper and run Lean
  on that wrapper; it may report the associated `.b64` staging blob when one
  exists, but the checked Lean module must use byte literals.
- Do not use `native_decide`, `axiom`, `sorry`, `admit`, or `unsafe`.

Done when:

```bash
lake build Cuboctahedron.Search.CertificateChecker
...
passes on the regenerated byte-list wrappers, and a one-certificate byte-list
smoke chunk also passes. If the byte-list parser/checker still fails to
kernel-reduce, record the measured failure and add a new step for a truly
predecoded structured-literal backend with a fresh source-size budget.

Result recorded on 2026-06-19:

- `lake build Cuboctahedron.Search.CertificateChecker` passed after adding
  byte-list checker entry points.
- A one-certificate nonidentity byte-list smoke file failed to reduce
  `checkPackedResidualBytes chunkBytes = true` with trusted `decide`.
- A one-certificate translation byte-list smoke file timed out reducing
  `checkPackedTranslationFarkasBytes chunkBytes = true`.
- A one-certificate structured-literal smoke file also failed with bare
  Boolean reduction for both compact checkers.

Therefore Step 14E.7B6 is not an accepted full backend. Keep the byte-list
checker functions as diagnostic scaffolding, but do not select the byte-list
backend for Step 14E.7B. Continue with Step 14E.7B7.

## Step 14E.7B7: Proof-Carrying Structured Literal Backend

Goal: replace trusted large Boolean reduction with generated proof-carrying
structured literals. Each generated chunk should expose compact certificate
literals plus small local theorems proving the relevant Boolean subchecks, then
assemble chunk soundness from those theorem facts rather than from `decide` over
the whole checker expression.

This step is required because both compact binary parsing and direct compact
record checking failed trusted one-certificate smoke tests. The failure is not
about trust: it is about Lean reduction not normalizing these checker
expressions far enough for `decide`.

Requirements:

- Do not use `native_decide`, `axiom`, `sorry`, `admit`, or `unsafe`.
- Add a source-size preflight for structured proof-carrying chunks before
  regenerating the full corpus.
- For nonidentity residual cases:
  - keep the compact residual record literals;
  - avoid `checkNonIdCoveredRank` recomputation when possible by deriving
    `ordinary.word = unrankPairWord rank` from generated rank/unrank witness
    facts or definitional `unrankPairWord` use;
  - emit generated lemmas for the residual subchecks currently proved in
    `ResidualTemplates.lean`, but share theorem bodies by failure shape wherever
    possible.
- For translation Farkas cases:
  - keep compact `(rank, mask, SourceFarkasCert)` literals;
  - emit source-Farkas subcheck proofs using the existing
    `translation_check_body_lines` source-Farkas simplification pattern;
  - group identical source-Farkas shapes so one proof body is transported to
    all matching `(rank, mask)` cases if possible.
- The generated chunk theorem should assemble from proved per-cert or
  per-family facts, not from `decide` over a parser/checker expression.
- The independent checker must validate the generated proof-carrying manifest,
  chunk ordering, no-forbidden-token constraints, and source-size estimate.
- The largest generated chunk must pass the memory-capped smoke guard before a
  full `lake build` is attempted.
...
python3 scripts/check_certificates_independently.py --mode exhaustive-real-certs
python3 scripts/smoke_largest_generated_chunk.py --memory-limit-gib 8 --timeout-seconds 900
lake build Cuboctahedron.Generated.NonIdentity.Residual.All
lake build Cuboctahedron.Generated.Translation.Farkas.All
```

passes for the proof-carrying structured-literal backend, with the manifest
recording that backend explicitly.

Implementation status:

- Added `--mode proof-carrying-structured-literals` to
  `scripts/generate_exact_certificates.py`.
- Added independent validation for
  `scripts/generated/proof_carrying_structured_literals.json`.
- Added Lean evidence records `CheckedNonIdRank` and
  `CheckedTranslationCase` in `Cuboctahedron/Search/CertificateFormat.lean`.
- Generated and Lake-built proof-carrying smoke modules:
...
- The manifest currently records `full_backend_complete = false`.
- The naive one-proof-per-case projection is about 162 GiB of Lean source, so
  this step is not complete as written for the full corpus. The next step must
  add proof sharing/family grouping before full emission.

Validated commands:

```bash
python3 -m py_compile scripts/generate_exact_certificates.py scripts/check_certificates_independently.py scripts/smoke_largest_generated_chunk.py
python3 scripts/generate_exact_certificates.py --mode proof-carrying-structured-literals
python3 scripts/check_certificates_independently.py --mode proof-carrying-structured-literals
lake build Cuboctahedron.Search.CertificateFormat
lake build Cuboctahedron.Generated.NonIdentity.Residual.ProofCarryingSmoke
lake build Cuboctahedron.Generated.Translation.Farkas.ProofCarryingSmoke
```

## Step 14E.7B8: Shared Proof-Carrying Family Backend

Goal: turn the trusted proof-carrying smoke backend from Step 14E.7B7 into a
size-safe full backend by sharing theorem bodies across residual nonidentity
families and translation source-Farkas families.

Requirements:

- Keep the `CheckedNonIdRank` and `CheckedTranslationCase` trusted interfaces.
- Replace per-case duplicated theorem bodies with family-level theorem schemas
  plus small per-case data facts.
- For residual nonidentity cases, group by failure kind, prefix state, axis
  witness shape, affine solve witness shape, and exact residual obstruction.
- For translation Farkas cases, group by source-Farkas shape and transport the
  proof across all `(rank, mask)` cases with the same checked constraint-source
  certificate.
- Add a source-size preflight using the actual shared-family generator output,
  not a one-cert smoke extrapolation.
- Refuse full emission unless projected tracked Lean source is below the hard
  repository budget and the largest generated chunk is below the memory smoke
  threshold.
- Extend the independent checker to validate family partitions, per-family
  proofs, per-case membership, and no-forbidden-token constraints.
...
Step 14E.7B6 shows that byte-list parsing is also not a viable
...
- Emit the fallback evidence using exactly this strategy:
  ...
  - predecoded packed-byte singleton nonidentity residual certificates for
    axis/simulation cases, parsed from Lean byte literals and checked by the
    generic compact residual checker;
  - translation prefix family for `badDirectionSign`;
  - translation prefix family for `badTranslationVector`;
  - predecoded packed-byte shared Farkas translation certificates for all
    normalized Farkas shapes.
...
the smoke test again. If a one-certificate predecoded byte-list chunk still
fails because `by decide` cannot kernel-reduce the packed parser/checker
expression, stop splitting and add the structured-literal backend described in
Step 14E.7B7 before attempting another full build.
