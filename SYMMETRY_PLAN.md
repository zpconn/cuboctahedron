...
- [x] Run Phase 6Z.6K.8AP.16DT positive-survivor classifier slice planner:
  ...
  ```
- [ ] Implement Phase 6Z.6K.8AP.16DU bounded positive-survivor candidate
  catalog emitter:
  generate and check a nonempty bounded
  `PositiveSurvivorBoolClassifierOnRange lo hi` or direct
  `SourcePositionRowProducerGoodCoverageOnRange lo hi` theorem from a private
  finite candidate catalog chunk.  It must use shared source/row producer facts
  and compact candidate membership, not rank-local singleton candidate-facts
  shards, all-mask replay, or generated bad-direction evidence.  Acceptance
  requires a focused guarded Lean build below 6500 MiB RSS, nonempty
  GoodDirection coverage over a meaningful bounded range/state, and an updated
  production projection still within the 5-6 wall-hour / 24 CPU-hour
  generated-checking budget.
