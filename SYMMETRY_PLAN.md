...
- The new bridge core defines `SourceRowFactsBridge` and
  `SourceRowPredicateBridge`, then proves both can produce
...
- Focused build passed:
  ...
  in 2.50s with 3,285,448 KiB peak RSS.
...
- The next proof-producing target is Phase 6Z.6K.8AP:
  `pairSignProducerCoverage_killedBridge`, a generated membership bridge from
  arbitrary `needsFarkas`/GoodDirection residual translation cases to
  source/row producer predicates and then to `Coverage.TranslationCaseKilled`.
...
- [ ] Implement Phase 6Z.6K.8AP producer membership bridge:
  define the generated membership root that turns an arbitrary
  `needsFarkas`/GoodDirection residual translation case into a matching source
  predicate and row predicate, then applies the generated source/row producer
  theorem and `TranslationGoodCaseKilled.killed` to satisfy
  `Cuboctahedron.Generated.Translation.KilledBridge`.
...
The next step is Phase 6Z.6K.8AP: implement the producer membership bridge
targeting `Cuboctahedron.Generated.Translation.KilledBridge`. That step should:

- keep using `SourceIndexStateKey.goodKilled_of_source_row` as the final
  semantic bridge for each covered producer case;
- prove or generate membership facts that derive a matching
...
wall time:   0:02.50
CPU:         103%
peak RSS:    3,285,448 KiB
exit status: 0
```

This is not global membership coverage. It proves that the remaining
production membership root only needs to supply either:

- `SourceRowPredicateBridge`, by deriving
  `SourceIndexStateSourcePredicate key.firstIndex key.secondIndex key.support r.val mask`
  plus `key.template.Rows key.support r.val mask`; or
