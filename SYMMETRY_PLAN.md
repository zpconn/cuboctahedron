...
- [x] Run Phase 6Z.6K.8AP.16DS production-style membership slice audit:
  ...
  ```
- [ ] Implement Phase 6Z.6K.8AP.16DT bounded positive-survivor classifier
  emitter:
  generate and check a nonempty bounded
  `PositiveSurvivorBoolSignatureClassifierOnRange lo hi` or direct
  `SourcePositionRowProducerGoodCoverageOnRange lo hi` theorem.  It must use
  shared source/row producer facts and compact positive-survivor signatures,
  not rank-local singleton candidate-facts shards, all-mask replay, or
  generated bad-direction evidence.  Acceptance requires a focused guarded
  Lean build below 6500 MiB RSS, nonempty GoodDirection coverage over a
  meaningful bounded range/state, and an updated production projection still
  within the 5-6 wall-hour / 24 CPU-hour generated-checking budget.
