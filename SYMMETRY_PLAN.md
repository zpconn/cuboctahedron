...
- [ ] Prove nonempty Phase 6Z.6K.8AP source/row language membership:
  generate or prove a real `SourceRowProducerGoodLanguageOnRange lo hi`,
  ...
  is to combine `SourcePairPositionSpec.Predicate` with row producer `Applies`
  facts into a nonempty producer-language `complete` theorem.  AP.5 catalogs
  may remain export boundaries, but not the production membership proof
  coordinate.
