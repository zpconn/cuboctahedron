...
6Z.6K.8V source-index/state language profiler, through the Phase 6Z.6K.8AN
producer-growth projection gate and Phase 6Z.6K.8AO production hierarchy
...
AP16DJ's bounded compact-cover batch plan and accepted split-trace guarded
batch retry, plus
AP16CP's
multi-fixture trace import smoke,
after rejecting raw `decide` against the recurrence as too reducer-heavy.
AP16DF is now accepted as a bounded smoke only through AP16DI's serial guard:
the old selected-impact normal proof was fixed for focused modules, the
rank101105 cover was parameterized away from rank100805-specific trace/rank
constants, and the two-signature root builds after selected-impact leaves are
...
AP16DF-style experiment.  AP16DJ is the bounded scaling gate: it plans a
five-signature compact-cover batch, rejects an eight-signature batch as too
large for the next safe smoke, and now emits AP16DK-style split trace modules.
The split-trace five-signature retry builds 126 serial Lean targets under the
AP16DI 5 GiB RSS guard, so future compact Walsh experiments should keep this
split layout and guarded serial build discipline.
...
Phase 6Z.6K.5 validates the source-parametric row-property quotient on the
same five disjoint 100k-rank calibration windows: all 63,725 sampled
GoodDirection survivors remain covered, and the primary
row-property-parametric coordinate has only 24 families. The generic runner
still labels the report `needs-more-sampling` because its older projection
gate assumes independent rank-window family growth, but that is no longer the
primary theorem coordinate. Phase 6Z.6K.6 then implements a bounded
row-property quotient representative emitter/root over `[0,1000)`. The
bounded root is memory-safe when checked through the external-cache validator:
46 generated group modules cover 1,465 GoodDirection survivors with zero
unsupported survivors; all groups check with max RSS 5.121 GiB and the root
checks with RSS 3.48 GiB after cached group `.olean`s are mirrored for import
...
Phase 6Z.6K.8AH then validates the current producer-coverage theorem surface
over the broader representative windows: 195 descriptor keys are killed from
191 source producers and 21 row producers, with generation using four Python
workers at 48.5 MB RSS and the focused Lean smoke building in 0.87s at
0.85 GiB RSS. This still needs a global/scaled membership profile before it
can feed public generated coverage.
Phase 6Z.6K.8AI adds that first scaling profile. Across 65,000 disjoint
sampled ranks, it finds 13,899 GoodDirection cases, 328 descriptor families,
294 source groups, 24 row groups, and only 318 source+row producer
obligations, with zero source-language validation failures. The run uses four
Python workers, takes 7:35.67 wall time, and peaks at 65.2 MB RSS. This keeps
the producer route alive and promotes a larger/resumable producer census or a
bounded scaling Lean smoke as the next step.
Phase 6Z.6K.8AJ adds the resumable producer census runner itself. A
checkpointed 10-window run over 25,000 sampled ranks completes in 3:38.19 wall
time with four workers and only 29.9 MB max RSS, then aggregate-only mode
replays from checkpoints in 0.06 seconds. The aggregate has 6,048
GoodDirection cases, 232 descriptor families, 225 source groups, 22 row
groups, 247 source+row producer obligations, and zero source-language
validation failures.
Phase 6Z.6K.8AK turns the 8AJ checkpoint-window surface into a theorem-valued
Lean smoke. It generates a 16,155-line bounded module with 232 descriptor
families, 225 source producers, and 22 row producers; the focused build passes
in 13.72 seconds with 3.545 GiB peak RSS. This confirms that the broader
producer surface remains Lean-checkable when stated as semantic source/row
producer composition, not raw rank/mask replay. Phase 6Z.6K.8AL then splits
the same checkpoint-window producer surface into shared `Source`/`Row`
producer modules, four bounded descriptor chunks, and a tiny root. The split
root builds in 10.50 seconds with 3.343 GiB RSS. This confirms that the
accepted producer surface can be organized in a production-style import
hierarchy rather than one monolithic module. Phase 6Z.6K.8AM doubles the
checkpoint-window count to 20 disjoint 2,500-rank windows across the rank
space. It finds 6,734 GoodDirection cases, 315 descriptor families, 298 source
groups, and the same 22 row groups; it emits five bounded descriptor chunks
and a tiny root. The focused split-root build passes in 10.86 seconds with
3.300 GiB peak RSS. This keeps the source/row producer route alive and shows
that bounded chunking, not one-file growth, is the right Lean import shape.
Phase 6Z.6K.8AN then runs a 39-window checkpointed census over 97,500 sampled
ranks and an incidence projection over the resulting keys. It observes 401
descriptor families, 374 source groups, 22 row groups, and 396 source+row
producer obligations, with zero source-language validation failures and only
29.9 MiB max window RSS. The incidence projection estimates 862 descriptor
families, 722 source groups, 26 row groups, 748 source+row producers, and
14 descriptor chunks at chunk size 64. This remains diagnostic, not proof, but
it is below the 5,000-descriptor and 2,000-producer gates.
Phase 6Z.6K.8AO converts that accepted projection into a concrete production
hierarchy design. Using the 8AM split-root Lean smoke as an empirical anchor,
it projects the translation producer hierarchy at about 862 descriptor
families, 722 source groups, 26 row groups, 748 source+row producers, 14
descriptor chunks, roughly 57,827 generated Lean lines / 2.3 MiB source, about
58 conservative CPU seconds, about 70 conservative wall seconds at four jobs,
and about 3.93 GiB peak RSS. This is still not proof evidence, but it accepts
the production source/row/chunk layout as well within the 5-6 hour wall-clock
budget. The remaining proof obligation is now sharper: build the membership
bridge `pairSignProducerCoverage_killedBridge`, deriving the source predicate
and row predicate for each arbitrary GoodDirection residual case, then applying
the generated source/row producer theorem and `TranslationGoodCaseKilled.killed`
to satisfy `Cuboctahedron.Generated.Translation.KilledBridge`.
Phase 6Z.6K.8AP.0 adds the hand-written bridge core
`PairSignProducerMembershipBridge.lean`. It defines the two remaining
membership targets, `SourceRowFactsBridge` and `SourceRowPredicateBridge`, and
proves that either one yields `Cuboctahedron.Generated.Translation.KilledBridge`.
The focused build passes in 2.50s with 3.29 GiB peak RSS. This is not global
membership coverage; it makes the final generated membership root precise.
...
Phase 6Z.6K.8AP.16A extends this bridge infrastructure: existing
source/row facts, predicate, and producer language surfaces now erase to the
direct descriptor-coverage target.  This is still not nonempty coverage, but
it lets the next generated emitter prove whichever semantic membership surface
is cheapest and recover descriptor coverage without additional generated data.
Phase 6Z.6K.8AP.16B reruns the pair-sign descriptor membership profiler with
memory-safe parallelism (`--jobs 4`) and accepts `source_language_full +
row_exact` as the next producer-membership route: on seven representative
windows it covers 7,112 GoodDirection cases with zero source-language
validation failures and 212 projected source+row producer obligations.
Phase 6Z.6K.8AP.16C adds the direct adapter from
`SourcePositionRowProducerGoodLanguageOnRange` to
`SourceIndexStateDescriptorGoodCoverageOnRange`, matching the strongest
source-position producer surface already in the repo.  The focused smoke builds
in 10.56s with 3,308,884 KiB peak RSS.  This is still bridge infrastructure
only; the nonempty membership theorem remains the AP.16 production gap.
Phase 6Z.6K.8AP.16D adds
`SourcePositionRowProducerGoodCoverageOnRange` as the smallest direct
source-position plus row-producer coverage target.  A generated chunk can now
prove only an existential source-position spec, row producer, key, source
predicate, and row-producer application for each arbitrary identity-linear
GoodDirection survivor; generic adapters package that into the source-position
language, all-Good coverage, and descriptor coverage.  The source-position
smoke builds in 8.27s with 3,296,632 KiB peak RSS, and the descriptor smoke
...
- Added the Phase 6Z.6K.8AP.0 bridge-core reports:
  - `scripts/generated/phase6z6k8ap_pair_sign_producer_membership_bridge_core.json`
  - `scripts/generated/phase6z6k8ap_pair_sign_producer_membership_bridge_core.md`
- Added the Phase 6Z.6K.8AP.1 GoodDirection-survivor bridge reports:
  - `scripts/generated/phase6z6k8ap1_pair_sign_good_membership_bridge.json`
  - `scripts/generated/phase6z6k8ap1_pair_sign_good_membership_bridge.md`
- Added the Phase 6Z.6K.8AP.2 classifier-free GoodDirection bridge reports:
  - `scripts/generated/phase6z6k8ap2_classifier_free_good_bridge.json`
  - `scripts/generated/phase6z6k8ap2_classifier_free_good_bridge.md`
- Added the Phase 6Z.6K.8AP.3 public all-Good coverage bridge reports:
  - `scripts/generated/phase6z6k8ap3_public_all_good_coverage_bridge.json`
  - `scripts/generated/phase6z6k8ap3_public_all_good_coverage_bridge.md`
- Added the Phase 6Z.6K.8AP.4 bounded AP interval smoke reports and generated
  Lean:
  - `scripts/generated/phase6z6k8ap4_source_index_state_bounded_ap_smoke.json`
  - `scripts/generated/phase6z6k8ap4_source_index_state_bounded_ap_smoke.md`
  - `Cuboctahedron/Generated/Translation/TwoSource/SupportFamilies/SourceIndexStateBoundedAPSmoke/`
- The new bridge core defines `SourceRowFactsBridge` and
  `SourceRowPredicateBridge`, plus `SourceRowFactsBridgeOnRange` and
  `SourceRowPredicateBridgeOnRange` for generated chunks. It proves the
  global bridges can produce `Cuboctahedron.Generated.Translation.KilledBridge`
  and the range bridges can produce `ResidualTranslationCoverageOnRange lo hi`.
- It also defines `ResidualTranslationGoodRankKilled` and
  `ResidualTranslationGoodCoverageOnRange`, and proves range chunks can target
  the cheaper GoodDirection-survivor predicate before the full-range root
  applies `TranslationGoodCaseKilled.killed`.
  It now also defines `AllTranslationGoodRankKilled`,
  `AllTranslationGoodCoverageOnRange`, `SourceRowFactsGoodBridge`, and
  `SourceRowPredicateGoodBridge`, plus range variants.  This is the preferred
  AP.2 target because generated chunks can prove source/row membership from
  `totalLinear = matId` and `GoodDirectionAtRank`, without depending on
  `translationEarlyFamilyClassOfChoice`.
- Focused build passed:
  `lake build Cuboctahedron.Generated.Translation.TwoSource.SupportFamilies.PairSignProducerMembershipBridge`
  in 4.12s with 3,277,560 KiB peak RSS after adding the classifier-free
  GoodDirection bridge.
- Added `Translation.Coverage.AllGoodRankKilled`,
  `Translation.Coverage.caseKilled_of_all_good_interval`,
  `PublicAllGoodSemanticCoverageIntervals`, and
  `semanticGeneratedCoverageOfAllGoodIntervals`.  These plug the AP.2
  classifier-free all-GoodDirection target into public semantic generated
  coverage without introducing raw evidence data.
- Focused AP.3 builds passed:
  `lake build Cuboctahedron.Generated.Translation.Coverage.All` in 0.91s
  with 856,656 KiB peak RSS, and
  `lake build Cuboctahedron.Generated.ExhaustiveCoverage` in 3.03s with
  3,287,764 KiB peak RSS.
- The AP.4 bounded root proves the new public AP surface on `[0,16)`:
  `AllTranslationGoodCoverageOnRange 0 16` and
  `CoversInterval Translation.Coverage.AllGoodRankKilled 0 16`.  The build
  passed, but took 288.66s wall time and 14,041,472 KiB peak RSS, so this
  finite rank/mask dispatcher is rejected as a production backend.  It remains
  useful only as a bridge regression test.
- Added the Phase 6Z.6K.8AP.5 catalog-erasure bridge reports:
  - `scripts/generated/phase6z6k8ap5_catalog_erasure_bridge.json`
  - `scripts/generated/phase6z6k8ap5_catalog_erasure_bridge.md`
- Added the Phase 6Z.6K.8AP.6 catalog-membership smoke reports and Lean:
...
- Added the Phase 6Z.6K.8AP.7 membership-surface audit:
...
- Added the Phase 6Z.6K.8AP.8 source/row language bridge reports and Lean:
  - `scripts/generated/phase6z6k8ap8_source_row_language_bridge.json`
  - `scripts/generated/phase6z6k8ap8_source_row_language_bridge.md`
  - `Cuboctahedron/Generated/Translation/TwoSource/SupportFamilies/SourceRowLanguageSmoke.lean`
- Added the Phase 6Z.6K.8AP.9 source-position spec bridge reports:
  - `scripts/generated/phase6z6k8ap9_source_position_spec_bridge.json`
  - `scripts/generated/phase6z6k8ap9_source_position_spec_bridge.md`
- Added the Phase 6Z.6K.8AP.10 source/row producer language bridge reports
  and Lean:
  - `scripts/generated/phase6z6k8ap10_source_row_producer_language_bridge.json`
  - `scripts/generated/phase6z6k8ap10_source_row_producer_language_bridge.md`
  - `Cuboctahedron/Generated/Translation/TwoSource/SupportFamilies/SourceRowProducerLanguageSmoke.lean`
- Added the Phase 6Z.6K.8AP.11 source-position producer language bridge
  reports and Lean:
  - `scripts/generated/phase6z6k8ap11_source_position_producer_language_bridge.json`
...
- Added the Phase 6Z.6K.8AP.16A descriptor/facts adapter reports:
  - `scripts/generated/phase6z6k8ap16a_descriptor_facts_adapter.json`
  - `scripts/generated/phase6z6k8ap16a_descriptor_facts_adapter.md`
- Added the Phase 6Z.6K.8AP.16B producer-membership candidate reports:
  - `scripts/generated/phase6z6k8ap16b_producer_membership_candidate.json`
  - `scripts/generated/phase6z6k8ap16b_producer_membership_candidate.md`
- Added the Phase 6Z.6K.8AP.16C source-position descriptor adapter reports:
  - `scripts/generated/phase6z6k8ap16c_source_position_descriptor_adapter.json`
  - `scripts/generated/phase6z6k8ap16c_source_position_descriptor_adapter.md`
- Added the Phase 6Z.6K.8AP.16D source-position direct coverage target
  reports:
...
- Added the Phase 6Z.6K.8AP.16BN generated-style Walsh polynomial bound smoke:
  - `Cuboctahedron/Generated/Translation/TwoSource/SupportFamilies/ImpactSubcubeWalshGeneratedSmoke.lean`
  - `scripts/generated/phase6z6k8ap16bn_walsh_generated_smoke.json`
  - `scripts/generated/phase6z6k8ap16bn_walsh_generated_smoke.md`
- Added the Phase 6Z.6K.8AP.16BO scripted Walsh-bound emitter smoke:
  - `scripts/generate_ap16bo_walsh_bound_smoke.py`
...
- Added the Phase 6Z.6K.8AP.16BY Walsh symbolic all-selected-impact smoke:
  - `scripts/generate_ap16by_walsh_symbolic_all_smoke.py`
...
- Phase 6Z.6K.8AO accepts the production translation producer hierarchy as a
  small source/row/chunk layout: projected 862 descriptor families, 748
...
- The next proof-producing target is Phase 6Z.6K.8AP.16:
  ...
  The AP.16B diagnostic selects the first concrete emitter target:
  `SourceRowProducerGoodLanguageOnRange lo hi` with source producers keyed by
  `source_language_full` and row producers keyed by `row_exact`, then erased to
  descriptor coverage through AP.16A.
...
Phase 6Z.6J.1 completed result: representative-window D4 semantic-family
transport aggregate is implemented and rejected for near-term Lean transport.

- Extended `scripts/profile_d4_semantic_family_transport.py` with
  `--representative-windows`.
- The aggregate loads the representative calibration windows from
  `translation_row_relation_families_profile_representative.json`, falling
  back to the documented calibration windows if that file is unavailable.
- It reuses the exact same row-template/diamond classifier and D4 round-trip
  validation as Phase 6Z.6J.
- It emits:
  - `scripts/generated/phase6z6j_d4_semantic_family_transport_representative.json`
  - `scripts/generated/phase6z6j_d4_semantic_family_transport_representative.md`
- Validation run:
...
python3 scripts/profile_d4_semantic_family_transport.py \
  --representative-windows \
  --workers 24 --shard-size 250 \
  --output-json scripts/generated/phase6z6j_d4_semantic_family_transport_representative.json \
  --output-md scripts/generated/phase6z6j_d4_semantic_family_transport_representative.md
```
...
- Interpretation:
  - `D4` transport is soundly measurable but too small to justify a Lean
    transport layer for the 5-6 hour build target.
  - The representative reduction, `1.265x`, is below the 1.5x gate.
  - Do not spend the next phase formalizing D4 transport adapters.
  - Return to the real semantic-family bottleneck: row extraction and
    source-agreement facts should become symbolic/integer family facts rather
    than repeated rank/mask reconstruction.

Phase 6Z.6K completed result: symbolic/integer row-extraction family profiler
is implemented, and the result calls for a targeted Lean pilot.

- Added `scripts/profile_symbolic_row_extraction_families.py`.
- The profiler is diagnostic only and emits no Lean evidence.
- It scans the representative calibration windows and recomputes every
  translation GoodDirection survivor.
- It compares proof-relevant family coordinates:
  - current `template_source`;
  - source-agreement only;
  - row-property only;
  - combined source-agreement plus row-property;
  - integer/scaled exact rows;
  - exact row shapes.
- It emits:
  - `scripts/generated/phase6z6k_symbolic_row_extraction_profile.json`
  - `scripts/generated/phase6z6k_symbolic_row_extraction_profile.md`
- Validation run:

```bash
python3 scripts/profile_symbolic_row_extraction_families.py \
  --workers 24 --shard-size 250 \
  --output-json scripts/generated/phase6z6k_symbolic_row_extraction_profile.json \
  --output-md scripts/generated/phase6z6k_symbolic_row_extraction_profile.md
```
...
exact row shapes:                    1,641
template_integer_scaled:             1,641
template_source_integer_scaled:      1,748
source-agreement unstable families:      0
row-property unstable families:          1
integer-scaled unstable families:      103
decision:          needs-targeted-lean-pilot
```

- Interpretation:
  - The symbolic route is not rejected.
  - Exact/scaled row data still fragments badly, so exact row-shape or
    integer-scaled row-shape emission is not the right proof coordinate.
  - Source agreement is stable across every observed `template_source` family.
  - Row-property facts are stable for all but one observed family.
  - The one unstable family is an `axis_b_only` source family with four
    row-property subcases.
  - The follow-up Phase 6Z.6K.1 is the small Lean smoke for source-agreement
    plus row-property family facts, including an explicit split for the
    `axis_b_only` four-subcase family.

Phase 6Z.6K.1 completed result: targeted symbolic Lean smoke accepted.

- Added
  `Cuboctahedron/Generated/Translation/TwoSource/SupportFamilies/SymbolicFacts.lean`.
- Added `scripts/generate_symbolic_row_family_smoke.py`.
- Generated
...
- The helper module factors row-template proofs into:
  - `SourceAgrees support r mask`;
  - a row-property predicate such as `EqEqPosVarFirstRows`,
    `OppOneMinusVarFirstRows`, `OppMinusOneVarFirstRows`, or
    `AxisBOnlyRows`;
  - a tiny bridge theorem recombining those facts into the already-proved
    row-template predicates.
- The smoke generator selected:
  - the three largest stable `template_source` families; and
  - all four row-property subcases of the single unstable `axis_b_only`
    source family.
- Smoke selection stats:

```text
pair words scanned:                21,036
identity words:                       152
translation sign assignments:       9,721
GoodDirection survivors:            1,280
selected smoke cases:                   7
axis_b_only row subcases:               4
```

- Validation passed:

```bash
lake build Cuboctahedron.Generated.Translation.TwoSource.SupportFamilies.SymbolicFacts
python3 -m py_compile scripts/generate_symbolic_row_family_smoke.py
python3 scripts/generate_symbolic_row_family_smoke.py
lake build Cuboctahedron.Generated.Translation.TwoSource.SupportFamilies.SymbolicSmoke
rg -n "TranslationCert|checkTranslationCert|native_decide|sorry|admit|unsafe" \
  Cuboctahedron/Generated/Translation/TwoSource/SupportFamilies/Symbolic*.lean || true
git diff --check
```

- Interpretation:
  - The source-agreement plus row-property split is Lean-checkable for the
    representative symbolic family surface.
  - The `axis_b_only` row-property split is viable in Lean when the four
    row-property subfamilies are explicit.
  - The smoke still uses private concrete rank/line facts as local witnesses;
    this is acceptable for the bounded pilot but is not final coverage.
  - The next active translation step is to build a production symbolic-family
    emitter that exports semantic family predicates and membership/coverage
    theorems, while avoiding public exact-row-shape keys and avoiding any
    ordinary `TranslationCert` bridge.

Phase 6Z.6K.2 completed result: bounded symbolic representative root accepted.

- Extended
  `Cuboctahedron/Generated/Translation/TwoSource/SupportFamilies/SymbolicFacts.lean`
  so the source-agreement/row-property split covers the full row-template
  vocabulary:
  - positive/negative equal-row templates, first and second variants;
  - positive/negative opposite-row templates, first and second variants;
  - `axis_a_only`;
  - `axis_b_only`;
  - `exact_two_source_valid`.
- Extended `scripts/generate_symbolic_row_family_smoke.py` with
  `--mode representative-root`.
- Generated theorem-valued representative modules under:
  - `Cuboctahedron/Generated/Translation/TwoSource/SupportFamilies/SymbolicRepresentative/Group000.lean`
  - ...
  - `Cuboctahedron/Generated/Translation/TwoSource/SupportFamilies/SymbolicRepresentative/Group007.lean`
  - `Cuboctahedron/Generated/Translation/TwoSource/SupportFamilies/SymbolicRepresentative/All.lean`
- Generated diagnostic summary:
  `scripts/generated/phase6z6k2_symbolic_representative_summary.json`.
- The generated modules export semantic predicates of this form:

```lean
SourceAgrees support r mask ∧ RowPropertyRows support r mask
```

  and prove `TranslationGoodCaseKilled` through the generic
  row-template soundness theorems. They do not construct `TranslationCert`
  values and do not call `checkTranslationCert`.

- Representative root stats:

```text
symbolic families:                 126
GoodDirection survivors:          4,779
observed covered cases:           4,779
groups:                               8
largest family size:              1,260
singleton families:                  12

template counts:
  eq_eq_pos_var_first             2,173
  opp_1m_var_first                1,020
  eq_eq_pos_var_second              541
  opp_m1_var_first                  485
  opp_1m_var_second                 256
  opp_m1_var_second                 160
  eq_eq_neg_var_first               107
  axis_b_only                        24
  eq_eq_neg_var_second               13
```
...
python3 -m py_compile scripts/generate_symbolic_row_family_smoke.py
python3 scripts/generate_symbolic_row_family_smoke.py
python3 scripts/generate_symbolic_row_family_smoke.py --mode representative-root
lake build Cuboctahedron.Generated.Translation.TwoSource.SupportFamilies.SymbolicRepresentative.Group000
lake build Cuboctahedron.Generated.Translation.TwoSource.SupportFamilies.SymbolicRepresentative.All
lake build Cuboctahedron.Generated.Translation.TwoSource.SupportFamilies.SymbolicSmoke
rg -n "TranslationCert|checkTranslationCert|native_decide|sorry|admit|unsafe" \
  Cuboctahedron/Generated/Translation/TwoSource/SupportFamilies/SymbolicFacts.lean \
  Cuboctahedron/Generated/Translation/TwoSource/SupportFamilies/SymbolicSmoke.lean \
  Cuboctahedron/Generated/Translation/TwoSource/SupportFamilies/SymbolicRepresentative \
  scripts/generate_symbolic_row_family_smoke.py || true
git diff --check
```

- Interpretation:
  - The representative symbolic root is now a real Lean-checkable semantic
    coverage surface for the bounded calibration set.
  - The generated group/root files remain small and compile quickly.
  - This is still bounded representative coverage, not global coverage.
  - The next active translation step is a global symbolic-family census/profile
    using the same `template + source + row-property` coordinate. Do not emit
    full global Lean evidence until that census shows the global family count
...
Phase 6Z.6K.4 completed result: broader disjoint symbolic-family sampling
accepted the semantic two-source vocabulary but rejected the current
`template + source-agreement + row-property` coordinate as a global Lean
emission surface.

- Ran five disjoint 100k-rank calibration windows:
  `0`, `10000000`, `30000000`, `60000000`, and `90000000`.
- Checkpoints were kept outside the generated source tree under:
  `/tmp/cuboctahedron_symbolic_family_census_disjoint_100k`.
- Generated reports:
  - `scripts/generated/phase6z6k4_symbolic_family_disjoint_100k.json`
  - `scripts/generated/phase6z6k4_symbolic_family_disjoint_100k.md`
- Aggregate-only replay from checkpoints reproduced the same result.

Disjoint sample result:
...
max window RSS:                    24,128 KiB
observed window elapsed:          4,971.16 s
linear full-space symbolic projection: 130,573
decision: needs-more-sampling
```

- Interpretation:
  - The row-template/two-source semantic vocabulary still covers every sampled
    GoodDirection survivor, with no uncovered or non-two-source cases.
  - The broader sample shows rank-space distribution is highly nonuniform:
    later calibration windows contain many chunks with no GoodDirection
    survivors.
  - Even so, the current symbolic coordinate projects far above the
    low-thousands build gate, so do not emit full Lean leaves at this coordinate.
  - The row-property coordinate itself has only 24 families in this broader
    sample. The next translation compression step should try to make source
...
as a memory-safe bounded diagnostic and rejected as the final global scaling
shape.

- Added `scripts/generate_row_property_quotient_root.py`.
- Generated bounded Lean evidence under:
  `Cuboctahedron/Generated/Translation/TwoSource/SupportFamilies/RowPropertyQuotientRepresentative/`.
- Generated reports:
  - `scripts/generated/phase6z6k6_row_property_quotient_representative_summary.json`
  - `scripts/generated/phase6z6k6_row_property_quotient_representative_focused_groups_check.json`
  - `scripts/generated/phase6z6k6_row_property_quotient_representative_all_groups_check.json`
  - `scripts/generated/phase6z6k6_row_property_quotient_representative_all_groups_mirror_check.json`
  - `scripts/generated/phase6z6k6_row_property_quotient_representative_root_check.json`
- Bounded generator result:

```text
rank range:                    [0,1000)
covered GoodDirection cases:        1,465
unsupported GoodDirection cases:        0
row-property families:                11
generated groups:                     46
largest generated group source: 238,264 bytes
```

- Focused group validation command:

```bash
python3 scripts/check_row_relation_classifier_shards.py \
  --summary scripts/generated/phase6z6k6_row_property_quotient_representative_summary.json \
  --kind groups --select focused --jobs 1 \
  --memory-budget-gib 40 --job-gib 12 \
  --lean-memory-limit-gib 12 \
  --timeout-seconds 1800 --time \
  --compile-external --reuse-cache \
  --cache-dir evidence/.row_property_quotient_olean_cache \
  --output scripts/generated/phase6z6k6_row_property_quotient_representative_focused_groups_check.json
```
...
checked groups:              4
failed groups:               0
max focused RSS:        5.06 GiB
max focused elapsed:   198.45 s
```

- All-group validation command:

```bash
python3 scripts/check_row_relation_classifier_shards.py \
  --summary scripts/generated/phase6z6k6_row_property_quotient_representative_summary.json \
  --kind groups --select all --jobs 2 \
  --memory-budget-gib 40 --job-gib 14 \
  --lean-memory-limit-gib 12 \
  --timeout-seconds 1800 --time \
  --compile-external --reuse-cache \
  --cache-dir evidence/.row_property_quotient_olean_cache \
  --output scripts/generated/phase6z6k6_row_property_quotient_representative_all_groups_check.json
```
...
group results:                 46
groups checked:                42
groups reused from cache:       4
failed groups:                  0
max RSS:                 5.121 GiB
max single-group elapsed: 195.355 s
sum elapsed across groups: 7,713.711 s
effective jobs:                 2
```

- Root validation required a cached mirror pass because Lean import resolution
  for the root module expects imported group `.olean`s under `.lake`. The
  mirror pass reused external-cache artifacts and did not recompile groups:
...
python3 scripts/check_row_relation_classifier_shards.py \
  ...
  --compile-external --reuse-cache --mirror-to-lake \
  --cache-dir evidence/.row_property_quotient_olean_cache \
  --output scripts/generated/phase6z6k6_row_property_quotient_representative_all_groups_mirror_check.json
```
...
python3 scripts/check_row_relation_classifier_shards.py \
  --summary scripts/generated/phase6z6k6_row_property_quotient_representative_summary.json \
  --kind root --select all --jobs 1 \
  --memory-budget-gib 40 --job-gib 14 \
  --lean-memory-limit-gib 12 \
  --timeout-seconds 600 --time \
  --compile-external --reuse-cache --mirror-to-lake \
  --cache-dir evidence/.row_property_quotient_olean_cache \
  --output scripts/generated/phase6z6k6_row_property_quotient_representative_root_check.json
```

- Root result:

```text
root modules checked:      1
root failures:             0
root RSS:           3.48 GiB
root elapsed:        1.3 s
```

- Forbidden-token check passed for the generated row-property representative
  directory:

```bash
grep -R "sorry\|admit\|axiom\|native_decide\|unsafe" \
  Cuboctahedron/Generated/Translation/TwoSource/SupportFamilies/RowPropertyQuotientRepresentative || true
```

- Interpretation:
  - The OOM failure was caused by broad package import fanout, not by the
    individual group modules.
  - The safe validation protocol is external-cache group compilation with
    bounded jobs and per-process memory limits, followed by a cached mirror
    only when a root import check needs `.lake` import resolution.
  - Do not run broad `lake build
    Cuboctahedron.Generated.Translation.TwoSource.SupportFamilies.RowPropertyQuotientRepresentative.All`
...
Phase 6Z.6K.8A completed result: membership signature discovery found a small
bounded pilot coordinate and added the generic Lean interface for future
membership families.

- Added
  `Cuboctahedron/Generated/Translation/TwoSource/SupportFamilies/MembershipBridge.lean`.
- Added `scripts/profile_row_property_membership_signatures.py`.
- Generated reports:
  - `scripts/generated/phase6z6k8a_row_property_membership_signatures.json`
  - `scripts/generated/phase6z6k8a_row_property_membership_signatures.md`
- Commands:

```bash
python3 -m py_compile scripts/profile_row_property_membership_signatures.py
python3 scripts/profile_row_property_membership_signatures.py \
  --rank-start 0 --limit 1000
lake build Cuboctahedron.Generated.Translation.TwoSource.SupportFamilies.MembershipBridge
```
...
covered two-source cases:               1,465
uncovered/non-two-source cases:             0
best pilot coordinate:      source_kind_row_property
best pilot family count:                   11
family gate:                              200
decision:                 needs-targeted-lean-pilot
```
...
```text
theorem_constructor:                    9
row_property_parametric:               11
source_kind_row_property:              11
source_kind_impact_row_property:       33
source_pair_skeleton_row_property:     43
source_pair_sign_skeleton_row_property:53
source_agreement:                      72
source_row_property:                   74
local_word_window_row_property:       151
exact_row_shape:                      574
```
...
- [x] Implement Phase 6Z.6J D4 semantic-family transport profiler after family
  predicates exist.
- [x] Implement and reject Phase 6Z.6J.1 representative-window D4 semantic-family
  transport aggregate before any Lean transport work.
- [x] Implement Phase 6Z.6K symbolic/integer row-extraction and
  source-agreement family profiler for the accepted row-template/diamond
  semantic families.
- [x] Implement Phase 6Z.6K.1 targeted Lean smoke for symbolic
  source-agreement plus row-property family facts, including the
  `axis_b_only` four-subcase split.
...
- [x] Implement Phase 6Z.6K.8AG pair-sign descriptor membership state-language profile:
  ...
  member replay.
- [x] Implement Phase 6Z.6K.8AH bounded producer-coverage Lean smoke:
  ...
  rank/mask replay.
- [x] Implement Phase 6Z.6K.8AI producer-coverage scaling profile:
  scale the pair-sign producer-coverage route beyond representative windows
  and accept it only if source/row producer membership stays bounded without
  member-list replay.
- [x] Implement Phase 6Z.6K.8AJ producer-coverage census runner:
  make the accepted 8AI scaling route resumable over many bounded windows, or
  emit a bounded scaling Lean smoke if the next profile remains below gate.
- [x] Implement Phase 6Z.6K.8AK bounded scaling Lean smoke:
  use the checkpointed 8AJ surface to either emit a broader theorem-valued
  Lean smoke, or extend the resumable census enough to decide whether a global
  producer root is plausible under the 24 CPU-hour budget.
- [x] Implement Phase 6Z.6K.8AL split producer group/root design:
  split the accepted checkpoint-window producer surface into shared source
  and row modules, bounded descriptor chunks, and a tiny root; focused build
  passed in 10.50s with 3.343 GiB RSS.
- [x] Implement Phase 6Z.6K.8AM larger split-root/census gate:
  emit a 20-window split-root smoke over 50,000 sampled ranks; the generated
  root covers 6,734 GoodDirection cases with 315 descriptor families, 298
  source groups, 22 row groups, five chunks, and builds in 10.86s with
  3.300 GiB RSS.
- [x] Implement Phase 6Z.6K.8AN producer-growth projection gate:
  run a 39-window checkpointed census over 97,500 sampled ranks and estimate
  862 descriptor families, 748 source+row producers, and 14 descriptor chunks
  by incidence projection, below the current gates.
- [x] Implement Phase 6Z.6K.8AO production split hierarchy design:
  turn the accepted producer-growth projection into a concrete full-emission
  design: module layout, group/root import strategy, expected source size,
  expected build time, and the remaining Lean bridge needed to connect this
  producer surface to public `Translation.complete`.
- [x] Implement Phase 6Z.6K.8AP.0 producer membership bridge core:
  add the hand-written theorem adapter showing that either
  `SourceRowPredicateBridge` or `SourceRowFactsBridge` is enough to produce
  `Cuboctahedron.Generated.Translation.KilledBridge`; also add range variants
  `SourceRowPredicateBridgeOnRange` and `SourceRowFactsBridgeOnRange` so
  generated chunks can compose `ResidualTranslationCoverageOnRange lo hi`
  before the full-range root produces the killed bridge.
- [x] Implement Phase 6Z.6K.8AP.1 GoodDirection-survivor bridge adapters:
  add `ResidualTranslationGoodRankKilled`,
  `ResidualTranslationGoodCoverageOnRange`, and full-range adapters so
  generated chunks can first prove GoodDirection-survivor coverage and then
  recover `Cuboctahedron.Generated.Translation.KilledBridge` through
  `TranslationGoodCaseKilled.killed`.
- [x] Implement Phase 6Z.6K.8AP.2 classifier-free GoodDirection bridge:
  add `AllTranslationGoodRankKilled`, `AllTranslationGoodCoverageOnRange`,
  `SourceRowFactsGoodBridge`, `SourceRowPredicateGoodBridge`, and range
  variants so generated chunks can avoid the old noncomputable early
  classifier and prove identity-linear GoodDirection survivor coverage
  directly.
- [x] Implement Phase 6Z.6K.8AP.3 public all-Good coverage bridge:
  add `Translation.Coverage.AllGoodRankKilled`,
  `Translation.Coverage.caseKilled_of_all_good_interval`,
  `PublicAllGoodSemanticCoverageIntervals`, and
  `semanticGeneratedCoverageOfAllGoodIntervals`; validate the translation
  coverage and public exhaustive coverage adapters with focused serial builds.
- [x] Implement Phase 6Z.6K.8AP.4 bounded AP interval smoke:
  adapt the bounded source-index/state generator to export
  `AllTranslationGoodCoverageOnRange lo hi` and public
...
- [x] Implement Phase 6Z.6K.8AP.8 source/row language bridge:
  add `SourceRowFactsGoodLanguageOnRange` and
  `SourceRowPredicateGoodLanguageOnRange`, plus erasure adapters to
  `SourceRowFactsGoodBridgeOnRange`,
  `SourceRowPredicateGoodBridgeOnRange`, and
  `AllTranslationGoodCoverageOnRange`.  Validate the API with an empty-range
  smoke.  This is accepted bridge infrastructure only.
- [x] Implement Phase 6Z.6K.8AP.9 source-position spec bridge:
  add `SourcePositionSpec` and `SourcePairPositionSpec` so generated source
  languages can prove static `xpStart`/ordering and dynamic interior source
  list lookups by semantic position predicates.  Validate dynamic and static
  source-fact examples in `SourcePositionLanguageSmoke`.
- [x] Implement Phase 6Z.6K.8AP.10 source/row producer language bridge:
  add `SourceRowProducerGoodLanguageOnRange` so generated modules can prove a
  language over private source/row producer pairs and erase it to
  `SourceRowFactsGoodLanguageOnRange`, `SourceRowFactsGoodBridgeOnRange`, and
  `AllTranslationGoodCoverageOnRange`.  Validate the erasure path with an
  empty-range smoke.
- [x] Implement Phase 6Z.6K.8AP.11 source-position producer language bridge:
  add `SourcePositionRowProducerGoodLanguageOnRange` so generated modules can
...
- [x] Implement Phase 6Z.6K.8AP.16A descriptor/facts adapter:
  add adapters from `SourceRowFactsGoodBridgeOnRange`,
  `SourceRowPredicateGoodBridgeOnRange`,
  `SourceRowFactsGoodLanguageOnRange`,
  `SourceRowPredicateGoodLanguageOnRange`, and
  `SourceRowProducerGoodLanguageOnRange` to
  `SourceIndexStateDescriptorGoodCoverageOnRange`.  Validate the path with an
  empty-range smoke.  This is accepted bridge infrastructure only.
- [x] Implement Phase 6Z.6K.8AP.16B producer-membership candidate profile:
  rerun the pair-sign descriptor membership profiler with `--jobs 4` and record
  `source_language_full + row_exact` as the accepted producer-membership route
  over the representative windows: 7,112 GoodDirection cases, zero
  source-language validation failures, 212 projected source+row producer
  obligations, 194.14s wall time, and 50,176 KiB peak RSS.  This is diagnostic
  only.
- [x] Implement Phase 6Z.6K.8AP.16C source-position descriptor adapter:
  add `SourcePositionRowProducerGoodLanguageOnRange.to_descriptorCoverage` so
  the source-position plus row-producer language selected by the AP path erases
  directly to `SourceIndexStateDescriptorGoodCoverageOnRange`.  Validate with
  the descriptor-language smoke in 10.56s wall time and 3,308,884 KiB peak RSS.
  This is accepted bridge infrastructure only.
- [x] Implement Phase 6Z.6K.8AP.16D source-position direct coverage target:
  add `SourcePositionRowProducerGoodCoverageOnRange`,
  `SourcePositionRowProducerGoodLanguageOnRange.of_coverage`, and adapters to
  both `AllTranslationGoodCoverageOnRange` and
  `SourceIndexStateDescriptorGoodCoverageOnRange`.  Validate with focused
...
- [x] Implement Phase 6Z.6K.8AP.16BN generated-style Walsh polynomial bound
  smoke:
  AP16BN adds
  `Cuboctahedron/Generated/Translation/TwoSource/SupportFamilies/ImpactSubcubeWalshGeneratedSmoke.lean`.
  It uses AP16BJ's first selected subcube, label `*000**`, at rank `100805`
  and impact `1`.  The actual six-term Walsh polynomial is bounded over the
  whole subcube through `WalshSubcubeUpperBound`, with no per-mask
  nonpositivity replay:
  ...
  ```text
  lake build Cuboctahedron.Generated.Translation.TwoSource.SupportFamilies.ImpactSubcubeWalshGeneratedSmoke
  passed; elapsed 4.50s; peak RSS 3.935 GiB; min available 45.111 GiB
  ```
  ...
  Decision: accepted as the real coefficient-level proof shape.  It is not
  final evidence because it intentionally does not yet connect
  `impactDenomAtRank` to the emitted Walsh polynomial.  The next AP16BO target
  is to automate this one-subcube emitter and design the scalable
  denominator-equality bridge.
- [x] Implement Phase 6Z.6K.8AP.16BO scripted Walsh-bound emitter smoke:
  AP16BO adds `scripts/generate_ap16bo_walsh_bound_smoke.py`, which reads the
  AP16BJ selected-subcube data, recomputes the exact Walsh polynomial for the
  selected impact, emits termwise bounds, and generates
  `Cuboctahedron/Generated/Translation/TwoSource/SupportFamilies/ImpactSubcubeWalshEmitterSmoke.lean`.
  The generated smoke matches the AP16BN hand-written proof shape for the
  first selected subcube and proves polynomial nonpositivity through
  `WalshSubcubeUpperBound`, without per-mask nonpositivity replay.

  Focused guarded build:

  ```text
  lake build Cuboctahedron.Generated.Translation.TwoSource.SupportFamilies.ImpactSubcubeWalshEmitterSmoke
  passed; elapsed 3.00s; peak RSS 3.935 GiB; min available 45.153 GiB
  ```
  ...
  Decision: accepted as the first scripted Walsh-bound emitter.  The next
  AP16BP target is to emit all 20 selected AP16BJ subcube Walsh-bound
...
- [x] Implement Phase 6Z.6K.8AP.16BY Walsh symbolic all-selected-impact smoke:
  AP16BY adds `scripts/generate_ap16by_walsh_symbolic_all_smoke.py` and the
  generated module
  `Cuboctahedron/Generated/Translation/TwoSource/SupportFamilies/ImpactSubcubeWalshSymbolicAllSmoke.lean`.
  The generator computes affine Walsh normal/vector records and expected
  `WalshQuadratic` denominator records for every AP16BS selected impact at
  rank `100805`:
  ...
  It validates externally that the symbolic dot-product coefficients match
  the AP16BS Walsh denominator coefficients before writing the Lean smoke.
  Lean then re-checks each coefficient equality through
  `WalshAffineVec3.dot`, `WalshQuadratic.add`, and `WalshAffine.mul`.
  ...
  Decision: accepted as an all-selected-impact coefficient smoke.  It still
  does not prove `impactDenomAtRank = poly.eval`; the next production bridge
...
- [x] Implement Phase 6Z.6K.8AP.16CL Walsh-vector trace bridge:
  ...
  theorem translationPrefixWalshStep_eq_stepAt
      ...
      translationPrefixWalshStep w n hn pref =
        translationPrefixWalshStepAt w ⟨n, hn⟩ pref
...
- [x] Implement Phase 6Z.6K.8AP.16CM generated Walsh-vector trace smoke:
  AP16CM adds
  `scripts/generate_ap16cm_walsh_vector_trace_smoke.py` and generated smoke
  module
  `Cuboctahedron/Generated/Translation/TwoSource/SupportFamilies/ImpactSubcubeWalshVectorTraceSmoke.lean`.

  The smoke uses the rank-`100805` fixture from the AP16CI/AP16CJ work, but
  proves the generated Walsh vector by emitting normalized prefix vectors
  `generatedPref00` through `generatedPref13` and local step equalities:
  ...
  It then packages those local facts as:

  ```lean
  private def generatedTrace :
      TranslationWalshVectorTrace generatedWord generatedVector

  private theorem generatedVector_eq_translationVectorWalsh :
  ...
  This confirms the AP16CL trace API is usable by generated leaves and avoids
  the two rejected vector-proof paths: one giant recurrence normalization and
  raw `decide` equality over unreduced rational expressions.
  ...
  python3 scripts/run_memory_guarded.py \
    ...
    --min-available-mib 12000 \
    --poll-seconds 0.5 \
    --json /tmp/ap16cm_walsh_vector_trace_smoke_guard.json \
    -- bash -lc 'export LEAN_NUM_THREADS=1; export LAKE_JOBS=1; timeout 240s lake build Cuboctahedron.Generated.Translation.TwoSource.SupportFamilies.ImpactSubcubeWalshVectorTraceSmoke'
  ```
  ...
  passed
  elapsed: 12.02s
  peak tree RSS: 4795 MiB
  minimum available memory: 45316 MiB
  ```

  Decision: accepted as the first nonempty generated trace proof for the
  Walsh-vector fixture.  The next AP16 target is to wire this trace equality
  into the compact denominator smoke/vector premise, then generate the same
  local-trace shape from positive-survivor family data rather than a single
  fixture.
- [x] Implement Phase 6Z.6K.8AP.16CN compact-denominator trace bridge:
  AP16CN wires the AP16CM generated trace theorem into
  `ImpactSubcubeWalshSymbolicCompactDenomSmoke.lean`.

  The trace smoke now exports the generated fixture constants
  `generatedRank`, `generatedWord`, and `generatedVector`, plus:
  ...
  The compact-denominator smoke imports the trace smoke and supplies its old
  vector hook:
  ...
  It then closes the concrete compact denominator equality:
  ...
  This is the first bridge from a generated Walsh-vector trace into the
  compact denominator recurrence theorem.  It replaces the remaining local
  AP16CI vector premise for the rank-`100805`, mask-`0`, impact-`0` fixture
  without replaying the full translation-vector recurrence in the compact
  denominator file.
  ...
  python3 scripts/run_memory_guarded.py \
    ...
    --min-available-mib 12000 \
    --poll-seconds 0.5 \
    --json /tmp/ap16cn_compact_denom_trace_bridge_guard.json \
    -- bash -lc 'export LEAN_NUM_THREADS=1; export LAKE_JOBS=1; timeout 240s lake build Cuboctahedron.Generated.Translation.TwoSource.SupportFamilies.ImpactSubcubeWalshSymbolicCompactDenomSmoke'
  ```
  ...
  passed
  elapsed: 14.02s
  peak tree RSS: 4771 MiB
  minimum available memory: 45203 MiB
  ```

  Decision: accepted as a memory-safe AP16CI-to-AP16CM bridge.  The next
  AP16 target is to generalize this pattern from the one mask/impact fixture
  to generated positive-survivor family data: emit a trace once per word
  fixture, expose vector evaluation, and let compact denominator/source-row
  facts consume the vector theorem rather than rebuilding it.
- [x] Implement Phase 6Z.6K.8AP.16CO reusable Walsh-vector trace fixture
  emitter:
  AP16CO refactors `scripts/generate_ap16cm_walsh_vector_trace_smoke.py` so
  the emitter is no longer tied to the one smoke namespace.  It now accepts:

  ```text
  --rank <nat>
  --lean <path>
  --namespace <fully.qualified.Lean.namespace>
  ```

  The default command still regenerates
  `ImpactSubcubeWalshVectorTraceSmoke.lean`; the generated Lean theorem bodies
  are unchanged except for the module docstring.  This makes the AP16CM trace
  proof shape usable by later positive-survivor family generators without
  copying the script or hard-coding one fixture module.

  Guarded build through the dependent compact-denominator smoke:
  ...
  python3 scripts/run_memory_guarded.py \
    ...
    --min-available-mib 12000 \
    --poll-seconds 0.5 \
    --json /tmp/ap16co_trace_emitter_refactor_guard.json \
    -- bash -lc 'export LEAN_NUM_THREADS=1; export LAKE_JOBS=1; timeout 240s lake build Cuboctahedron.Generated.Translation.TwoSource.SupportFamilies.ImpactSubcubeWalshSymbolicCompactDenomSmoke'
  ```
  ...
  passed
  elapsed: 16.52s
  peak tree RSS: 4775 MiB
  minimum available memory: 45196 MiB
  ```

  Decision: accepted as a production-oriented emitter refactor.  The next
  AP16 target should generate a second tiny trace fixture under a distinct
  namespace/path, import two trace fixtures from one compact consumer smoke,
  and confirm the namespace-parametric emitter can support multiple generated
  word fixtures without name collisions.
- [x] Implement Phase 6Z.6K.8AP.16CP multi-fixture Walsh-vector trace smoke:
  AP16CP uses the AP16CO namespace-parametric emitter to generate a second
  trace fixture:

  ```text
  Cuboctahedron/Generated/Translation/TwoSource/SupportFamilies/
    ImpactSubcubeWalshVectorTraceRank101105Smoke.lean
  ```

  for rank `101105`, the singleton range already used by the
  positive-survivor membership smoke.  It also adds:

  ```text
  Cuboctahedron/Generated/Translation/TwoSource/SupportFamilies/
    ImpactSubcubeWalshVectorTraceMultiSmoke.lean
  ```

  The multi smoke imports both trace fixtures and proves that both exported
  vector-evaluation theorems are available in one consumer module:

  ```lean
  theorem firstTraceVectorEval (mask : SignMask) : ...
  theorem secondTraceVectorEval (mask : SignMask) : ...
  theorem traceFixtureRankValues : ... = 100805 /\ ... = 101105
  ```

  This confirms the reusable emitter can support multiple generated word
  fixtures without namespace collisions before it is connected to broader
  positive-survivor family generation.
  ...
  python3 scripts/run_memory_guarded.py \
    ...
    --min-available-mib 12000 \
    --poll-seconds 0.5 \
    --json /tmp/ap16cp_trace_multi_smoke_guard.json \
    -- bash -lc 'export LEAN_NUM_THREADS=1; export LAKE_JOBS=1; timeout 240s lake build Cuboctahedron.Generated.Translation.TwoSource.SupportFamilies.ImpactSubcubeWalshVectorTraceMultiSmoke'
  ```
  ...
  passed
  elapsed: 13.02s
  peak tree RSS: 4753 MiB
  minimum available memory: 45254 MiB
  ```

  Decision: accepted as a memory-safe multi-fixture trace check.  The next
...
- [x] Implement Phase 6Z.6K.8AP.16DK split Walsh-vector trace smoke:
  AP16DK adds a split-trace emitter:

  ```text
  scripts/generate_ap16dk_split_walsh_vector_trace_smoke.py
  ```

  It keeps the AP16CM/AP16DJ theorem surface but splits one generated
  `TranslationWalshVectorTrace` into:

  - one data module with the generated word, prefix vectors, final vector, and
    exported local simp facts;
  - thirteen local step-proof modules;
  - one final-vector proof module;
  - one tiny root that assembles the trace.

  The stress-rank split smoke was generated for rank `6000745`:
  ...
  scripts/generated/phase6z6k8ap16dk_split_trace_smoke.md
  ```
  ...
  ```text
  scripts/generated/phase6z6k8ap16dk_split_trace_smoke_guard.json
  scripts/generated/phase6z6k8ap16dk_split_trace_smoke_guard.md
  ```
  ...
  min available memory seen: 45780 MiB
  guard kill: no
  ```

  Decision: accepted as the trace-layout repair.  The monolithic
  rank-`6000745` trace exceeded the 5 GiB AP16DJ production gate, while the
  split trace builds below that cap without weakening the proof theorem
  surface.  Future compact denominator batch emission should use this split
  trace layout and should still build generated modules serially under the
  AP16DI memory guard.  The next step is to adapt the AP16DJ batch emitter and
  serial runner to use AP16DK split traces for every trace target, then rerun
  the same five-signature guarded batch under the 5 GiB cap.
- [x] Accept Phase 6Z.6K.8AP.16DJ split-trace guarded batch retry:
  AP16DJ's batch emitter now uses the AP16DK split trace layout for every
  selected signature instead of regenerating monolithic
  `ImpactSubcubeWalshVectorTrace...Smoke` modules.  The selected-impact compact
  denominator generator also simplifies `WalshAffine.const`, which was needed
  for the split route's generated selected-impact modules.
  ...
  The guarded batch command was:

  ```text
  python3 scripts/run_ap16dj_serial_guarded.py \
    --emit \
    --json /tmp/ap16dj_split_serial_guarded/summary_emit_retry.json \
    --out-dir /tmp/ap16dj_split_serial_guarded/targets_emit_retry
  ```

  Compact guard report:

  ```text
  scripts/generated/phase6z6k8ap16dj_serial_guard_split_emit_passed.json
  scripts/generated/phase6z6k8ap16dj_serial_guard_split_emit_passed.md
  ```
  ...
  Decision: accepted as a bounded split-trace compact Walsh batch smoke.  This
  repairs the AP16DJ monolithic-trace OOM-risk without weakening the theorem
  surface, but it is still smoke infrastructure rather than final generated
...
- [x] Implement Phase 6Z.6K.8AP.16DN compact Walsh classifier facts smoke:
  AP16DN adds a first nonempty singleton all-Good coverage theorem that
  combines the AP16DL/AP16DM compact-Walsh GoodDirection membership bridge with
  actual source/row candidate facts:

  ```text
  Cuboctahedron/Generated/Translation/TwoSource/SupportFamilies/
    PositiveSurvivorSharedRank6000480CandidateFactsSmoke.lean
    PositiveSurvivorSharedRank6000480RoutingSmoke.lean
    CompactWalshBatchClassifierFactsSmoke.lean
  ```
  ...
  The first implementation imported the larger
  `PositiveSurvivorSharedTop5Global4CandidateFactsSmoke` module and the
  4.8 GiB memory guard intentionally killed the build at 4825 MiB RSS.  That
  was the right failure mode: the proof should not load a 60-fact global bundle
  to discharge a one-rank bridge.  The accepted implementation uses a
  rank-local facts shard with only the six positive masks for rank `6000480`.
  ...
  ```text
  PositiveSurvivorSharedRank6000480CandidateFactsSmoke:
    passed
    elapsed: 10.52s
    peak process-tree RSS: 4634 MiB
    minimum MemAvailable seen: 45409 MiB

  CompactWalshBatchClassifierFactsSmoke:
    passed
    elapsed: 2.50s
  ...
  ```text
  scripts/generated/phase6z6k8ap16dn_rank6000480_candidate_facts_guard.json
  scripts/generated/phase6z6k8ap16dn_rank6000480_candidate_facts_guard.md
  scripts/generated/phase6z6k8ap16dn_compact_walsh_batch_classifier_facts_guard.json
  scripts/generated/phase6z6k8ap16dn_compact_walsh_batch_classifier_facts_guard.md
  ```

  Decision: accepted as the first nonempty AP16 all-Good coverage theorem using
  compact-Walsh positive-survivor membership plus source/row facts.  The next
  step should generalize this exact pattern across the remaining four AP16DJ
  ranks, but it must keep candidate-facts shards rank-local or otherwise
  bounded.  Larger global shared fact bundles are now known to be unsafe under
  the 5 GiB smoke cap.
- [x] Implement Phase 6Z.6K.8AP.16DO five-rank classifier-facts singleton
  smoke:
  AP16DO generalizes the AP16DN pattern across all five AP16DJ smoke ranks by
  generating rank-local source/row candidate-facts shards plus singleton
  classifier-facts coverage modules for ranks `6000745`, `6000625`, `6000480`,
  `6000720`, and `6000662`.

  New generator:

  ```text
  scripts/generate_ap16do_compact_walsh_classifier_facts.py
  ```

  New singleton theorem modules:
  ...
  ```text
  CompactWalshBatchClassifierSmoke:
    passed under 4800 MiB guard
    elapsed: 6.01s
    peak process-tree RSS: 4043.54 MiB

  CompactWalshBatchClassifierFactsRank6000745Smoke:
    passed under 5600 MiB guard
    elapsed: 2.50s
    peak process-tree RSS: 4094.50 MiB

  CompactWalshBatchClassifierFactsRank6000625Smoke:
    passed under 5600 MiB guard
    elapsed: 15.02s
    peak process-tree RSS: 5127.29 MiB

  CompactWalshBatchClassifierFactsRank6000480Smoke:
    passed under 5600 MiB guard
    elapsed: 2.50s
    peak process-tree RSS: 3773.48 MiB

  CompactWalshBatchClassifierFactsRank6000720Smoke:
    passed under 5600 MiB guard
    elapsed: 10.51s
    peak process-tree RSS: 4681.70 MiB

  CompactWalshBatchClassifierFactsRank6000662Smoke:
    passed under 5600 MiB guard
    elapsed: 16.03s
    peak process-tree RSS: 5175.27 MiB
  ```

  Reports:

  ```text
  scripts/generated/phase6z6k8ap16do_five_rank_classifier_facts_guard.md
  ...
  Decision: accepted as bounded singleton evidence.  The memory warning is now
  part of the plan: two singleton modules exceed the previous 5 GiB smoke cap,
  so the five-rank root theorem is intentionally deferred until we have a
  thinner erased export layer or an explicitly measured guarded root build.
  Do not import all five heavy singleton modules through an ordinary broad
  package build.
- [x] Implement Phase 6Z.6K.8AP.16DP sparse coverage core:
  AP16DP observes that the five AP16DO smoke ranks are sparse, not contiguous,
  so they cannot honestly be composed with `CoversInterval.concat` into a
  single interval without proving the gap ranks.  It adds:

  ```text
  Cuboctahedron/Generated/Coverage/Sparse.lean
  ```

  defining `CoversRanks` plus `nil`, `single`, `cons`, `append`, and `mono`
  combinators.  This module is intentionally separate from
  `Coverage.Interval.lean`, so sparse smoke helpers do not invalidate the
  interval-based generated coverage dependencies.  Focused validation:
  ...
  passed in 2.07s, with Lake reporting 165ms for the Lean target.

  Report:

  ```text
  scripts/generated/phase6z6k8ap16dp_sparse_coverage_core.md
  ```
- [x] Implement Phase 6Z.6K.8AP.16DQ sparse five-rank root:
  use `CoversRanks` to compose the five AP16DO singleton
  `AllTranslationGoodCoverage` theorems into a sparse smoke theorem over the
  rank list `[6000745, 6000625, 6000480, 6000720, 6000662]`.
  ...
  Result: passed in 3.00s with peak process-tree RSS 4103 MiB and minimum
  MemAvailable 46124 MiB.  This confirms the five-rank sparse smoke root is
  buildable in the focused cached path.  It remains smoke evidence only;
  production coverage still needs a real range/state membership layer.

  Report:

  ```text
  scripts/generated/phase6z6k8ap16dq_five_rank_sparse_root_guard.md
  scripts/generated/phase6z6k8ap16dq_five_rank_sparse_root_guard.json
  ```
- [x] Run Phase 6Z.6K.8AP.16DR translation production route audit:
  AP16DR reconciles the accepted AP16AO production producer-hierarchy
  projection with the AP16DO/AP16DQ guarded singleton evidence.

  New audit script:

  ```text
  scripts/audit_ap16dr_translation_production_route.py
  ```

  Command:

  ```text
  python3 scripts/audit_ap16dr_translation_production_route.py
  ```
  ...
  producer_hierarchy_within_budget: true
  singleton_route_decision: rejected-for-production
  max_singleton_classifier_facts_rss_mib: 5175.27
  next_phase: 6Z.6K.8AP.16DS
  ```

  The accepted production route remains the split source/row producer
  hierarchy from AP16AO:

  ```text
  projected descriptor families: 862
  projected source+row producers: 748
  projected chunks: 14
  projected source: 57,827 lines / 2,362 KiB
  projected checking: 1.17m wall / 58s CPU
  projected peak RSS: 3.93 GiB
  ```

  The AP16DO/AP16DQ singleton classifier-facts path is accepted only as
  bounded smoke evidence and explicitly rejected as the production compression
  coordinate.  It proves real semantic coverage for five ranks, but two
  singleton modules exceed the old 5 GiB smoke cap and the route still expands
  through rank-local candidate-facts shards.

  Reports:

  ```text
  scripts/generated/phase6z6k8ap16dr_translation_production_route_audit.json
  scripts/generated/phase6z6k8ap16dr_translation_production_route_audit.md
  ```
- [x] Run Phase 6Z.6K.8AP.16DS production-style membership slice audit:
  AP16DS checks whether the current repo already contains a nonempty bounded
  production-style membership theorem targeting
  `SourcePositionRowProducerGoodCoverageOnRange lo hi` or an equivalent
  all-Good semantic coverage target.  The audit deliberately distinguishes
  the accepted split source/row producer hierarchy from the rejected
  rank-local singleton classifier-facts route.

  New audit script:

  ```text
  scripts/audit_ap16ds_production_membership_slice.py
  ```

  Command:

  ```text
  python3 scripts/audit_ap16ds_production_membership_slice.py
  python3 -m py_compile scripts/audit_ap16ds_production_membership_slice.py
  ```
  ...
  ```text
  AP16I positive-survivor profile: accepted
  AP16DR producer hierarchy route: accepted and within budget
  split hierarchy conditional good-killed theorems: present
  split hierarchy nonempty coverage/complete export: absent
  singleton/sparse all-Good smoke: present but rejected as production coordinate
  ```

  This means AP16DS is not honestly complete as a production slice.  The next
  proof-producing step is AP16DT: emit a bounded
  `PositiveSurvivorBoolSignatureClassifierOnRange lo hi` (or equivalent)
  whose `completeBool` field proves the GoodDirection-to-candidate membership
  bridge over a meaningful range/state, then erase it to
  `SourcePositionRowProducerGoodCoverageOnRange lo hi` and
  `AllTranslationGoodCoverageOnRange lo hi`.

  Reports:

  ```text
  scripts/generated/phase6z6k8ap16ds_production_membership_slice_audit.json
  scripts/generated/phase6z6k8ap16ds_production_membership_slice_audit.md
  ```
- [x] Run Phase 6Z.6K.8AP.16DT positive-survivor classifier slice planner:
  AP16DT decides which AP16I catalog coordinate should be used by the first
  production-style classifier emitter.  It rejects direct survivor-signature
  anchoring because 725 of the 757 sampled signatures are single-rank and
  would recreate rank-local proof.  It accepts the shared positive candidate
  catalog as the next emitter coordinate: 195 candidate groups cover all 7,112
  profiled GoodDirection cases, the top 64 groups cover 6,454 of them, and the
  route emits zero bad-direction evidence.

  New planning script:

  ```text
  scripts/plan_ap16dt_positive_survivor_classifier_slice.py
  ```

  Command:

  ```text
  python3 scripts/plan_ap16dt_positive_survivor_classifier_slice.py
  python3 -m py_compile scripts/plan_ap16dt_positive_survivor_classifier_slice.py
  ```
  ...
  signature_route: rejected-rank-local-signature-anchor
  candidate_route: accepted-next-emitter-coordinate
  ```

  The next proof-producing step is AP16DU: emit the first bounded
  candidate-catalog classifier slice, targeting
  `PositiveSurvivorBoolClassifierOnRange lo hi` and erasing through
  `SourcePositionRowProducerGoodCoverageOnRange.to_allGoodCoverage`.

  Reports:

  ```text
  scripts/generated/phase6z6k8ap16dt_positive_survivor_classifier_slice_plan.json
  scripts/generated/phase6z6k8ap16dt_positive_survivor_classifier_slice_plan.md
  ```
- [x] Implement Phase 6Z.6K.8AP.16DU.0 bounded positive-survivor candidate
  ...
  ```text
  Cuboctahedron/Generated/Translation/TwoSource/SupportFamilies/PositiveSurvivorCandidateCatalogSurfaceSmoke.lean
  ```
  ...
  final coverage because the generated theorem still takes the
  catalog-completeness premise `hcomplete` explicitly:
  ...
  scripts/generated/phase6z6k8ap16du_candidate_catalog_surface_guard.md
  ```
- [x] Run Phase 6Z.6K.8AP.16DU.1 candidate-catalog completeness route audit:
  AP16DU.1 checks whether the obvious positive-survivor-signature bridge is a
  viable way to prove the AP16DU.0 catalog-completeness premise.  The audit is
  intentionally not proof evidence; it protects the proof plan from silently
  reverting to rank-local singleton anchors under a different name.

  New audit script:

  ```text
  scripts/audit_ap16du_candidate_completeness_route.py
  ```

  Commands:

  ```text
  python3 scripts/audit_ap16du_candidate_completeness_route.py
  python3 -m py_compile scripts/audit_ap16du_candidate_completeness_route.py
  ```
  ...
  The audit confirms the range-specific candidate catalog is coherent with the
  positive-survivor signature catalog, but it also rejects the signature route
  for the actual `hcomplete` proof: 444 of 464 signatures are singletons and
  would recreate the rank-local proof shape that AP16DT rejected.  The next
  proof-producing AP16DU step must derive candidate membership from a reusable
  source-position/state predicate or compact trace bridge, not from one
  signature/rank theorem per signature.

  Reports:

  ```text
  scripts/generated/phase6z6k8ap16du_candidate_completeness_route_audit.json
  scripts/generated/phase6z6k8ap16du_candidate_completeness_route_audit.md
  ```
- [x] Run Phase 6Z.6K.8AP.16DU.2 source-index/state classifier profile:
  AP16DU.2 reruns the older source-index/state computable-classifier profiler
  on the exact `[0,5000)` DU range after DU.1 rejected the singleton-heavy
  signature route.  This finds the right next proof coordinate: a
  GoodDirection-only classifier over the same 125 source-index/state families
  as the corrected candidate catalog, with no bad-direction branches and no
  per-rank signature anchors.

  Command:

  ```text
  python3 scripts/profile_source_index_state_computable_classifier.py --rank-start 0 --limit 5000 --max-rule-count 500 --top-limit 20 --json scripts/generated/phase6z6k8ap16du2_source_index_state_classifier_profile.json --md scripts/generated/phase6z6k8ap16du2_source_index_state_classifier_profile.md
  python3 -m py_compile scripts/profile_source_index_state_computable_classifier.py
  ```
  ...
  covered GoodDirection survivors: 4693
  not-GoodDirection masks avoided: 26475
  source-index/state families: 125
  largest family: 1237
  bounded replay obligations rejected: 35681
  template-only lower bound rejected: 9 templates too coarse
  ```

  This recovers the candidate-catalog family count while avoiding the DU.1
  signature pitfall.  The next Lean step should instantiate a nonempty
  source-index/state GoodDirection classifier/root for `[0,5000)`, not prove
  the DU.0 `hcomplete` premise by case-splitting ranks or masks.
...
- [x] Run Phase 6Z.6K.8AP.16DU.8 classifier membership route audit:
  ...
  ```
- [x] Implement Phase 6Z.6K.8AP.16DU.9A classifier-key membership bridge:
  DU.9A adds a public `ClassifierKey` index for the 125 bounded
  source-index/state classifier families and proves the key-erasure bridge:

  ```lean
  theorem classifierApplies_of_key_matches
      {key : ClassifierKey} {r : Nat} {mask : SignMask}
      (h : key.Matches r mask) :
      ClassifierApplies r mask
  ```

  It also retains the DU.7 Prop/Bool completion adapter, so the remaining
  bounded completeness theorem can now target the smaller existential
  ...
  rather than constructing `ClassifierApplies rank mask` directly.  The
  guarded focused build passed:

  ```text
  lake build Cuboctahedron.Generated.Translation.TwoSource.SupportFamilies.SourceIndexStateClassifierDU3Smoke
  exit=0, elapsed=5.00s, peak_tree_rss=4256 MiB, min_available=45842 MiB
  ```
...
Phase 6Z.6K.6 is implemented as a bounded, memory-safe diagnostic. It proves
the external-cache hierarchy can check and compose the `[0,1000)`
row-property quotient root without OOM, but it also proves that
...
Phase 6Z.6K.8A is implemented. It adds the generic membership-family Lean
interface and profiles candidate coordinates for `[0,1000)`. The best bounded
candidate is `source_kind_row_property`, with 11 families, but this is only a
...
Phase 6Z.6K.8AH is complete as a bounded pair-sign producer-coverage Lean
smoke. It added
`scripts/generate_pair_sign_producer_coverage_smoke.py` and generated
`Cuboctahedron/Generated/Translation/TwoSource/SupportFamilies/PairSignProducerCoverageSmoke.lean`.
The smoke covers the same 8AD/8AF/8AG representative windows by composing
reusable source producers and exact row producers through
`SourceIndexStateKey.goodKilled_of_source_row`, with no concrete rank/mask
member branches in the generated Lean module.

The generator command used memory-safe Python parallelism:

```bash
/usr/bin/time -v python3 scripts/generate_pair_sign_producer_coverage_smoke.py --jobs 4
```
...
source groups:                 191
row groups:                    21
represented GoodDirection:     7,112
wall time:                     3:13.63
CPU:                           257%
peak RSS:                      48,532 KB
exit status:                   0
...
```bash
/usr/bin/time -v lake build \
  Cuboctahedron.Generated.Translation.TwoSource.SupportFamilies.PairSignProducerCoverageSmoke
```
...
This is still bounded evidence, not global coverage. It validates the theorem
surface: final translation evidence can be phrased as source-language
membership plus exact row-producer membership, and Lean can compose those
facts cheaply into semantic `TranslationGoodCaseKilled` theorems.

Phase 6Z.6K.8AI is complete as the first broader producer-coverage scaling
profile. It added
`scripts/profile_pair_sign_producer_coverage_scaling.py`, which emits no Lean
and is not trusted as proof. It samples broader disjoint rank windows and
tracks cumulative descriptor, source-producer, row-producer, and source-row
edge growth.

Profiling command:

```bash
/usr/bin/time -v python3 scripts/profile_pair_sign_producer_coverage_scaling.py --jobs 4
```
...
sampled ranks:                  65,000
GoodDirection cases:            13,899
descriptor families:            328
source groups:                  294
row groups:                     24
source-row edges:               328
source+row producer obligations:318
source-language validation:     0 failures
wall time:                      7:35.67
CPU:                            227%
peak RSS:                       65,212 KB
exit status:                    0
...
The profile keeps the route alive: after expanding from the 8AH representative
windows to 65,000 sampled ranks, the source+row producer surface is still only
318 obligations, well below the 2,000-obligation diagnostic gate. Descriptor
families are 328, below the 5,000 descriptor gate. This remains diagnostic
evidence, not exhaustive coverage.

Phase 6Z.6K.8AJ is complete as resumable producer-census infrastructure. It
added `scripts/run_pair_sign_producer_coverage_census.py`, which emits no
Lean and is not trusted as proof. It writes atomic per-window checkpoints to
`/tmp/cuboctahedron_pair_sign_producer_census` by default and aggregates only
compact semantic keys/counts into tracked reports.

Checkpointed census command:

```bash
/usr/bin/time -v python3 scripts/run_pair_sign_producer_coverage_census.py --workers 4
```

Checkpointed census result:

```text
status:                         accepted-resumable-census-candidate
windows completed/requested:    10 / 10
sampled ranks:                  25,000
GoodDirection cases:            6,048
...
wall time:                      3:38.19
CPU:                            203%
max process RSS:                29,912 KB
max window RSS:                 29,528 KB
exit status:                    0
```

Aggregate-only replay also passed:

```bash
/usr/bin/time -v python3 scripts/run_pair_sign_producer_coverage_census.py \
  --workers 4 --aggregate-only
```
...
This proves the engineering path is resumable and compact: the checkpoint run
is not exhaustive coverage, but it can be safely extended without losing
finished windows or storing raw rank/mask member branches in tracked source.

Phase 6Z.6K.8AK is complete as a bounded scaling Lean smoke over the 8AJ
checkpoint-window surface. It generated
`Cuboctahedron/Generated/Translation/TwoSource/SupportFamilies/PairSignProducerCoverageScalingSmoke.lean`
using the same source/row producer theorem shape as 8AH, but on the 8AJ
window set.

Generation command:

```bash
/usr/bin/time -v python3 scripts/generate_pair_sign_producer_coverage_smoke.py \
  --jobs 4 \
  --phase 6Z.6K.8AK \
  ...
  --out Cuboctahedron/Generated/Translation/TwoSource/SupportFamilies/PairSignProducerCoverageScalingSmoke.lean \
  --json scripts/generated/phase6z6k8ak_pair_sign_producer_coverage_scaling_smoke.json \
  --md scripts/generated/phase6z6k8ak_pair_sign_producer_coverage_scaling_smoke.md \
...
```text
status:                         pair-sign-producer-coverage-smoke-emitted
represented GoodDirection:      6,048
descriptor families:            232
source groups:                  225
row groups:                     22
Lean source lines:              16,155
wall time:                      2:30.26
CPU:                            297%
peak RSS:                       44,772 KB
exit status:                    0
...
```bash
/usr/bin/time -v lake build \
  Cuboctahedron.Generated.Translation.TwoSource.SupportFamilies.PairSignProducerCoverageScalingSmoke
```
...
wall time:   0:13.72
CPU:         111%
peak RSS:    3,544,748 KB
exit status: 0
```

This confirms that the 8AJ checkpoint-window producer surface is not merely a
Python count: its source/row producer composition can be emitted as
theorem-valued Lean and kernel-checked with bounded memory. It remains a
bounded smoke, not exhaustive generated coverage.

Phase 6Z.6K.8AL is complete as the first split source/row/chunk/root layout
for the accepted pair-sign producer surface. It added
`scripts/generate_pair_sign_producer_coverage_split_smoke.py` and generated
`Cuboctahedron/Generated/Translation/TwoSource/SupportFamilies/PairSignProducerCoverageScalingSplit/`.
The split layout emits source producers once in `Source.lean`, row producers
once in `Row.lean`, descriptor-key coverage theorems in four bounded chunk
modules, and a tiny `All.lean` root.

Generation command:

```bash
/usr/bin/time -v python3 scripts/generate_pair_sign_producer_coverage_split_smoke.py \
  --jobs 4 --chunk-size 64
```
...
```bash
/usr/bin/time -v lake build \
  Cuboctahedron.Generated.Translation.TwoSource.SupportFamilies.PairSignProducerCoverageScalingSplit.All
```

Build result:

```text
status:      accepted-split-smoke
wall time:   0:10.50
CPU:         231%
peak RSS:    3,343,292 KB
exit status: 0
```

This validates the production-shaped import hierarchy for the current bounded
surface: source and row facts are shared, chunks are bounded, and the root is
small. It is still not exhaustive generated coverage.

Phase 6Z.6K.8AM is complete as a larger split-root/census gate over 20
disjoint 2,500-rank windows. It reuses the 8AL generator with a distinct
namespace/output directory:
...
/usr/bin/time -v python3 scripts/generate_pair_sign_producer_coverage_split_smoke.py \
  ...
  --out-dir Cuboctahedron/Generated/Translation/TwoSource/SupportFamilies/PairSignProducerCoverageScalingSplit50k \
  --json scripts/generated/phase6z6k8am_pair_sign_producer_coverage_split50k_smoke.json \
  --md scripts/generated/phase6z6k8am_pair_sign_producer_coverage_split50k_smoke.md \
  --namespace Cuboctahedron.Generated.Translation.TwoSource.SupportFamilies.PairSignProducerCoverageScalingSplit50k
```
...
```text
status:                         split-pair-sign-producer-coverage-smoke-emitted
sampled ranks:                  50,000
represented GoodDirection:      6,734
descriptor families:            315
source groups:                  298
row groups:                     22
chunk count:                    5
Lean source lines:              21,870
source directory size:          912 KiB
wall time:                      3:29.12
CPU:                            272%
peak RSS:                       43,724 KB
exit status:                    0
...
```bash
/usr/bin/time -v lake build \
  Cuboctahedron.Generated.Translation.TwoSource.SupportFamilies.PairSignProducerCoverageScalingSplit50k.All
```

Build result:

```text
status:      accepted-larger-split-smoke
wall time:   0:10.86
CPU:         276%
peak RSS:    3,299,724 KB
exit status: 0
```

The 8AM growth signal is favorable: doubling the number of checkpoint windows
from 10 to 20 increases descriptor families from 232 to 315 and source groups
from 225 to 298, while row groups remain at 22 and root build time/RSS stay
essentially flat. This is still bounded smoke evidence, not exhaustive
coverage.

Phase 6Z.6K.8AN is complete as a producer-growth projection gate. It first
extends the checkpointed census to 39 evenly spaced 2,500-rank windows using
top-level, memory-safe Python parallelism:
...
/usr/bin/time -v python3 scripts/run_pair_sign_producer_coverage_census.py \
  ...
  --json scripts/generated/phase6z6k8an_pair_sign_producer_coverage_census_2p5m.json \
  --md scripts/generated/phase6z6k8an_pair_sign_producer_coverage_census_2p5m.md
```
...
```text
status:                         accepted-resumable-census-candidate
windows completed:              39
sampled ranks:                  97,500
GoodDirection cases:            10,402
descriptor families:            401
source groups:                  374
row groups:                     22
source+row producer obligations:396
source-language failures:       0
wall time:                      3:56.94
CPU:                            397%
max window RSS:                 29,924 KiB
exit status:                    0
```

It then runs the growth projection over the checkpoint files:

```bash
python3 scripts/project_pair_sign_producer_growth.py \
  --checkpoint-dir /tmp/cuboctahedron_pair_sign_producer_census_8an_2p5m \
  --json scripts/generated/phase6z6k8an_pair_sign_producer_growth_projection.json \
  --md scripts/generated/phase6z6k8an_pair_sign_producer_growth_projection.md \
  --phase 6Z.6K.8AN \
  --chunk-size 64 \
  --producer-gate 2000 \
...
estimated descriptor chunks:    14
validation failures:            0
```

This remains diagnostic evidence, not proof. It is nevertheless the strongest
scaling signal for the translation branch so far: the projected source/row
producer surface remains below the 2,000-producer gate and the projected
descriptor surface remains below the 5,000-descriptor gate. The immediate next
step is Phase 6Z.6K.8AO.

Phase 6Z.6K.8AO is complete as a production split hierarchy design. It adds
`scripts/design_pair_sign_producer_hierarchy.py` and combines the 8AN growth
projection with the measured 8AM split-root Lean smoke.

Design command:

```bash
python3 scripts/design_pair_sign_producer_hierarchy.py
```
...
projected source+row producers: 748
projected descriptor chunks:    14
projected source lines:         57,827
projected source size:          2,362 KiB
projected CPU check budget:     58s
projected wall check budget:    70s at 4 jobs
projected peak RSS:             3.93 GiB
```

The proposed production layout is:

```text
Cuboctahedron/Generated/Translation/TwoSource/SupportFamilies/Production/
  Source.lean
  Row.lean
  Chunks/ChunkNNN.lean
  All.lean
  Membership/Core.lean
  Membership/All.lean
```

This remains diagnostic evidence, not proof. It accepts the source/row/chunk
hierarchy as small enough to emit once the membership theorem shape is ready.
The next step is full Phase 6Z.6K.8AP: implement generated producer
membership chunks and a full-range bridge.  The preferred endpoint is now the
classifier-free AP.2 route:
`AllTranslationGoodCoverageOnRange 0 numPairWords`, followed by
`AllTranslationGoodCoverageOnRange.to_caseKilled_of_fullRange`.  The older
`Cuboctahedron.Generated.Translation.KilledBridge` route remains available for
compatibility, but should not be the first production target because it carries
the old noncomputable early-classifier condition. That step should:

- keep using `SourceIndexStateKey.goodKilled_of_source_row` as the final
  semantic bridge for each covered producer case;
- prove or generate `SourceRowPredicateGoodBridgeOnRange lo hi` or
  `SourceRowFactsGoodBridgeOnRange lo hi` for each rank chunk, deriving a
  matching `SourceIndexStateSourcePredicate` and row predicate for arbitrary
  identity-linear `GoodDirectionAtRank` translation cases in that chunk;
- compose chunk theorems with `CoversInterval.concat` into a full-range root,
  then use `SourceRowPredicateGoodBridgeOnRange.to_allGoodCoverage` or the
  facts-bridge equivalent;
- finish by applying `AllTranslationGoodCoverageOnRange.to_caseKilled_of_fullRange`,
  so the public theorem target is `Coverage.TranslationCaseKilled`, not an
  ordinary `TranslationCert`;
- use memory-safe Python parallelism, starting at `--jobs 4`, for profiling
  and generation;
- keep source/row producer facts shared across chunk modules;
- avoid raw rank/mask replay, concrete per-rank row arithmetic, and giant
  `simp` searches over full source lists;
- keep generated files theorem-valued and private-data-minimal;
- run focused Lean builds first, with memory measurement, before any broader
  package build;
- reject the producer-coverage route if the larger/resumable profile diverges
  far from the observed producer surface or turns into member-list replay.

Phase 6Z.6K.8AP.0 is complete as the hand-written producer membership bridge
core. It adds
...
theorem SourceRowFactsBridge.to_killedBridge :
  SourceRowFactsBridge -> Cuboctahedron.Generated.Translation.KilledBridge
theorem SourceRowFactsBridge.of_rangeAll :
  SourceRowFactsBridgeOnRange 0 numPairWords -> SourceRowFactsBridge
theorem SourceRowFactsBridgeOnRange.to_residualCoverage :
  SourceRowFactsBridgeOnRange lo hi -> ResidualTranslationCoverageOnRange lo hi
theorem SourceRowFactsBridgeOnRange.to_residualGoodCoverage :
  SourceRowFactsBridgeOnRange lo hi -> ResidualTranslationGoodCoverageOnRange lo hi
theorem ResidualTranslationGoodCoverageOnRange.to_killedBridge_of_fullRange :
  ResidualTranslationGoodCoverageOnRange 0 numPairWords ->
    Cuboctahedron.Generated.Translation.KilledBridge
theorem SourceRowPredicateBridge.to_factsBridge :
  SourceRowPredicateBridge -> SourceRowFactsBridge
theorem SourceRowPredicateBridgeOnRange.to_factsBridgeOnRange :
  SourceRowPredicateBridgeOnRange lo hi -> SourceRowFactsBridgeOnRange lo hi
theorem SourceRowPredicateBridge.of_rangeAll :
  SourceRowPredicateBridgeOnRange 0 numPairWords -> SourceRowPredicateBridge
theorem SourceRowPredicateBridgeOnRange.to_residualCoverage :
  SourceRowPredicateBridgeOnRange lo hi -> ResidualTranslationCoverageOnRange lo hi
theorem SourceRowPredicateBridgeOnRange.to_residualGoodCoverage :
  SourceRowPredicateBridgeOnRange lo hi -> ResidualTranslationGoodCoverageOnRange lo hi
theorem SourceRowPredicateBridge.to_killedBridge :
  SourceRowPredicateBridge -> Cuboctahedron.Generated.Translation.KilledBridge
theorem SourceRowPredicateBridgeOnRange.to_killedBridge_of_fullRange :
  SourceRowPredicateBridgeOnRange 0 numPairWords ->
    Cuboctahedron.Generated.Translation.KilledBridge
...
theorem SourceRowFactsGoodBridgeOnRange.to_allGoodCoverage :
  SourceRowFactsGoodBridgeOnRange lo hi -> AllTranslationGoodCoverageOnRange lo hi
theorem SourceRowPredicateGoodBridgeOnRange.to_allGoodCoverage :
  SourceRowPredicateGoodBridgeOnRange lo hi -> AllTranslationGoodCoverageOnRange lo hi
theorem AllTranslationGoodCoverageOnRange.to_caseKilled_of_fullRange :
  AllTranslationGoodCoverageOnRange 0 numPairWords ->
    (r : Fin numPairWords) -> (mask : SignMask) -> TranslationCaseKilled r mask
```
...
wall time:   0:02.74
CPU:         60%
peak RSS:    853,684 KiB
exit status: 0
```

This is not global membership coverage. It proves that the remaining
production membership chunks only need to supply either:

- `SourceRowPredicateBridgeOnRange lo hi`, by deriving
  `SourceIndexStateSourcePredicate key.firstIndex key.secondIndex key.support r.val mask`
  plus `key.template.Rows key.support r.val mask` throughout the rank range; or
...
AP.2 supersedes the residual route as the preferred production target.  Range
chunks should export `SourceRowPredicateGoodBridgeOnRange lo hi` or
`SourceRowFactsGoodBridgeOnRange lo hi`; once a generated root covers
`[0, numPairWords)`, the full-range adapter yields ordinary
`TranslationCaseKilled` for each rank/mask.  This keeps the
feasibility-to-GoodDirection proof centralized in
`TranslationGoodCaseKilled.killed` and avoids the old noncomputable early
classifier in generated membership modules.

Phase 6Z.6K.8AP.1 adds the GoodDirection-survivor adapters and reports:

- `scripts/generated/phase6z6k8ap1_pair_sign_good_membership_bridge.json`
- `scripts/generated/phase6z6k8ap1_pair_sign_good_membership_bridge.md`

Phase 6Z.6K.8AP.2 adds the classifier-free GoodDirection bridge and reports:

- `scripts/generated/phase6z6k8ap2_classifier_free_good_bridge.json`
- `scripts/generated/phase6z6k8ap2_classifier_free_good_bridge.md`

The AP.2 route is now preferred.  It avoids generated dependence on
`translationEarlyFamilyClassOfChoice`; generated membership evidence should
prove `SourceRowPredicateGoodBridgeOnRange lo hi` or
`SourceRowFactsGoodBridgeOnRange lo hi` directly from `totalLinear = matId`
and `GoodDirectionAtRank`.  The full Phase 6Z.6K.8AP remains open until
generated membership evidence proves one of these AP.2 bridge predicates over
the full rank range.

Phase 6Z.6K.8AP.3 connects the classifier-free all-GoodDirection target to the
public generated coverage API and reports:

- `scripts/generated/phase6z6k8ap3_public_all_good_coverage_bridge.json`
- `scripts/generated/phase6z6k8ap3_public_all_good_coverage_bridge.md`

It adds `Translation.Coverage.AllGoodRankKilled`,
`Translation.Coverage.caseKilled_of_all_good_interval`,
`PublicAllGoodSemanticCoverageIntervals`, and
`semanticGeneratedCoverageOfAllGoodIntervals`.  The focused serial builds pass:
`lake build Cuboctahedron.Generated.Translation.Coverage.All` in 0.91s with
856,656 KiB peak RSS, and
`lake build Cuboctahedron.Generated.ExhaustiveCoverage` in 3.03s with
3,287,764 KiB peak RSS.  This is not the full generated evidence; it is the
small public adapter that will consume the full AP source/row membership root
once generated.

Phase 6Z.6K.8AP.4 adapts the bounded source-index/state coverage generator to
export the new AP interval theorems and reports:
...
It generates
`Cuboctahedron/Generated/Translation/TwoSource/SupportFamilies/SourceIndexStateBoundedAPSmoke/`
and proves both `AllTranslationGoodCoverageOnRange 0 16` and
`CoversInterval Cuboctahedron.Generated.Translation.Coverage.AllGoodRankKilled 0 16`.
The focused build passes:
`lake build Cuboctahedron.Generated.Translation.TwoSource.SupportFamilies.SourceIndexStateBoundedAPSmoke.All`
in 288.66s with 14,041,472 KiB peak RSS.  This validates the AP theorem
surface, but it rejects finite rank/mask dispatch as a production strategy:
...
Phase 6Z.6K.8AP.8 adds that source/row language theorem surface and reports:

- `scripts/generated/phase6z6k8ap8_source_row_language_bridge.json`
- `scripts/generated/phase6z6k8ap8_source_row_language_bridge.md`
- `Cuboctahedron/Generated/Translation/TwoSource/SupportFamilies/SourceRowLanguageSmoke.lean`

The hand-written AP bridge now defines:

```lean
structure SourceRowFactsGoodLanguageOnRange (lo hi : Nat)
structure SourceRowPredicateGoodLanguageOnRange (lo hi : Nat)
```

and erases either language surface to the existing AP targets:
...
passes in 1.01s with 852,772 KiB peak RSS on a warm build.  This is accepted
only as bridge/API infrastructure: the smoke covers `[0,0)`, and the full AP
proof still needs a nonempty generated language membership theorem over
meaningful rank ranges without rank/mask replay.

Phase 6Z.6K.8AP.9 factors the source-position half of that nonempty language
proof into reusable Lean definitions and reports:

- `scripts/generated/phase6z6k8ap9_source_position_spec_bridge.json`
- `scripts/generated/phase6z6k8ap9_source_position_spec_bridge.md`

The new theorem surface in
`Cuboctahedron/Generated/Translation/TwoSource/SupportFamilies/SourcePositionLanguage.lean`
is:
...
`SourcePositionLanguageSmoke` now checks both a dynamic interior source pattern
and a static `xpStart` plus ordering pattern through the generic spec surface:

```lean
theorem generic_source_000_sourceFacts :
    generic_source_000_spec.Predicate r mask ->
      SourceIndexStateSourceFacts generic_source_000_key r mask

theorem generic_source_004_sourceFacts :
    generic_source_004_spec.Predicate r mask ->
...
passes in 12.80s with 3,649,176 KiB peak RSS.  The first attempt failed
because `SourceChecks` was not opened from `RowRelationTemplates`; after
patching the namespace import, the same focused build passed.  This is accepted
as a source-language building block, not as complete AP coverage.  The next
step is to combine `SourcePairPositionSpec.Predicate` with row producers and a
nonempty `complete` theorem for meaningful rank ranges.

Phase 6Z.6K.8AP.10 adds a producer-level version of the AP language bridge and
reports:
...
The new hand-written bridge surface is:
...
This bridge is closer to the accepted split producer hierarchy than AP.8:
generated modules can define source and row producers privately, prove that
every identity-linear `GoodDirectionAtRank` survivor in a range belongs to one
producer pair and key, and then erase the producer-level language to the
public all-Good coverage theorem.  The focused validation command

```text
/usr/bin/time -v lake build Cuboctahedron.Generated.Translation.TwoSource.SupportFamilies.SourceRowProducerLanguageSmoke
```

passes in 2.08s with 3,300,380 KiB peak RSS.  Two small declaration-shape
failures were fixed: structure-valued adapters must be `def`s, not `theorem`s.
This is accepted as bridge/API infrastructure only.  It still covers `[0,0)`.
The next AP target is a nonempty producer-language `complete` theorem over a
meaningful range, using `SourcePairPositionSpec.Predicate` for the source side
and row producer `Applies` facts for the row side, without finite rank/mask
replay.

Phase 6Z.6K.8AP.11 adds the source-position plus row-producer version of that
language bridge and reports:
...
This bridge moves the future generated obligation one step closer to the
accepted source-position infrastructure: generated chunks can prove a
`Language spec rowProducer key rank mask` predicate, together with key/index
alignment, `spec.Predicate rank mask`, and `rowProducer.Applies key rank mask`.
...
passes in 8.41s with 3,302,808 KiB peak RSS.  This is still an empty-range API
...
Phase 6Z.6K.8AP.16A adds facts/predicate/language erasure into the descriptor
coverage target:
...
passes in 8.68s with 3,296,324 KiB peak RSS.  This is also bridge
infrastructure only.  It means AP.16 may target either direct descriptor
coverage or the older source/row facts and language surfaces; the direct
descriptor coverage theorem follows without any generated rank/mask data.

Phase 6Z.6K.8AP.16B reruns the descriptor-membership state profiler:
...
It completes in 194.14s with 50,176 KiB peak RSS.  The diagnostic scans seven
representative windows and 7,112 GoodDirection cases.  Source-language
validation failures are zero.  The best producer route is:

```text
source_language_full + row_exact
```

with 212 projected source+row producer obligations.  The best descriptor route
is `descriptor_source_language_row_exact` with 195 obligations, but the
producer route matches the existing `SourceRowProducerGoodLanguageOnRange`
surface and erases through AP.16A.  This is accepted as the AP.16B diagnostic
target only.  The next Lean-producing step is a bounded emitter over the same
representative windows proving a nonempty
`SourceRowProducerGoodLanguageOnRange lo hi` root without the rejected
`interval_cases r` plus `fin_cases mask` replay.

Phase 6Z.6K.8AP.16C adds the source-position producer language to the direct
descriptor erasure path:
...
passes in 10.56s with 3,308,884 KiB peak RSS.  This closes a glue gap between
the AP.11 source-position language surface and the AP.15/AP.16 descriptor
coverage target.  It is still not nonempty generated membership coverage: the
next emitter must prove the `complete` field of a
`SourcePositionRowProducerGoodLanguageOnRange lo hi` or direct descriptor
coverage theorem for meaningful ranges from arbitrary identity-linear
`GoodDirectionAtRank` cases.

Phase 6Z.6K.8AP.16D adds the direct source-position coverage target:
...
This target is narrower than the full language structure: generated chunks only
need to prove the existential source-position spec, row producer, key, source
predicate, and row-producer application for each arbitrary identity-linear
GoodDirection survivor.  Generic adapters then package that into the existing
source-position language and erase it to all-Good and descriptor coverage.
...
pass in 8.27s / 3,296,632 KiB peak RSS and 3.26s / 3,328,084 KiB peak RSS,
respectively.  This closes another glue gap, not the AP.16 membership gap.  The
next Lean-producing step should target a nonempty
...
- Do not run AP16DF-style roots as ordinary broad `lake build` experiments.
  They are allowed only through the AP16DI serial guard, which prebuilds
