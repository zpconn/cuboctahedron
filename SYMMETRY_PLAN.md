...
- [x] Run Phase 6Z.6K.8AP.16DR translation production route audit:
  ...
  ```
- [ ] Implement Phase 6Z.6K.8AP.16DS production-style membership slice:
  emit and check a nonempty bounded range/state theorem targeting
  `SourcePositionRowProducerGoodCoverageOnRange lo hi` or an equivalent
  all-Good semantic coverage target.  The slice must import shared source
  producer and row producer modules plus bounded membership chunks, not
  rank-local singleton candidate-facts shards.  Acceptance requires a focused
  guarded Lean build below 6500 MiB RSS, nonempty GoodDirection coverage over a
  meaningful bounded range, and an updated projection still within the
  5-6 wall-hour / 24 CPU-hour generated-checking budget.
