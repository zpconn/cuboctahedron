...
- [x] Implement Phase 6Z.6K.8AP.16DO five-rank classifier-facts singleton
  ...
  package build.
- [ ] Implement Phase 6Z.6K.8AP.16DP thin five-rank export/root:
  add a lighter export boundary over the AP16DO singleton theorems, or measure a
  five-rank root under an explicit guard, so a bounded root can compose the five
  singleton `AllTranslationGoodCoverage` results without pulling every
  rank-local candidate-facts proof into normal generated builds.
