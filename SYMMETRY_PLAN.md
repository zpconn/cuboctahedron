...
6Z.6K.8AP.3 public all-Good coverage bridge.
...
- [ ] Implement Phase 6Z.6K.8AP producer membership bridge:
  define generated membership chunks that turn arbitrary identity-linear
  `GoodDirectionAtRank` translation cases in each rank range into matching
  source predicates and row predicates, preferably export
  `SourceRowPredicateGoodBridgeOnRange lo hi` or
  `SourceRowFactsGoodBridgeOnRange lo hi`, compose those chunks with
  `CoversInterval.concat`, then use the AP.2 full-range adapter to derive
  ordinary `TranslationCaseKilled` without raw certificates.
