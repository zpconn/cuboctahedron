...
- [ ] Implement Phase 6Z.6K.8AP producer membership bridge:
  define the generated membership root and Lean theorem shape that turns an
  arbitrary `needsFarkas`/GoodDirection residual translation case into a
  matching source predicate and row predicate, then applies the generated
  source/row producer theorem and `TranslationGoodCaseKilled.killed` to
  satisfy `Cuboctahedron.Generated.Translation.KilledBridge`.
