...
namespace WeightedDenomCubeDU9IQDirectBridgeGeneratedIdx40Smoke
...
def cubexx1110 : MaskSubcube where
  fixed
...
def poly903_8 : WalshQuadratic where
  c := 8
  y := 0
  z := 0
  d111 := -28/9
...
private def reduced903_8 : WalshQuadratic where
  c := -260/27
  y := 20/9
  z := 52/9
  d111 := 0
...
private theorem poly903_8_eq_reduced_on_cube
    {mask : SignMask} (hmask : cubexx1110.Member mask) :
    poly903_8.coeffEval mask = reduced903_8.coeffEval mask := by
  have hd111 : maskBitForPair mask SignBit.d111.toPairId = true := by
    simpa [cubexx1110, MaskSubcube.Member] using hmask SignBit.d111
  have hd11m : maskBitForPair mask SignBit.d11m.toPairId = true := by
    simpa [cubexx1110, MaskSubcube.Member] using hmask SignBit.d11m
  have hd1m1 : maskBitForPair mask SignBit.d1m1.toPairId = true := by
    simpa [cubexx1110, MaskSubcube.Member] using hmask SignBit.d1m1
  have hdm11 : maskBitForPair mask SignBit.dm11.toPairId = false := by
    simpa [cubexx1110, MaskSubcube.Member] using hmask SignBit.dm11
  by_cases hy : maskBitForPair mask SignBit.y.toPairId
  all_goals
    by_cases hz : maskBitForPair mask SignBit.z.toPairId
    all_goals
      simp [poly903_8, reduced903_8, WalshQuadratic.coeffEval, SignBit.value, hd111, hd11m, hd1m1, hdm11, hy, hz]
      norm_num

private def reducedBound903_8 :
    WalshQuadraticReducedAbsBound cubexx1110 poly903_8 where
  reduced := reduced903_8
  eq_on_cube := by
    intro mask hmask
    exact poly903_8_eq_reduced_on_cube hmask
  abs_bound_nonpos := by
    norm_num [reduced903_8, WalshQuadratic.absBoundSum]

private def cubeBound903_8 :
    WalshQuadraticCubeBound cubexx1110 poly903_8 :=
  WalshQuadraticCubeBound.leaf reducedBound903_8

private theorem poly903_8_nonpos
    {mask : SignMask} (hmask : cubexx1110.Member mask) :
    poly903_8.coeffEval mask <= 0 :=
  cubeBound903_8.coeffEval_nonpos hmask

def reducedObstruction903_8
    (direct_eq :
      forall {mask : SignMask} (hlt : rank903 < numPairWords),
        cubexx1110.Member mask ->
          weightedDirectWalshDotAtRank
            (⟨rank903, hlt⟩ : Fin numPairWords) mask weights8 =
            poly903_8.coeffEval mask) :
    WeightedWalshQuadraticNonposObstruction rank903 cubexx1110.Member where
  weights := weights8
  ...
  poly_nonpos := by
    intro mask hmember
    exact poly903_8_nonpos hmember
...
theorem weightedDenom_nonpos_of_reduced_bound
    ...
    (hmember : cubexx1110.Member mask) :
    weightedDenomAtRank
      (⟨rank903, hlt⟩ : Fin numPairWords) mask weights8 <= 0 :=
  (reducedObstruction903_8 direct_eq).nonpos hlt hmember
...
private theorem dot_sum_eval
    ...
    dot07.coeffEval mask =
      poly903_8.coeffEval mask := by
  simp [dot07, poly903_8,
    WalshQuadratic.coeffEval]
  try ring_nf

theorem direct_eq
    {mask : SignMask} (hlt : rank903 < numPairWords)
    (_hmember : cubexx1110.Member mask) :
    weightedDirectWalshDotAtRank
        (⟨rank903, hlt⟩ : Fin numPairWords) mask weights8 =
      poly903_8.coeffEval mask := by
...
end WeightedDenomCubeDU9IQDirectBridgeGeneratedIdx40Smoke
