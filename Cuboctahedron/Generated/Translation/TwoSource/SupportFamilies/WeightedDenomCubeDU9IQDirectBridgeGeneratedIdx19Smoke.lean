...
namespace WeightedDenomCubeDU9IQDirectBridgeGeneratedIdx19Smoke
...
def cube1x010x : MaskSubcube where
  fixed
    | SignBit.y => some true
...
private theorem poly897_11_eq_reduced_on_cube
    {mask : SignMask} (hmask : cube1x010x.Member mask) :
    poly897_11.coeffEval mask = reduced897_11.coeffEval mask := by
  have hy : maskBitForPair mask SignBit.y.toPairId = true := by
    simpa [cube1x010x, MaskSubcube.Member] using hmask SignBit.y
  have hd111 : maskBitForPair mask SignBit.d111.toPairId = false := by
    simpa [cube1x010x, MaskSubcube.Member] using hmask SignBit.d111
  have hd11m : maskBitForPair mask SignBit.d11m.toPairId = true := by
    simpa [cube1x010x, MaskSubcube.Member] using hmask SignBit.d11m
  have hd1m1 : maskBitForPair mask SignBit.d1m1.toPairId = false := by
    simpa [cube1x010x, MaskSubcube.Member] using hmask SignBit.d1m1
  by_cases hz : maskBitForPair mask SignBit.z.toPairId
  all_goals
    by_cases hdm11 : maskBitForPair mask SignBit.dm11.toPairId
    all_goals
      simp [poly897_11, reduced897_11, WalshQuadratic.coeffEval, SignBit.value, hy, hd111, hd11m, hd1m1, hz, hdm11]
      norm_num

private def reducedBound897_11 :
    WalshQuadraticReducedAbsBound cube1x010x poly897_11 where
...
private def cubeBound897_11 :
    WalshQuadraticCubeBound cube1x010x poly897_11 :=
...
theorem weightedDenom_nonpos_of_reduced_bound
    ...
    (hmember : cube1x010x.Member mask) :
    weightedDenomAtRank
...
theorem direct_eq
    {mask : SignMask} (hlt : rank897 < numPairWords)
    (_hmember : cube1x010x.Member mask) :
    weightedDirectWalshDotAtRank
        (⟨rank897, hlt⟩ : Fin numPairWords) mask weights11 =
      poly897_11.coeffEval mask := by
...
end WeightedDenomCubeDU9IQDirectBridgeGeneratedIdx19Smoke
