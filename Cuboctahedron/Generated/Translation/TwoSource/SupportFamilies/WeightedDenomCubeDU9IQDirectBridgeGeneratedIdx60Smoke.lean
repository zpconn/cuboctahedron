...
namespace WeightedDenomCubeDU9IQDirectBridgeGeneratedIdx60Smoke
...
def cube111xx0 : MaskSubcube where
  fixed
    | SignBit.y => some true
    | SignBit.z => some true
...
def poly905_2_11 : WalshQuadratic where
  c := 12
  y := 0
  z := 0
  d111 := 0
...
private def reduced905_2_11 : WalshQuadratic where
  c := -56/27
  y := 0
  z := 0
  d111 := 0
...
private theorem poly905_2_11_eq_reduced_on_cube
    {mask : SignMask} (hmask : cube111xx0.Member mask) :
    poly905_2_11.coeffEval mask = reduced905_2_11.coeffEval mask := by
  have hy : maskBitForPair mask SignBit.y.toPairId = true := by
    simpa [cube111xx0, MaskSubcube.Member] using hmask SignBit.y
  have hz : maskBitForPair mask SignBit.z.toPairId = true := by
    simpa [cube111xx0, MaskSubcube.Member] using hmask SignBit.z
  have hd111 : maskBitForPair mask SignBit.d111.toPairId = true := by
    simpa [cube111xx0, MaskSubcube.Member] using hmask SignBit.d111
  have hdm11 : maskBitForPair mask SignBit.dm11.toPairId = false := by
    simpa [cube111xx0, MaskSubcube.Member] using hmask SignBit.dm11
  by_cases hd11m : maskBitForPair mask SignBit.d11m.toPairId
  all_goals
    by_cases hd1m1 : maskBitForPair mask SignBit.d1m1.toPairId
    all_goals
      simp [poly905_2_11, reduced905_2_11, WalshQuadratic.coeffEval, SignBit.value, hy, hz, hd111, hdm11, hd11m, hd1m1]
      norm_num

private def reducedBound905_2_11 :
    WalshQuadraticReducedAbsBound cube111xx0 poly905_2_11 where
  reduced := reduced905_2_11
  eq_on_cube := by
    intro mask hmask
    exact poly905_2_11_eq_reduced_on_cube hmask
  abs_bound_nonpos := by
    norm_num [reduced905_2_11, WalshQuadratic.absBoundSum]

private def cubeBound905_2_11 :
    WalshQuadraticCubeBound cube111xx0 poly905_2_11 :=
  WalshQuadraticCubeBound.leaf reducedBound905_2_11

private theorem poly905_2_11_nonpos
    {mask : SignMask} (hmask : cube111xx0.Member mask) :
    poly905_2_11.coeffEval mask <= 0 :=
  cubeBound905_2_11.coeffEval_nonpos hmask

def reducedObstruction905_2_11
    (direct_eq :
      forall {mask : SignMask} (hlt : rank905 < numPairWords),
        cube111xx0.Member mask ->
          weightedDirectWalshDotAtRank
            (⟨rank905, hlt⟩ : Fin numPairWords) mask weights2_11 =
            poly905_2_11.coeffEval mask) :
    WeightedWalshQuadraticNonposObstruction rank905 cube111xx0.Member where
  weights := weights2_11
  ...
  poly_nonpos := by
    intro mask hmember
    exact poly905_2_11_nonpos hmember
...
theorem weightedDenom_nonpos_of_reduced_bound
    ...
    (hmember : cube111xx0.Member mask) :
    weightedDenomAtRank
      (⟨rank905, hlt⟩ : Fin numPairWords) mask weights2_11 <= 0 :=
  (reducedObstruction905_2_11 direct_eq).nonpos hlt hmember
...
private theorem dot_sum_eval
    (mask : SignMask) :
    dot01.coeffEval mask + dot10.coeffEval mask =
      poly905_2_11.coeffEval mask := by
  simp [dot01, dot10, poly905_2_11,
    WalshQuadratic.coeffEval]
  try ring_nf

theorem direct_eq
    {mask : SignMask} (hlt : rank905 < numPairWords)
    (_hmember : cube111xx0.Member mask) :
    weightedDirectWalshDotAtRank
        (⟨rank905, hlt⟩ : Fin numPairWords) mask weights2_11 =
      poly905_2_11.coeffEval mask := by
...
theorem notGoodDirection
  ...
  have hpos :
      0 <
        weightedDenomAtRank
          (⟨rank905, hlt⟩ : Fin numPairWords) mask weights2_11 :=
    weightedDenomAtRank_pos_of_goodDirection hlt weights2_11_nonnegative
      weights2_11_positive hgood
...
end WeightedDenomCubeDU9IQDirectBridgeGeneratedIdx60Smoke
