...
namespace WeightedDenomCubeDU9IQDirectBridgeGeneratedIdx05Smoke
...
def cube0x0x00 : MaskSubcube where
  fixed
...
private theorem poly896_11_eq_reduced_on_cube
    {mask : SignMask} (hmask : cube0x0x00.Member mask) :
    poly896_11.coeffEval mask = reduced896_11.coeffEval mask := by
  have hy : maskBitForPair mask SignBit.y.toPairId = false := by
    simpa [cube0x0x00, MaskSubcube.Member] using hmask SignBit.y
  have hd111 : maskBitForPair mask SignBit.d111.toPairId = false := by
    simpa [cube0x0x00, MaskSubcube.Member] using hmask SignBit.d111
  have hd1m1 : maskBitForPair mask SignBit.d1m1.toPairId = false := by
    simpa [cube0x0x00, MaskSubcube.Member] using hmask SignBit.d1m1
  have hdm11 : maskBitForPair mask SignBit.dm11.toPairId = false := by
    simpa [cube0x0x00, MaskSubcube.Member] using hmask SignBit.dm11
  by_cases hz : maskBitForPair mask SignBit.z.toPairId
  all_goals
    by_cases hd11m : maskBitForPair mask SignBit.d11m.toPairId
    all_goals
      simp [poly896_11, reduced896_11, WalshQuadratic.coeffEval, SignBit.value, hy, hd111, hd1m1, hdm11, hz, hd11m]
      norm_num

private def reducedBound896_11 :
    WalshQuadraticReducedAbsBound cube0x0x00 poly896_11 where
...
private def cubeBound896_11 :
    WalshQuadraticCubeBound cube0x0x00 poly896_11 :=
...
theorem weightedDenom_nonpos_of_reduced_bound
    ...
    (hmember : cube0x0x00.Member mask) :
    weightedDenomAtRank
...
theorem direct_eq
    {mask : SignMask} (hlt : rank896 < numPairWords)
    (_hmember : cube0x0x00.Member mask) :
    weightedDirectWalshDotAtRank
        (⟨rank896, hlt⟩ : Fin numPairWords) mask weights11 =
      poly896_11.coeffEval mask := by
...
end WeightedDenomCubeDU9IQDirectBridgeGeneratedIdx05Smoke
