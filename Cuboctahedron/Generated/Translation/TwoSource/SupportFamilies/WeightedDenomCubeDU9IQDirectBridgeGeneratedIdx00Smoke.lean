...
namespace WeightedDenomCubeDU9IQDirectBridgeGeneratedIdx00Smoke
...
def cubexx00x1 : MaskSubcube where
  fixed
...
def poly896_1 : WalshQuadratic where
  c := 4
  y := 0
  z := 0
  d111 := 8/3
...
private def reduced896_1 : WalshQuadratic where
  c := -4
  y := 0
  z := 0
  d111 := 0
...
private theorem poly896_1_eq_reduced_on_cube
    {mask : SignMask} (hmask : cubexx00x1.Member mask) :
    poly896_1.coeffEval mask = reduced896_1.coeffEval mask := by
  have hd111 : maskBitForPair mask SignBit.d111.toPairId = false := by
    simpa [cubexx00x1, MaskSubcube.Member] using hmask SignBit.d111
  have hd11m : maskBitForPair mask SignBit.d11m.toPairId = false := by
    simpa [cubexx00x1, MaskSubcube.Member] using hmask SignBit.d11m
  have hdm11 : maskBitForPair mask SignBit.dm11.toPairId = true := by
    simpa [cubexx00x1, MaskSubcube.Member] using hmask SignBit.dm11
  ...
  all_goals
    ...
    all_goals
      ...
      all_goals
        simp [poly896_1, reduced896_1, WalshQuadratic.coeffEval, SignBit.value, hd111, hd11m, hdm11, hy, hz, hd1m1]
        norm_num

private def reducedBound896_1 :
    WalshQuadraticReducedAbsBound cubexx00x1 poly896_1 where
  reduced := reduced896_1
  eq_on_cube := by
    intro mask hmask
    exact poly896_1_eq_reduced_on_cube hmask
  abs_bound_nonpos := by
    norm_num [reduced896_1, WalshQuadratic.absBoundSum]

private def cubeBound896_1 :
    WalshQuadraticCubeBound cubexx00x1 poly896_1 :=
  WalshQuadraticCubeBound.leaf reducedBound896_1

private theorem poly896_1_nonpos
    {mask : SignMask} (hmask : cubexx00x1.Member mask) :
    poly896_1.coeffEval mask <= 0 :=
  cubeBound896_1.coeffEval_nonpos hmask

def reducedObstruction896_1
    (direct_eq :
      forall {mask : SignMask} (hlt : rank896 < numPairWords),
        cubexx00x1.Member mask ->
          weightedDirectWalshDotAtRank
            (⟨rank896, hlt⟩ : Fin numPairWords) mask weights1 =
            poly896_1.coeffEval mask) :
    WeightedWalshQuadraticNonposObstruction rank896 cubexx00x1.Member where
  weights := weights1
  ...
  poly_nonpos := by
    intro mask hmember
    exact poly896_1_nonpos hmember
...
theorem weightedDenom_nonpos_of_reduced_bound
    ...
    (hmember : cubexx00x1.Member mask) :
    weightedDenomAtRank
      (⟨rank896, hlt⟩ : Fin numPairWords) mask weights1 <= 0 :=
  (reducedObstruction896_1 direct_eq).nonpos hlt hmember
...
private theorem dot_sum_eval
    ...
    dot00.coeffEval mask =
      poly896_1.coeffEval mask := by
  simp [dot00, poly896_1,
    WalshQuadratic.coeffEval]
  try ring_nf

theorem direct_eq
    {mask : SignMask} (hlt : rank896 < numPairWords)
    (_hmember : cubexx00x1.Member mask) :
    weightedDirectWalshDotAtRank
        (⟨rank896, hlt⟩ : Fin numPairWords) mask weights1 =
      poly896_1.coeffEval mask := by
...
end WeightedDenomCubeDU9IQDirectBridgeGeneratedIdx00Smoke
