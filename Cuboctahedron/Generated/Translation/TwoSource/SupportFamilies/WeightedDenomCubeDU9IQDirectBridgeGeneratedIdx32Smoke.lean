...
namespace WeightedDenomCubeDU9IQDirectBridgeGeneratedIdx32Smoke
...
def cubex010xx : MaskSubcube where
  fixed
...
def poly899_4_6 : WalshQuadratic where
  c := 12
  y := 0
  z := 0
  d111 := 0
...
private def reduced899_4_6 : WalshQuadratic where
  c := -4
  y := -4
  z := 0
  d111 := 0
...
private theorem poly899_4_6_eq_reduced_on_cube
    {mask : SignMask} (hmask : cubex010xx.Member mask) :
    poly899_4_6.coeffEval mask = reduced899_4_6.coeffEval mask := by
  have hz : maskBitForPair mask SignBit.z.toPairId = false := by
    simpa [cubex010xx, MaskSubcube.Member] using hmask SignBit.z
  have hd111 : maskBitForPair mask SignBit.d111.toPairId = true := by
    simpa [cubex010xx, MaskSubcube.Member] using hmask SignBit.d111
  have hd11m : maskBitForPair mask SignBit.d11m.toPairId = false := by
    simpa [cubex010xx, MaskSubcube.Member] using hmask SignBit.d11m
  ...
  all_goals
    ...
    all_goals
      ...
      all_goals
        simp [poly899_4_6, reduced899_4_6, WalshQuadratic.coeffEval, SignBit.value, hz, hd111, hd11m, hy, hd1m1, hdm11]
        norm_num

private def reducedBound899_4_6 :
    WalshQuadraticReducedAbsBound cubex010xx poly899_4_6 where
  reduced := reduced899_4_6
  eq_on_cube := by
    intro mask hmask
    exact poly899_4_6_eq_reduced_on_cube hmask
  abs_bound_nonpos := by
    norm_num [reduced899_4_6, WalshQuadratic.absBoundSum]

private def cubeBound899_4_6 :
    WalshQuadraticCubeBound cubex010xx poly899_4_6 :=
  WalshQuadraticCubeBound.leaf reducedBound899_4_6

private theorem poly899_4_6_nonpos
    {mask : SignMask} (hmask : cubex010xx.Member mask) :
    poly899_4_6.coeffEval mask <= 0 :=
  cubeBound899_4_6.coeffEval_nonpos hmask

def reducedObstruction899_4_6
    (direct_eq :
      forall {mask : SignMask} (hlt : rank899 < numPairWords),
        cubex010xx.Member mask ->
          weightedDirectWalshDotAtRank
            (⟨rank899, hlt⟩ : Fin numPairWords) mask weights4_6 =
            poly899_4_6.coeffEval mask) :
    WeightedWalshQuadraticNonposObstruction rank899 cubex010xx.Member where
  weights := weights4_6
  ...
  poly_nonpos := by
    intro mask hmember
    exact poly899_4_6_nonpos hmember
...
theorem weightedDenom_nonpos_of_reduced_bound
    ...
    (hmember : cubex010xx.Member mask) :
    weightedDenomAtRank
      (⟨rank899, hlt⟩ : Fin numPairWords) mask weights4_6 <= 0 :=
  (reducedObstruction899_4_6 direct_eq).nonpos hlt hmember
...
private theorem dot_sum_eval
    ...
    dot03.coeffEval mask + dot05.coeffEval mask =
      poly899_4_6.coeffEval mask := by
  simp [dot03, dot05, poly899_4_6,
    WalshQuadratic.coeffEval]
  try ring_nf

theorem direct_eq
    {mask : SignMask} (hlt : rank899 < numPairWords)
    (_hmember : cubex010xx.Member mask) :
    weightedDirectWalshDotAtRank
        (⟨rank899, hlt⟩ : Fin numPairWords) mask weights4_6 =
      poly899_4_6.coeffEval mask := by
...
end WeightedDenomCubeDU9IQDirectBridgeGeneratedIdx32Smoke
