...
namespace WeightedDenomCubeDU9IQDirectBridgeGeneratedIdx56Smoke
...
def cubexx1x01 : MaskSubcube where
  fixed
...
def poly905_1 : WalshQuadratic where
  c := 4
  y := 0
  z := 0
  d111 := -56/27
...
private def reduced905_1 : WalshQuadratic where
  c := -92/27
  y := 0
  z := 0
  d111 := 0
...
private theorem poly905_1_eq_reduced_on_cube
    {mask : SignMask} (hmask : cubexx1x01.Member mask) :
    poly905_1.coeffEval mask = reduced905_1.coeffEval mask := by
  have hd111 : maskBitForPair mask SignBit.d111.toPairId = true := by
    simpa [cubexx1x01, MaskSubcube.Member] using hmask SignBit.d111
  have hd1m1 : maskBitForPair mask SignBit.d1m1.toPairId = false := by
    simpa [cubexx1x01, MaskSubcube.Member] using hmask SignBit.d1m1
  have hdm11 : maskBitForPair mask SignBit.dm11.toPairId = true := by
    simpa [cubexx1x01, MaskSubcube.Member] using hmask SignBit.dm11
  ...
  all_goals
    ...
    all_goals
      ...
      all_goals
        simp [poly905_1, reduced905_1, WalshQuadratic.coeffEval, SignBit.value, hd111, hd1m1, hdm11, hy, hz, hd11m]
        norm_num

private def reducedBound905_1 :
    WalshQuadraticReducedAbsBound cubexx1x01 poly905_1 where
  reduced := reduced905_1
  eq_on_cube := by
    intro mask hmask
    exact poly905_1_eq_reduced_on_cube hmask
  abs_bound_nonpos := by
    norm_num [reduced905_1, WalshQuadratic.absBoundSum]

private def cubeBound905_1 :
    WalshQuadraticCubeBound cubexx1x01 poly905_1 :=
  WalshQuadraticCubeBound.leaf reducedBound905_1

private theorem poly905_1_nonpos
    {mask : SignMask} (hmask : cubexx1x01.Member mask) :
    poly905_1.coeffEval mask <= 0 :=
  cubeBound905_1.coeffEval_nonpos hmask

def reducedObstruction905_1
    (direct_eq :
      forall {mask : SignMask} (hlt : rank905 < numPairWords),
        cubexx1x01.Member mask ->
          weightedDirectWalshDotAtRank
            (⟨rank905, hlt⟩ : Fin numPairWords) mask weights1 =
            poly905_1.coeffEval mask) :
    WeightedWalshQuadraticNonposObstruction rank905 cubexx1x01.Member where
  weights := weights1
  ...
  poly_nonpos := by
    intro mask hmember
    exact poly905_1_nonpos hmember
...
theorem weightedDenom_nonpos_of_reduced_bound
    ...
    (hmember : cubexx1x01.Member mask) :
    weightedDenomAtRank
      (⟨rank905, hlt⟩ : Fin numPairWords) mask weights1 <= 0 :=
  (reducedObstruction905_1 direct_eq).nonpos hlt hmember
...
private theorem dot_sum_eval
    ...
    dot00.coeffEval mask =
      poly905_1.coeffEval mask := by
  simp [dot00, poly905_1,
    WalshQuadratic.coeffEval]
  try ring_nf

theorem direct_eq
    {mask : SignMask} (hlt : rank905 < numPairWords)
    (_hmember : cubexx1x01.Member mask) :
    weightedDirectWalshDotAtRank
        (⟨rank905, hlt⟩ : Fin numPairWords) mask weights1 =
      poly905_1.coeffEval mask := by
...
end WeightedDenomCubeDU9IQDirectBridgeGeneratedIdx56Smoke
