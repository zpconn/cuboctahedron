...
namespace WeightedDenomCubeDU9IQDirectBridgeGeneratedIdx14Smoke
...
def cubexxx001 : MaskSubcube where
  fixed
...
def poly897_1 : WalshQuadratic where
  c := 4
  y := 0
  z := 0
  d111 := 8/9
...
private def reduced897_1 : WalshQuadratic where
  c := -4
  y := 0
  z := 0
  d111 := 8/9
...
private theorem poly897_1_eq_reduced_on_cube
    {mask : SignMask} (hmask : cubexxx001.Member mask) :
    poly897_1.coeffEval mask = reduced897_1.coeffEval mask := by
  have hd11m : maskBitForPair mask SignBit.d11m.toPairId = false := by
    simpa [cubexxx001, MaskSubcube.Member] using hmask SignBit.d11m
  have hd1m1 : maskBitForPair mask SignBit.d1m1.toPairId = false := by
    simpa [cubexxx001, MaskSubcube.Member] using hmask SignBit.d1m1
  have hdm11 : maskBitForPair mask SignBit.dm11.toPairId = true := by
    simpa [cubexxx001, MaskSubcube.Member] using hmask SignBit.dm11
  ...
  all_goals
    ...
    all_goals
      ...
      all_goals
        simp [poly897_1, reduced897_1, WalshQuadratic.coeffEval, SignBit.value, hd11m, hd1m1, hdm11, hy, hz, hd111]
        norm_num

private def reducedBound897_1 :
    WalshQuadraticReducedAbsBound cubexxx001 poly897_1 where
  reduced := reduced897_1
  eq_on_cube := by
    intro mask hmask
    exact poly897_1_eq_reduced_on_cube hmask
  abs_bound_nonpos := by
    norm_num [reduced897_1, WalshQuadratic.absBoundSum]

private def cubeBound897_1 :
    WalshQuadraticCubeBound cubexxx001 poly897_1 :=
  WalshQuadraticCubeBound.leaf reducedBound897_1

private theorem poly897_1_nonpos
    {mask : SignMask} (hmask : cubexxx001.Member mask) :
    poly897_1.coeffEval mask <= 0 :=
  cubeBound897_1.coeffEval_nonpos hmask

def reducedObstruction897_1
    (direct_eq :
      forall {mask : SignMask} (hlt : rank897 < numPairWords),
        cubexxx001.Member mask ->
          weightedDirectWalshDotAtRank
            (⟨rank897, hlt⟩ : Fin numPairWords) mask weights1 =
            poly897_1.coeffEval mask) :
    WeightedWalshQuadraticNonposObstruction rank897 cubexxx001.Member where
  weights := weights1
  ...
  poly_nonpos := by
    intro mask hmember
    exact poly897_1_nonpos hmember
...
theorem weightedDenom_nonpos_of_reduced_bound
    ...
    (hmember : cubexxx001.Member mask) :
    weightedDenomAtRank
      (⟨rank897, hlt⟩ : Fin numPairWords) mask weights1 <= 0 :=
  (reducedObstruction897_1 direct_eq).nonpos hlt hmember
...
private theorem dot_sum_eval
    ...
    dot00.coeffEval mask =
      poly897_1.coeffEval mask := by
  simp [dot00, poly897_1,
    WalshQuadratic.coeffEval]
  try ring_nf

theorem direct_eq
    {mask : SignMask} (hlt : rank897 < numPairWords)
    (_hmember : cubexxx001.Member mask) :
    weightedDirectWalshDotAtRank
        (⟨rank897, hlt⟩ : Fin numPairWords) mask weights1 =
      poly897_1.coeffEval mask := by
...
end WeightedDenomCubeDU9IQDirectBridgeGeneratedIdx14Smoke
