...
namespace WeightedDenomCubeDU9IQDirectBridgeGeneratedIdx43Smoke
...
def poly903_1 : WalshQuadratic where
  c := 4
  y := 0
  z := 0
  d111 := -56/27
...
private def reduced903_1 : WalshQuadratic where
  c := -140/27
  y := 0
  z := 0
  d111 := 0
...
private theorem poly903_1_eq_reduced_on_cube
    {mask : SignMask} (hmask : cubexx1x01.Member mask) :
    poly903_1.coeffEval mask = reduced903_1.coeffEval mask := by
  ...
  all_goals
    ...
    all_goals
      ...
      all_goals
        simp [poly903_1, reduced903_1, WalshQuadratic.coeffEval, SignBit.value, hd111, hd1m1, hdm11, hy, hz, hd11m]
        norm_num

private def reducedBound903_1 :
    WalshQuadraticReducedAbsBound cubexx1x01 poly903_1 where
  reduced := reduced903_1
  eq_on_cube := by
    intro mask hmask
    exact poly903_1_eq_reduced_on_cube hmask
  abs_bound_nonpos := by
    norm_num [reduced903_1, WalshQuadratic.absBoundSum]

private def cubeBound903_1 :
    WalshQuadraticCubeBound cubexx1x01 poly903_1 :=
  WalshQuadraticCubeBound.leaf reducedBound903_1

private theorem poly903_1_nonpos
    {mask : SignMask} (hmask : cubexx1x01.Member mask) :
    poly903_1.coeffEval mask <= 0 :=
  cubeBound903_1.coeffEval_nonpos hmask

def reducedObstruction903_1
    (direct_eq :
      forall {mask : SignMask} (hlt : rank903 < numPairWords),
        cubexx1x01.Member mask ->
          weightedDirectWalshDotAtRank
            (⟨rank903, hlt⟩ : Fin numPairWords) mask weights1 =
            poly903_1.coeffEval mask) :
    WeightedWalshQuadraticNonposObstruction rank903 cubexx1x01.Member where
  weights := weights1
  ...
  poly_nonpos := by
    intro mask hmember
    exact poly903_1_nonpos hmember
...
theorem weightedDenom_nonpos_of_reduced_bound
    ...
    weightedDenomAtRank
      (⟨rank903, hlt⟩ : Fin numPairWords) mask weights1 <= 0 :=
  (reducedObstruction903_1 direct_eq).nonpos hlt hmember
...
private theorem dot_sum_eval
    ...
    dot00.coeffEval mask =
      poly903_1.coeffEval mask := by
  simp [dot00, poly903_1,
...
end WeightedDenomCubeDU9IQDirectBridgeGeneratedIdx43Smoke
