...
Several optimizations are designed to reduce generated data while keeping Lean as the trusted endpoint. Pair-words avoid duplicating linear calculations across opposite signs. Translation sign masks package six independent sign choices into `Fin 64`. Generated certificate chunks are split by rank so Lean can cache and check them locally. The code also has symmetry and canonical-transport machinery for started symmetries fixing `Face.xp`; representative generated cases now pass through a transport checker that derives the raw certificate result from a canonical certificate plus a Lean-checked checker-agreement condition and explicit shape checks, without a separate raw-failure helper. The independent checker also records a full canonical-orbit manifest covering all `97,297,200` pair-words and all `157,957,632` translation choices, with checks that the eight started symmetries are distinct, fix the `x` pair, and are closed under composition.
