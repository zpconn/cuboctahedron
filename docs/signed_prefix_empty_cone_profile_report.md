...
- The remaining frontier is still about `2.54B` signed completions, so the next
  profiler gate must either go deeper or quotient frontier states by holonomy,
  axis, reduced-shadow, or cone-signature invariants.

Decision:

- Promote signed-state empty-cone/Gordan pruning as the next nontranslation
  compression line.
- Do not emit Lean leaves yet.
- Next run should test depth `7` and possibly `8` under the same memory-safe
  parallel mode, while adding frontier-state clustering so the profiler reports
  whether the surviving frontier has reusable holonomy/signature structure.
