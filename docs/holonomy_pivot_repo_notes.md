...
Thirtieth split path:

- Path object index `29` / rank `947589` was run as a single guarded path with
  `--check-stage missing`.  No batch execution or parallel Lean was used.
- Dry-run command:

  ```bash
  python3 scripts/run_bellman_split_smoke_path.py 29 \
    --check \
    --check-stage missing \
    --dry-run \
    --json scripts/generated/bellman_split_path_29_missing_dry_run.json
  ```
...
- Proof-bearing command:
  ...
  python3 scripts/run_bellman_split_smoke_path.py 29 \
    --check \
    --check-stage missing \
    --json scripts/generated/bellman_split_path_29_missing_run.json
  ```

- Result:
  - trace shard with `.olean` emission: passed in `7.51s`, peak RSS
    `4030 MiB`, hard-AS cap `6144 MiB`, minimum available `46192 MiB`;
  - split root with `.olean` emission: passed in `2.00s`, peak RSS
    `3974 MiB`, hard-AS cap `6144 MiB`, minimum available `46280 MiB`.
- Refreshed strict `[0,16)` dry-run guard remains `accepted-dry-run` with `0`
  blocked entries and `0` total blockers.
- Refreshed `[0,37)` planner summary: `0` over budget, `30` fresh trace
  artifacts, `30` fresh split artifacts, `1184 KiB` planned trace source, and
  `74 KiB` planned split source.
- Refreshed selector chose path index `30`, rank `947591`, as the next possible
  single-path target.

Decision: accepted as another proof-bearing sampled path under the strict
post-crash guard.

Thirty-first split path:

- Path object index `30` / rank `947591` was run as a single guarded path with
  `--check-stage missing`.  No batch execution or parallel Lean was used.
- Dry-run command:

  ```bash
  python3 scripts/run_bellman_split_smoke_path.py 30 \
    --check \
    --check-stage missing \
    --dry-run \
    --json scripts/generated/bellman_split_path_30_missing_dry_run.json
  ```
...
- Proof-bearing command:
  ...
  python3 scripts/run_bellman_split_smoke_path.py 30 \
    --check \
    --check-stage missing \
    --json scripts/generated/bellman_split_path_30_missing_run.json
  ```

- Result:
  - trace shard with `.olean` emission: passed in `7.01s`, peak RSS
    `4030 MiB`, hard-AS cap `6144 MiB`, minimum available `46201 MiB`;
  - split root with `.olean` emission: passed in `2.00s`, peak RSS
    `3613 MiB`, hard-AS cap `6144 MiB`, minimum available `46388 MiB`.
- Refreshed strict `[0,16)` dry-run guard remains `accepted-dry-run` with `0`
  blocked entries and `0` total blockers.
- Refreshed `[0,37)` planner summary: `0` over budget, `31` fresh trace
  artifacts, `31` fresh split artifacts, `1184 KiB` planned trace source, and
  `74 KiB` planned split source.
- Refreshed selector chose path index `31`, rank `947593`, as the next possible
  single-path target.

Decision: accepted as another proof-bearing sampled path under the strict
post-crash guard.

Thirty-second split path:

- Path object index `31` / rank `947593` was run as a single guarded path with
  `--check-stage missing`.  No batch execution or parallel Lean was used.
- Dry-run command:
...
- Result:
  - trace shard with `.olean` emission: passed in `9.51s`, peak RSS
    `4023 MiB`, hard-AS cap `6144 MiB`, minimum available `46202 MiB`;
  - split root with `.olean` emission: passed in `2.00s`, peak RSS
    `3971 MiB`, hard-AS cap `6144 MiB`, minimum available `46278 MiB`.
- Refreshed strict `[0,16)` dry-run guard remains `accepted-dry-run` with `0`
  blocked entries and `0` total blockers.
- Refreshed `[0,37)` planner summary: `0` over budget, `32` fresh trace
  artifacts, `32` fresh split artifacts, `1184 KiB` planned trace source, and
  `74 KiB` planned split source.
- Refreshed selector chose path index `32`, rank `947599`, as the next possible
  single-path target.

Decision: accepted as another proof-bearing sampled path under the strict
post-crash guard.

Thirty-third split path:

- Path object index `32` / rank `947599` was run as a single guarded path with
  `--check-stage missing`.  No batch execution or parallel Lean was used.
- Dry-run command:

  ```bash
  python3 scripts/run_bellman_split_smoke_path.py 32 \
    --check \
    --check-stage missing \
    --dry-run \
    --json scripts/generated/bellman_split_path_32_missing_dry_run.json
  ```
...
- Proof-bearing command:
  ...
  python3 scripts/run_bellman_split_smoke_path.py 32 \
    --check \
    --check-stage missing \
    --json scripts/generated/bellman_split_path_32_missing_run.json
  ```

- Result:
  - trace shard with `.olean` emission: passed in `7.51s`, peak RSS
    `4028 MiB`, hard-AS cap `6144 MiB`, minimum available `46201 MiB`;
  - split root with `.olean` emission: passed in `2.00s`, peak RSS
    `3619 MiB`, hard-AS cap `6144 MiB`, minimum available `46398 MiB`.
- Refreshed strict `[0,16)` dry-run guard remains `accepted-dry-run` with `0`
  blocked entries and `0` total blockers.
- Refreshed `[0,37)` planner summary: `0` over budget, `33` fresh trace
  artifacts, `33` fresh split artifacts, `1184 KiB` planned trace source, and
  `74 KiB` planned split source.
- Refreshed selector chose path index `33`, rank `947601`, as the next possible
  single-path target.

Decision: accepted as another proof-bearing sampled path under the strict
post-crash guard.

Thirty-fourth split path:

- Path object index `33` / rank `947601` was run as a single guarded path with
  `--check-stage missing`.  No batch execution or parallel Lean was used.
- Dry-run command:

  ```bash
  python3 scripts/run_bellman_split_smoke_path.py 33 \
    --check \
    --check-stage missing \
    --dry-run \
    --json scripts/generated/bellman_split_path_33_missing_dry_run.json
  ```
...
- Proof-bearing command:
  ...
  python3 scripts/run_bellman_split_smoke_path.py 33 \
    --check \
    --check-stage missing \
    --json scripts/generated/bellman_split_path_33_missing_run.json
  ```

- Result:
  - trace shard with `.olean` emission: passed in `7.51s`, peak RSS
    `4028 MiB`, hard-AS cap `6144 MiB`, minimum available `46204 MiB`;
  - split root with `.olean` emission: passed in `2.00s`, peak RSS
    `3968 MiB`, hard-AS cap `6144 MiB`, minimum available `46284 MiB`.
- Refreshed strict `[0,16)` dry-run guard remains `accepted-dry-run` with `0`
  blocked entries and `0` total blockers.
- Refreshed `[0,37)` planner summary: `0` over budget, `34` fresh trace
  artifacts, `34` fresh split artifacts, `1184 KiB` planned trace source, and
  `74 KiB` planned split source.
- Refreshed selector chose path index `34`, rank `947625`, as the next possible
  single-path target.

Decision: accepted as another proof-bearing sampled path under the strict
post-crash guard.

Thirty-fifth split path:

- Path object index `34` / rank `947625` was run as a single guarded path with
  `--check-stage missing`.  No batch execution or parallel Lean was used.
- Dry-run command:

  ```bash
  python3 scripts/run_bellman_split_smoke_path.py 34 \
    --check \
    --check-stage missing \
    --dry-run \
    --json scripts/generated/bellman_split_path_34_missing_dry_run.json
  ```
...
- Proof-bearing command:
  ...
  python3 scripts/run_bellman_split_smoke_path.py 34 \
    --check \
    --check-stage missing \
    --json scripts/generated/bellman_split_path_34_missing_run.json
  ```

- Result:
  - trace shard with `.olean` emission: passed in `7.51s`, peak RSS
    `4025 MiB`, hard-AS cap `6144 MiB`, minimum available `46208 MiB`;
  - split root with `.olean` emission: passed in `2.00s`, peak RSS
    `3606 MiB`, hard-AS cap `6144 MiB`, minimum available `46408 MiB`.
- Refreshed strict `[0,16)` dry-run guard remains `accepted-dry-run` with `0`
  blocked entries and `0` total blockers.
- Refreshed `[0,37)` planner summary: `0` over budget, `35` fresh trace
  artifacts, `35` fresh split artifacts, `1184 KiB` planned trace source, and
  `74 KiB` planned split source.
- Refreshed selector chose path index `35`, rank `947626`, as the next possible
  single-path target.

Decision: accepted as another proof-bearing sampled path under the strict
post-crash guard.

Thirty-sixth split path:

- Path object index `35` / rank `947626` was run as a single guarded path with
  `--check-stage missing`.  No batch execution or parallel Lean was used.
- Dry-run command:

  ```bash
  python3 scripts/run_bellman_split_smoke_path.py 35 \
    --check \
    --check-stage missing \
    --dry-run \
    --json scripts/generated/bellman_split_path_35_missing_dry_run.json
  ```
...
- Proof-bearing command:
  ...
  python3 scripts/run_bellman_split_smoke_path.py 35 \
    --check \
    --check-stage missing \
    --json scripts/generated/bellman_split_path_35_missing_run.json
  ```

- Result:
  - trace shard with `.olean` emission: passed in `8.10s`, peak RSS
    `4028 MiB`, hard-AS cap `6144 MiB`, minimum available `46206 MiB`;
  - split root with `.olean` emission: passed in `2.03s`, peak RSS
    `3965 MiB`, hard-AS cap `6144 MiB`, minimum available `46298 MiB`.
- Refreshed strict `[0,16)` dry-run guard remains `accepted-dry-run` with `0`
  blocked entries and `0` total blockers.
- Refreshed `[0,37)` planner summary: `0` over budget, `36` fresh trace
  artifacts, `36` fresh split artifacts, `1184 KiB` planned trace source, and
  `74 KiB` planned split source.
- Refreshed selector chose path index `36`, rank `947627`, as the next possible
  single-path target.
